...
class \User extends \DSF_Db_Table 
{
	...
	{
		//validate password
		$newPwd = \DSF\Filter\Post::get('newPassword');
		$confirm = \DSF\Filter\Post::get('newConfirmPassword');
		if($newPwd == $confirm)
		{
			$this->data['password'] = md5($newPwd);			
		}else{
			$e = new \DSF\View\Error();
	...
	{ 
		...
		if($curr->email == \DSF\Filter\Post::raw('email')){
			unset($this->Unique[array_search('email',$this->Unique)]);
		}
		
		//update the password
		if(\DSF\Filter\Post::int('update_password') == 1)
		{ 
			$newPwd = \DSF\Filter\Post::get('newPassword');
			$confirm = \DSF\Filter\Post::get('newConfirmPassword');
			if($newPwd == $confirm)
	...
	{
		$currentUser = \DSF\Auth::getIdentity();
		if($currentUser) {
