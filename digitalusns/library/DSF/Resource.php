...
class \DSF_File
{
	...
	public function upload($key, $path, $filename = false, $createPath = true)
	{
		if(self::isUploaded($key))
		{
		    ...
		    $path = \DSF\Toolbox\ToolboxString::stripLeading('/', $path);
		    if($createPath)
		    {
		        //attempt to create the new path
		        \DSF\Filesystem\Dir::makeRecursive(self::PATH_TO_FileS, $path);
		    }
		    
		    //clean the filename
		    $filename = \DSF\Filesystem\File::cleanFilename($filename);
		    $filename = \DSF\Toolbox\ToolboxString::getSelfFromPath($filename);
		    $fullPath .= "/" . $filename;
		    if(move_uploaded_file($_FILES[$key]['tmp_name'], $fullPath))
		    {
		        ...
		        $fullPath = \DSF\Toolbox\ToolboxString::stripLeading('./', $fullPath);
		        return $fullPath;
		    }
		}
	}
