...
namespace DSF\Controller\Plugin;
...
use DSF\View\RegisterHelpers as RegisterHelpers;
use zend\controller\request as request;
...
class  LayoutLoader  extends \Zend_Controller_Plugin_Abstract
{
    public $pathToViews;
    public $view;
    public $layout;
    ...
    public function preDispatch( ControllerRequestAbstract  $request)
    {
        if(!$request->isXmlHttpRequest()) {
    		...
    		if($this->isAdminPage($request))
    		{
    		    ...
    		    $this->layout =  Layout ::startMvc($options);
    		    $this->view = $this->layout->getView();
    		    
    		    //load the common helpers
                 RegisterHelpers ::register($this->view);
                ...
                foreach ($config->language->translations as $locale => $translation){
                    if(is_object($translate)) {
                       $translate->addTranslation($config->language->path . '/' . $translation . '.csv', $locale); 
                        ...
                        $translate = new  Translate ('csv', $config->language->path . '/' . $translation . '.csv', $locale);
                    }
                }
                $locale = $config->language->defaultLocale;
                $translate->setLocale($locale);
                $translate->setCache( Registry ::get('cache'));
