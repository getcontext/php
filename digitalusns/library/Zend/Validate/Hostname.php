...
use Zend\Validate\Ip as Ip;
use Zend\Loader as Loader;




class  Hostname  extends  ValidateAbstract 
{
     ...
     */
    public function __construct($allow = self::ALLOW_DNS, $validateIdn = true, $validateTld = true,  Ip  $ipValidator = null)
    {
        // Set allow options
        $this->setAllow($allow);
     ...
     *
     * @param  string $value
     * @throws  ValidateException  if a fatal error occurs \for validation process
     * @return boolean
     */
    public function isValid($value)
    {
        ...
        if ((count($domainParts) > 1) && (strlen($valueString) >= 4) && (strlen($valueString) <= 254)) {
            ...
            do {
                ...
                if (preg_match('/([a-z]{2,10})$/i', end($domainParts), $matches)) {
                     ...
                     * @see  Hostname _Interface
                     */
                    $labelChars = 'a-z0-9';
                    $utf8 = false;
                    $classFile = 'Zend/Validate/Hostname/' . ucfirst($this->_tld) . '.php';
                    if ($this->_validateIdn) {
                        if ( Loader ::isReadable($classFile)) {

                            // Load additional characters
                            $className = '\Zend\Validate\Hostname_' . ucfirst($this->_tld);
                    ...
                    foreach ($domainParts as $domainPart) {
                        ...
                        if ($status === false) {
                            ...
                            throw new  ValidateException ('Internal error: DNS validation failed');
                        } elseif ($status === 0) {
                            $valid = false;
                        }
                    }
        ...
        if (false === $status) {
             ...
             */
            require_once 'Zend/Validate/Exception.php';
            throw new  ValidateException ('Internal error: local network name validation failed');
        }
    ...
    protected function _checkRegexType($type)
    {
        if (!isset($this->_regex[$type])) {
            require_once 'Zend/Validate/Exception.php';
            throw new  ValidateException ("'$type' must be one \of ('" . implode(', ', array_keys($this->_regex))
                                            . "')");
        }
        return $this;
    }
