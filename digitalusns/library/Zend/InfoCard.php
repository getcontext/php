...
use Zend\InfoCard\Xml\Assertion\AssertionInterface as InfoCardXmlAssertionInterface;
use Zend\InfoCard\Cipher\Pki\PkiInterface as InfoCardCipherPkiInterface;
use Zend\InfoCard\Xml\KeyInfo\XmlDSig as XmlDSig;
use Zend\InfoCard\Xml\EncryptedData as EncryptedData;
use Zend\InfoCard\Adapter\AdapterInterface as InfoCardAdapterInterface;
use Zend\InfoCard\Adapter\AdapterDefault as InfoCardAdapterDefault;
use Zend\InfoCard\Xml\Assertion as Assertion;
use Zend\InfoCard\Xml\Security as Security;
use Zend\InfoCard\Exception as InfoCardException;
...
class  InfoCard 
{
     ...
     *
     * @var  InfoCardCipherPkiInterface 
     */
     ...
     *
     * @var  InfoCardCipherSymmetricInterface 
     */
    ...
    public function __construct()
    {
        ...
        if(!extension_loaded('mcrypt')) {
            throw new  InfoCardException ("Use \of the  InfoCard  component requires the mcrypt extension to be enabled in PHP");
        }

        if(!extension_loaded('openssl')) {
            throw new  InfoCardException ("Use \of the  InfoCard  component requires the openssl extension to be enabled in PHP");
        }
    }
     ...
     *
     * @param  InfoCardAdapterInterface  $a The Adapter instance
     * @return  InfoCard  The instnace
     */
     ...
     * If no adapter was set then an instance \of  InfoCardAdapterDefault  is used
     *
     * @return  InfoCardAdapterInterface  The Adapter instance
     */
    public function getAdapter()
    {
        if(is_null($this->_adapter)) {
             Loader ::loadClass('\Zend\InfoCard\Adapter\AdapterDefault');
            $this->setAdapter(new  InfoCardAdapterDefault ());
        }
    ...
    public function removeCertificatePair($key_id)
    {
        ...
        if(!key_exists($key_id, $this->_keyPairs)) {
            throw new  InfoCardException ("Attempted to remove unknown key id: $key_id");
        }
    ...
    public function addCertificatePair($private_key_file, $public_key_file, $type =  Cipher ::ENC_RSA_OAEP_MGF1P, $password = null)
    {
        if(!file_exists($private_key_file) ||
           !file_exists($public_key_file)) {
               throw new  InfoCardException ("Could not locate the public and private certificate pair files: $private_key_file, $public_key_file");
        }

        if(!is_readable($private_key_file) ||
           !is_readable($public_key_file)) {
               throw new  InfoCardException ("Could not read the public and private certificate pair files (check permissions): $private_key_file, $public_key_file");
          }

          $key_id = md5($private_key_file.$public_key_file);

          if(key_exists($key_id, $this->_keyPairs)) {
              throw new  InfoCardException ("Attempted to add previously existing certificate pair: $private_key_file, $public_key_file");
          }
          ...
          switch($type) {
              ...
              case  Cipher ::ENC_RSA_OAEP_MGF1P:
                ...
                return $key_id;
                  break;
              default:
                  throw new  InfoCardException ("Invalid Certificate Pair Type specified: $type");
          }
    }
    ...
    public function getCertificatePair($key_id)
    {
        ...
        throw new  InfoCardException ("Invalid Certificate Pair ID provided: $key_id");
    }
    ...
    protected function _getPublicKeyDigest($key_id, $digestMethod = self::DIGEST_SHA1)
    {
        ...
        switch($digestMethod) {
            case self::DIGEST_SHA1:
                ...
                break;
            default:
                throw new  InfoCardException ("Invalid Digest Type Provided: $digestMethod");
        }
    ...
    {
        $encryptedData =  EncryptedData ::getInstance($strXmlToken);
        ...
        switch($encryptedData->getEncryptionMethod()) {
            ...
            case  Cipher ::ENC_AES256CBC:
                break;
            default:
                throw new  InfoCardException ("Unknown Encryption Method used in the secure token");
        }
        ...
        if(!($keyinfo instanceof  XmlDSig )) {
            throw new  InfoCardException ("Expected a XML digital signature KeyInfo, but was not found");
        }
        ...
        switch($encryptedKey->getEncryptionMethod()) {
            ...
            case  Cipher ::ENC_RSA_OAEP_MGF1P:
                break;
            default:
                throw new  InfoCardException ("Unknown Key Encryption Method used in secure token");
        }
        ...
        if(!$key_id) {
            throw new  InfoCardException ("Unable to find key pair used to encrypt symmetric InfoCard Key");
        }
        ...
        if($certificate_pair['type_uri'] != $encryptedKey->getEncryptionMethod()) {
            throw new  InfoCardException ("Certificate Pair which matches digest is not \of same algorithm type as document, check addCertificate()");
        }
    ...
    public function process($strXmlToken)
    {
        ...
        try {
            $assertions =  Assertion ::getInstance($signedAssertionsXml);
        } catch( InfoCardException  $e) {
        ...
        if(!($assertions instanceof  InfoCardXmlAssertionInterface )) {
            throw new  InfoCardException ("Invalid Assertion Object returned");
        }

        if(!($reference_id =  Security ::validateXMLSignature($assertions->asXML()))) {
