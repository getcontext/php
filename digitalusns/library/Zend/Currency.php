...
use Zend\Currency\Exception as CurrencyException;
...
class  Currency 
{
     ...
     * @param  string| Locale  $locale   OPTIONAL locale name
     * @throws  CurrencyException  When currency is invalid
     */
    public function __construct($currency = null, $locale = null)
    {
        if ( Locale ::isLocale($currency) !== false) {
            $temp     = $locale;
            $locale   = $currency;
            $currency = $temp;
        }
    ...
    /**
     * Gets the information required \for formating the currency from  Locale 
     *
     * @return  Currency 
     */
    protected function _updateFormat()
    {
        ...
        $format =  LocaleData ::getContent($locale, 'currencynumber');
    ...
    public function toCurrency($value, array $options = array())
    {
        ...
        if ((isset($value) === false) or (is_numeric($value) === false)) {
            require_once 'Zend/Currency/Exception.php';
            throw new  CurrencyException ("Value '$value' has to be numeric");
        }
        ...
        $value =  Format ::toNumber($value, array('locale' => $options['format'],
                                                            'precision' => $options['precision']));

        // Localize the number digits
        if (empty($options['script']) === false) {
            $value =  Format ::convertNumerals($value, 'Latn', $options['script']);
        }
     ...
     * @param  string| Locale  $locale   (Optional) Locale to display informations
     * @throws  CurrencyException  When locale contains no region
    ...
    private function _checkParams($currency = null, $locale = null)
    {
        ...
        if ((empty($locale) === true) and (empty($currency) === false) and
            ( Locale ::isLocale($currency) !== false)) {
            $locale   = $currency;
            $currency = null;
        }
        ...
        $country = null;
        $locale  =  Locale ::isLocale($locale);
        if (($locale !== false) and (strlen($locale) > 4)) {
            $country = substr($locale, (strpos($locale, '_') + 1));
        } else {
            require_once 'Zend/Currency/Exception.php';
            throw new  CurrencyException ("No region found within the locale '$locale'");
        }

        // Get the available currencies \for this country
        $data =  LocaleData ::getContent($locale, 'currencytoregion', $country);
    ...
    public function getSymbol($currency = null, $locale = null)
    {
        ...
        // Get the symbol
        $symbol =  LocaleData ::getContent($params['locale'], 'currencysymbol', $params['currency']);
        if (empty($symbol) === true) {
            $symbol =  LocaleData ::getContent($params['locale'], 'currencysymbol', $params['name']);
        }
     ...
     * @param  string| Locale  $locale   OPTIONAL The locale
     * @return string
     */
    public function getShortName($currency = null, $locale = null)
    {
        ...
        $list =  LocaleData ::getContent($params['locale'], 'currencytoname', $params['currency']);
        if (empty($list) === true) {
            $list =  LocaleData ::getContent($params['locale'], 'nametocurrency', $params['currency']);
            if (empty($list) === false) {
    ...
    public function getName($currency = null, $locale = null)
    {
        ...
        // Get the name
        $name =  LocaleData ::getContent($params['locale'], 'nametocurrency', $params['currency']);
        if (empty($name) === true) {
            $name =  LocaleData ::getContent($params['locale'], 'nametocurrency', $params['name']);
        }
    ...
    public function getRegionList($currency = null)
    {
        ...
        if (empty($currency) === true) {
            require_once 'Zend/Currency/Exception.php';
            throw new  CurrencyException ('No currency defined');
        }

        $data =  LocaleData ::getContent('', 'regiontocurrency', $currency);
    ...
    public function getCurrencyList($region = null)
    {
        ...
        return  LocaleData ::getList('', 'regiontocurrency', $region);
    }
     ...
     *
     * @param   Core  $cache Cache to set
     * @return void
     */
    public static function setCache( Core  $cache)
    {
         LocaleData ::setCache($cache);
    }
    ...
    public function setLocale($locale = null)
    {
        if ($locale instanceof  Locale ) {
            ...
            $this->_locale =  Locale ::isLocale($locale, true);
            if ($this->_locale === false) {
                require_once 'Zend/Currency/Exception.php';
                throw new  CurrencyException ("Given locale ($locale) does not exist");
            }
        }
     ...
     * @throws  CurrencyException  On unknown position
     * @throws  CurrencyException  On unknown locale
     * @throws  CurrencyException  On unknown display
     * @throws  CurrencyException  On precision not between -1 and 30
     * @throws  CurrencyException  On problem with script conversion
     * @throws  CurrencyException  On unknown options
     * @return array
     */
            ...
            switch($name) {
                case 'position':
                    if (($value !== self::STANDARD) and ($value !== self::RIGHT) and ($value !== self::LEFT)) {
                        require_once 'Zend/Currency/Exception.php';
                        throw new  CurrencyException ("Unknown position '" . $value . "'");
                    }
                ...
                case 'format':
                    if ((empty($value) === false) and ( Locale ::isLocale($value) === false)) {
                        require_once 'Zend/Currency/Exception.php';
                ...
                case 'display':
                    if (is_numeric($value) and ($value !== self::NO_SYMBOL) and ($value !== self::USE_SYMBOL) and
                        ...
                        require_once 'Zend/Currency/Exception.php';
                        throw new  CurrencyException ("Unknown display '$value'");
                    }
                    break;
                ...
                case 'precision':
                    ...
                    if (($value < -1) or ($value > 30)) {
                        require_once 'Zend/Currency/Exception.php';
                        throw new  CurrencyException ("'$value' precision has to be between -1 and 30.");
                    }
                    break;

                case 'script':
                    try {
                         Format ::convertNumerals(0, $options['script']);
                ...
                default:
                    require_once 'Zend/Currency/Exception.php';
                    throw new  CurrencyException ("Unknown option: '$name' = '$value'");
                    break;
            }
        }
