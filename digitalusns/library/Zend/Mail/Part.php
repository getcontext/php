...
use Zend\Mail\Part\PartInterface as MailPartInterface;
use Zend\Mail\Exception as MailException;
use Zend\Mime\Decode as Decode;
use Zend\Mime as Mime;




class  Part  implements \RecursiveIterator,  MailPartInterface 
{
    /**
     ...
     * - handler    a instance \of  MailStorageAbstract  \for late fetch
    ...
    public function __construct(array $params)
    {
        if (isset($params['handler'])) {
            if (!$params['handler'] instanceof  MailStorageAbstract ) {
                /**
                 * @see  MailException 
                 */
                require_once 'Zend/Mail/Exception.php';
                throw new  MailException ('handler is not a valid mail handler');
            }
            if (!isset($params['id'])) {
                /**
                 * @see  MailException 
                 */
                require_once 'Zend/Mail/Exception.php';
                throw new  MailException ('need a message id with a handler');
            }
        ...
        if (isset($params['raw'])) {
             Decode ::splitMessage($params['raw'], $this->_headers, $this->_content);
        } else if (isset($params['headers'])) {
            if (is_array($params['headers'])) {
                $this->_headers = $params['headers'];
            } else {
                if (!empty($params['noToplines'])) {
                     Decode ::splitMessage($params['headers'], $this->_headers, $null);
                } else {
                     Decode ::splitMessage($params['headers'], $this->_headers, $this->_topLines);
                }
            }
    ...
    public function getContent()
    {
        ...
        if ($this->_mail) {
             ...
             */
            require_once 'Zend/Mail/Exception.php';
            throw new  MailException ('no content');
        }
    }
    ...
    protected function _cacheContent()
    {
        ...
        $parts =  Decode ::splitMessageStruct($this->_content, $boundary);
        $counter = 1;
        ...
        if ($this->_headers === null) {
            if (!$this->_mail) {
                ...
                $part = $this->_mail->getRawHeader($this->_messageNum);
                 Decode ::splitMessage($part, $this->_headers, $null);
            }
        }
    ...
    public function getHeader($name, $format = null)
    {
        ...
        if (!isset($this->_headers[$lowerName])) {
            ...
            if (!isset($this->_headers[$lowerName])) {
                 ...
                 */
                require_once 'Zend/Mail/Exception.php';
                throw new  MailException ("no Header with Name $name found");
            }
        }
        $name = $lowerName;
        ...
        switch ($format) {
            case 'string':
                if (is_array($header)) {
                    $header = implode( Mime ::LINEEND, $header);
                }
                break;
            case 'array':
                $header = (array)$header;
     ...
     * is returned.
     *
     * Throws a  MailException  if the requested header does not exist. If
     ...
     * @throws \Zend_Exception,  MailException 
     */
    public function getHeaderField($name, $wantedPart = 0, $firstName = 0) {
    	return  Decode ::splitHeaderField(current($this->getHeader($name, 'array')), $wantedPart, $firstName);
    }
