...
/**
 * @see  MailStorageFolderMaildir 
 */
require_once 'Zend/Mail/Storage/Folder/Maildir.php';

/**
 * @see  MailStorageWritableInterface 
 */
...
use Zend\Mail\Storage\Writable\WritableInterface as MailStorageWritableInterface;
use Zend\Mail\Storage\Folder\Maildir as MailStorageFolderMaildir;
use Zend\Mail\Storage\Exception as MailStorageException;
use Zend\Mail\Storage\Maildir as MailStorageMaildir;
...
class  Maildir  extends     MailStorageFolderMaildir 
                                         implements  MailStorageWritableInterface 
{
	/**
     ...
     * @param   string| Folder  $parentFolder parent folder \for new folder, else root folder is parent
    ...
    public function createFolder($name, $parentFolder = null)
    {
        ...
        if ($exists) {
            ...
            throw new  MailStorageException ('folder already exists');
        }
        ...
        if (strpos($folder, $this->_delim . $this->_delim) !== false) {
             ...
             */
            require_once 'Zend/Mail/Storage/Exception.php';
            throw new  MailStorageException ('invalid name - folder parts may not be empty');
        }
        ...
        if (strpos($folder, DIRECTORY_SEPARATOR) !== false || strpos($folder, '/') !== false
             ...
             */
            require_once 'Zend/Mail/Storage/Exception.php';
            throw new  MailStorageException ('invalid name - no directory seprator allowed in folder name');
        }
        ...
        if (!@mkdir($fulldir) || !@mkdir($fulldir . DIRECTORY_SEPARATOR . 'cur')) {
             ...
             */
            require_once 'Zend/Mail/Storage/Exception.php';
            throw new  MailStorageException ('error while creating new folder, may be created incompletly');
        }
    ...
    public function removeFolder($name)
    {
        ...
        if (!$this->getFolders($name)->isLeaf()) {
             ...
             */
            require_once 'Zend/Mail/Storage/Exception.php';
            throw new  MailStorageException ('delete children first');
        }
        ...
        if ($name == 'INBOX' || $name == DIRECTORY_SEPARATOR || $name == '/') {
             ...
             */
            require_once 'Zend/Mail/Storage/Exception.php';
            throw new  MailStorageException ('wont delete INBOX');
        }
        ...
        if ($name == $this->getCurrentFolder()) {
             ...
             */
            require_once 'Zend/Mail/Storage/Exception.php';
            throw new  MailStorageException ('wont delete selected folder');
        }
        ...
        foreach (array('tmp', 'new', 'cur', '.') as $subdir) {
            ...
            if (!$dh) {
                 ...
                 */
                require_once 'Zend/Mail/Storage/Exception.php';
                throw new  MailStorageException ("error opening $subdir");
            }
            while (($entry = readdir($dh)) !== false) {
                ...
                if (!unlink($dir . DIRECTORY_SEPARATOR . $entry)) {
                     ...
                     */
                    require_once 'Zend/Mail/Storage/Exception.php';
                    throw new  MailStorageException ("error cleaning $subdir");
                }
            }
            closedir($dh);
            if ($subdir !== '.') {
                if (!rmdir($dir)) {
                     ...
                     */
                    require_once 'Zend/Mail/Storage/Exception.php';
                    throw new  MailStorageException ("error removing $subdir");
                }
            }
        }
        ...
        if (!rmdir($this->_rootdir . '.' . $name)) {
             ...
             */
            require_once 'Zend/Mail/Storage/Exception.php';
            throw new  MailStorageException ("error removing maindir");
        }
    ...
    public function renameFolder($oldName, $newName)
    {
        ...
        if (strpos($newName, $oldName . $this->_delim) === 0) {
             ...
             */
            require_once 'Zend/Mail/Storage/Exception.php';
            throw new  MailStorageException ('new folder cannot be a child \of old folder');
        }
        ...
        if ($oldName == 'INBOX' || $oldName == DIRECTORY_SEPARATOR || $oldName == '/') {
             ...
             */
            require_once 'Zend/Mail/Storage/Exception.php';
            throw new  MailStorageException ('wont rename INBOX');
        }
        ...
        if ($oldName == $this->getCurrentFolder()) {
             ...
             */
            require_once 'Zend/Mail/Storage/Exception.php';
            throw new  MailStorageException ('wont rename selected folder');
        }
        ...
        foreach (array('tmp', 'new', 'cur') as $subdir) {
            ...
            if (!rename($olddir . $subdir, $newdir . $subdir)) {
                 ...
                 */
                require_once 'Zend/Mail/Storage/Exception.php';
                throw new  MailStorageException ('error while moving ' . $subdir);
            }
        }
    ...
    protected function _createTmpFile($folder = 'INBOX')
    {
        ...
        if (!file_exists($tmpdir)) {
            if (!mkdir($tmpdir)) {
                 ...
                 */
                require_once 'Zend/Mail/Storage/Exception.php';
                throw new  MailStorageException ('problems creating tmp dir');
            }
        }
        ...
        \for ($i = 0; $i < $max_tries; ++$i) {
            ...
            if (!file_exists($tmpdir . $uniq)) {
                ...
                if (!$fh) {
                     ...
                     */
                    require_once 'Zend/Mail/Storage/Exception.php';
                    throw new  MailStorageException ('could not open temp file');
                }
                break;
            }
            sleep(1);
        ...
        if (!$fh) {
             ...
             */
            require_once 'Zend/Mail/Storage/Exception.php';
            throw new  MailStorageException ("tried $max_tries unique ids \for a temp file, but all were taken"
                                                . ' - giving up');
        }
    ...
    protected function _getInfoString(&$flags)
    {
        ...
        if (isset($wanted_flags[ Storage ::FLAG_RECENT])) {
             ...
             */
            require_once 'Zend/Mail/Storage/Exception.php';
            throw new  MailStorageException ('recent flag may not be set');
        }

        $info = ':2,';
        $flags = array();
        foreach ( MailStorageMaildir ::$_knownFlags as $char => $flag) {
        ...
        if (!empty($wanted_flags)) {
             ...
             */
            require_once 'Zend/Mail/Storage/Exception.php';
            throw new  MailStorageException ('unknown flag(s): ' . $wanted_flags);
        }
     ...
     * @param   null|string| Folder        $folder  folder \for new message, else current folder is taken
     * @param   null|array                                 $flags   set flags \for new message, else a default set is used
     * @param   bool                                       $recent  handle this mail as if recent flag has been set,
     *                                                              should only be used in delivery
     * @throws   MailStorageException 
     */
     // not yet * @param string|\Zend\Mail\Message| Message  $message message as string or instance \of message class
    ...
    public function appendMessage($message, $folder = null, $flags = null, $recent = false)
    {
    	if ($this->_quota && $this->checkQuota()) {
             ...
             */
            require_once 'Zend/Mail/Storage/Exception.php';
            throw new  MailStorageException ('storage is over quota!');    		
    	}
        ...
        if (!link($temp_file['filename'], $new_filename)) {
             ...
             */
            require_once 'Zend/Mail/Storage/Exception.php';
            $exception = new  MailStorageException ('cannot link message file to final dir');
        }
    ...
    public function copyMessage($id, $folder)
    {
        ...
        if (!copy($old_file, $temp_file['filename'])) {
             ...
             */
            require_once 'Zend/Mail/Storage/Exception.php';
            $exception = new  MailStorageException ('cannot copy message file');
    ...
    public function moveMessage($id, $folder) {
        ...
        if ($folder->getGlobalName() == $this->_currentFolder
             ...
             */
            require_once 'Zend/Mail/Storage/Exception.php';
            throw new  MailStorageException ('target is current folder');
        }
        ...
        if (!rename($old_file, $new_file)) {
             ...
             */
            require_once 'Zend/Mail/Storage/Exception.php';
            $exception = new  MailStorageException ('cannot move message file');
        }
        @unlink($temp_file['filename']);
    ...
    public function setFlags($id, $flags)
    {
        ...
        if (!@rename($filedata['filename'], $new_filename)) {
             ...
             */
            require_once 'Zend/Mail/Storage/Exception.php';
            throw new  MailStorageException ('cannot rename file');
        }
    ...
    public function removeMessage($id)
    {
        ...
        if (!@unlink($filename)) {
             ...
             */
            require_once 'Zend/Mail/Storage/Exception.php';
            throw new  MailStorageException ('cannot remove message');
        }
        unset($this->_files[$id - 1]);
        // remove the gap
        $this->_files = array_values($this->_files);
    ...
    public function getQuota($fromStorage = false) {
    	if ($fromStorage) {
			...
			if (!$fh) {
				 ...
				 */
				require_once 'Zend/Mail/Storage/Exception.php';
				throw new  MailStorageException ('cannot open maildirsize');
			}
			$definition = fgets($fh);
			fclose($fh);
    ...
    protected function _calculateMaildirsize() {
		...
		if (is_array($this->_quota)) {
			...
			try {
				...
				throw new  MailStorageException ('no quota defintion found');
			}
		}
