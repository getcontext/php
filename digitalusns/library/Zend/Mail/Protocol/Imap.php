...
class  Imap 
{
    ...
    public function connect($host, $port = null, $ssl = false)
    {
        ...
        if (!$this->_assumedNextLine('* OK')) {
             ...
             */
            require_once 'Zend/Mail/Protocol/Exception.php';
            throw new  MailProtocolException ('host doesn\'t allow connection');
        }
    ...
    protected function _nextLine()
    {
        ...
        if ($line === false) {
             ...
             */
            require_once 'Zend/Mail/Protocol/Exception.php';
            throw new  MailProtocolException ('cannot read - connection closed?');
        }
        ...
        foreach ($tokens as $token) {
            if (is_array($token)) {
                if (@fputs($this->_socket, $line . ' ' . $token[0] . "\r\n") === false) {
                     ...
                     */
                    require_once 'Zend/Mail/Protocol/Exception.php';
                    throw new  MailProtocolException ('cannot write - connection closed?');
                }
                if (!$this->_assumedNextLine('+ ')) {
                     ...
                     */
                    require_once 'Zend/Mail/Protocol/Exception.php';
                    throw new  MailProtocolException ('cannot send literal string');
                }
                $line = $token[1];
            } else {
                $line .= ' ' . $token;
...
//              $result[] = $this->escapeString($v);
    ...
    public function fetch($items, $from, $to = null)
    {
        ...
        if ($to === null && !is_array($from)) {
             ...
             */
            require_once 'Zend/Mail/Protocol/Exception.php';
            throw new  MailProtocolException ('the single id was not found in response');
        }
