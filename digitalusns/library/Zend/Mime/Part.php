...
class  Part  {
     ...
     *
     * @return stream
     * @throws  MimeException  if not a stream or unable to append filter
     */
    public function getEncodedStream()
    {
        if (!$this->_isStream) {
            throw new  MimeException ('Attempt to get a stream from a string part');
        }
        ...
        switch ($this->encoding) {
            case  Mime ::ENCODING_QUOTEDPRINTABLE:
                ...
                if (!is_resource($filter)) {
                    throw new  MimeException ('Failed to append quoted-printable filter');
                }
                break;
            case  Mime ::ENCODING_BASE64:
                ...
                if (!is_resource($filter)) {
                    throw new  MimeException ('Failed to append base64 filter');
                }
                break;
            default:
        }
