...
abstract class  DateObject  {
    ...
    protected function setUnixTimestamp($timestamp = null)
    {
        ...
        if (is_numeric($timestamp)) {
            ...
            throw new  DateException ('\'' . $timestamp . '\' is not a valid UNIX timestamp', $timestamp);
        }
    ...
    /**
     * Internal mktime function used by \Zend\Date.
    ...
    protected function mktime($hour, $minute, $second, $month, $day, $year, $gmt = false)
    {
        ...
        if (isset(self::$_cache)) {
            $id = strtr('\Zend\DateObject_mkTime_' . $this->_offset . '_' . $year.$month.$day.'_'.$hour.$minute.$second . '_'.(int)$gmt, '-','_');
    ...
    /**
     * Internal mktime function used by \Zend\Date \for handling 64bit timestamps.
     *
    ...
    protected function date($format, $timestamp = null, $gmt = false)
    {
        ...
        $jump = false;
        if (isset(self::$_cache)) {
            $idstamp = strtr('\Zend\DateObject_date_' . $this->_offset . '_'. $timestamp . '_'.(int)$gmt, '-','_');
    ...
    protected function getDateParts($timestamp = null, $fast = null)
    {
        ...
        if (isset(self::$_cache)) {
            $id = strtr('\Zend\DateObject_getDateParts_' . $timestamp.'_'.(int)$fast, '-','_');
            if ($result = self::$_cache->load($id)) {
                return unserialize($result);
            }
        }
    ...
    public function setTimezone($zone = null)
    {
        ...
        if (function_exists('timezone_open')) {
            if (!@timezone_open($zone)) {
                require_once 'Zend/Date/Exception.php';
                throw new  DateException ("timezone ($zone) is not a known timezone", $zone);
            }
        }
