...
use Zend\Dojo\Form\Element\DateTextBox as DateTextBox;
...
class  TimeTextBox  extends  DateTextBox 
{
    ...
    protected function _validateIso8601($format)
    {
        if (!preg_match('/^T\d{2}:\d{2}:\d{2}$/', $format)) {
            ...
            throw new  FormElementException (sprintf('Invalid format "%s" provided; must match T:00:00:00 format', $format));
        }
        return true;
    }
