...
use Zend\Dojo\Exception as DojoException;
...
class  Data  implements \ArrayAccess,\Iterator,\Countable 
{
    ...
    public function addItem($item, $id = null)
    {
        ...
        if ($this->hasItem($item['id'])) {
            ...
            throw new  DojoException ('Overwriting items using addItem() is not allowed');
        }
    ...
    public function addItems($items)
    {
        if (!is_array($items) && (!is_object($items) || !($items instanceof \Traversable))) {
            require_once 'Zend/Dojo/Exception.php';
            throw new  DojoException ('Only arrays and \Traversable objects may be added to ' . __CLASS__);
        }
    ...
    public function setIdentifier($identifier)
    {
        if (null === $identifier) {
            ...
            require_once 'Zend/Dojo/Exception.php';
            throw new  DojoException ('Invalid identifier; please use a string or integer');
        }
    ...
    public function fromJson($json)
    {
        if (!is_string($json)) {
            require_once 'Zend/Dojo/Exception.php';
            throw new  DojoException ('fromJson() expects JSON input');
        }
    ...
    public function toArray()
    {
        if (null === ($identifier = $this->getIdentifier())) {
            require_once 'Zend/Dojo/Exception.php';
            throw new  DojoException ('Serialization requires that an identifier be present in the object; first call setIdentifier()');
        }
    ...
    protected function _normalizeItem($item, $id)
    {
        if (null === ($identifier = $this->getIdentifier())) {
            require_once 'Zend/Dojo/Exception.php';
            throw new  DojoException ('You must set an identifier prior to adding items');
        }

        if (!is_object($item) && !is_array($item)) {
            require_once 'Zend/Dojo/Exception.php';
            throw new  DojoException ('Only arrays and objects may be attached');
        }
        ...
        if ((null === $id) && !array_key_exists($identifier, $item)) {
            require_once 'Zend/Dojo/Exception.php';
            throw new  DojoException ('Item must contain a column matching the currently set identifier');
        } elseif (null === $id) {
            $id = $item[$identifier];
        } else {
            $item[$identifier] = $id;
