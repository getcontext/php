...
class  Data 
{
    ...
    private static function _findRoute($locale, $path, $attribute, $value, &$temp)
    {
        ...
        if (empty(self::$_ldml[(string) $locale])) {
            ...
            if (!file_exists($filename)) {
                ...
                throw new  LocaleException ("Missing locale file '$filename' \for '$locale' locale.");
            }
    ...
    private static function _checkLocale($locale)
    {
        ...
        if (!($locale =  Locale ::isLocale($locale))) {
            require_once 'Zend/Locale/Exception.php';
            throw new  LocaleException ("Locale ($locale) is a unknown locale");
        }
        return $locale;
    }
    ...
    public static function getList($locale, $path, $value = false)
    {
        ...
        if (isset(self::$_cache)) {
            ...
            $val = urlencode($val);
            $id = strtr('\Zend\LocaleL_' . $locale . '_' . $path . '_' . $val, array('-' => '_', '%' => '_', '+' => '_'));
        ...
        switch(strtolower($path)) {
            ...
            default :
                require_once 'Zend/Locale/Exception.php';
                throw new  LocaleException ("Unknown list ($path) \for parsing locale data.");
                break;
        }
    ...
    public static function getContent($locale, $path, $value = false)
    {
        ...
        if (isset(self::$_cache)) {
            ...
            $val = urlencode($val);
            $id = strtr('\Zend\LocaleC_' . $locale . '_' . $path . '_' . $val, array('-' => '_', '%' => '_', '+' => '_'));
            if ($result = self::$_cache->load($id)) {
                return unserialize($result);
            }
        }
        ...
        switch(strtolower($path)) {
            ...
            default :
                require_once 'Zend/Locale/Exception.php';
                throw new  LocaleException ("Unknown detail ($path) \for parsing locale data.");
                break;
        }
    ...
    /**
     * Set a cache \for  Data 
     * 
     * @param  Core  $cache A cache frontend
     */
