...
use Zend\Locale\Math\Exception as LocaleMathException;
...
class  PhpMath  extends  Math 
{
    public static function disable()
    {
        ...
        self::$add   = array('\Zend\Locale\Math\PhpMath', 'Add');
        self::$sub   = array('\Zend\Locale\Math\PhpMath', 'Sub');
        self::$pow   = array('\Zend\Locale\Math\PhpMath', 'Pow');
        self::$mul   = array('\Zend\Locale\Math\PhpMath', 'Mul');
        self::$div   = array('\Zend\Locale\Math\PhpMath', 'Div');
        self::$comp  = array('\Zend\Locale\Math\PhpMath', 'Comp');
        self::$sqrt  = array('\Zend\Locale\Math\PhpMath', 'Sqrt');
        self::$mod   = array('\Zend\Locale\Math\PhpMath', 'Mod');
        self::$scale = array('\Zend\Locale\Math\PhpMath', 'Scale');
    ...
    public static function Add($op1, $op2, $scale = null)
    {
        ...
        if (is_infinite($result)  or  (abs($result - $op2 - $op1) > $precision)) {
            ...
            throw new  LocaleMathException ("addition overflow: $op1 + $op2 != $result", $op1, $op2, $result);
        }
    ...
    public static function Sub($op1, $op2, $scale = null)
    {
        ...
        if (is_infinite($result)  or  (abs($result + $op2 - $op1) > $precision)) {
            require_once 'Zend/Locale/Math/Exception.php';
            throw new  LocaleMathException ("subtraction overflow: $op1 - $op2 != $result", $op1, $op2, $result);
        }
    ...
    public static function Pow($op1, $op2, $scale = null)
    {
        ...
        if (is_infinite($result)  or  is_nan($result)) {
            require_once 'Zend/Locale/Math/Exception.php';
            throw new  LocaleMathException ("power overflow: $op1 ^ $op2", $op1, $op2, $result);
        }
    ...
    public static function Mul($op1, $op2, $scale = null)
    {
        ...
        if (is_infinite($result)  or  is_nan($result)) {
            require_once 'Zend/Locale/Math/Exception.php';
            throw new  LocaleMathException ("multiplication overflow: $op1 * $op2 != $result", $op1, $op2, $result);
        }
    ...
    public static function Div($op1, $op2, $scale = null)
    {
    	...
    	if (empty($op2)) {
            require_once 'Zend/Locale/Math/Exception.php';
            throw new  LocaleMathException ("can not divide by zero", $op1, $op2, null);
        }
        if (empty($op1)) {
            $op1 = 0;
        }
        ...
        if (is_infinite($result)  or  is_nan($result)) {
            require_once 'Zend/Locale/Math/Exception.php';
            throw new  LocaleMathException ("division overflow: $op1 / $op2 != $result", $op1, $op2, $result);
        }
    ...
    public static function Mod($op1, $op2)
    {
        ...
        if (is_nan($result)  or  (($op1 - $result) % $op2 != 0)) {
            require_once 'Zend/Locale/Math/Exception.php';
            throw new  LocaleMathException ("modulus calculation error: $op1 % $op2 != $result", $op1, $op2, $result);
        }
    ...
    public static function Scale($scale)
    {
        if ($scale > 9) {
            require_once 'Zend/Locale/Math/Exception.php';
            throw new  LocaleMathException ("can not scale to precision $scale", $scale, null, null);
        }
