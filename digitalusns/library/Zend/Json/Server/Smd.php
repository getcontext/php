...
use Zend\Json\Server\Smd\Service as Service;
...
class  Smd 
{
    ...
    public function setTransport($transport)
    {
        if (!in_array($transport, $this->_transportTypes)) {
            ...
            throw new  JsonServerException (sprintf('Invalid transport "%s" specified', $transport));
        }
    ...
    public function setEnvelope($envelopeType)
    {
        if (!in_array($envelopeType, $this->_envelopeTypes)) {
            require_once 'Zend/Json/Server/Exception.php';
            throw new  JsonServerException (sprintf('Invalid envelope type "%s"', $envelopeType));
        }
    ...
    public function setContentType($type)
    {
        if (!preg_match($this->_contentTypeRegex, $type)) {
            require_once 'Zend/Json/Server/Exception.php';
            throw new  JsonServerException (sprintf('Invalid content type "%s" specified', $type));
        }
    ...
    public function addService($service)
    {
        ...
        if ($service instanceof  Service ) {
            ...
            require_once 'Zend/Json/Server/Exception.php';
            throw new  JsonServerException ('Invalid service passed to addService()');
        }

        if (array_key_exists($name, $this->_services)) {
            require_once 'Zend/Json/Server/Exception.php';
            throw new  JsonServerException ('Attempt to register a service already registered detected');
        }
