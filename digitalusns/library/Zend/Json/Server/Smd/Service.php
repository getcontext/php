...
class  Service 
{
     ...
     * @return void
     * @throws  JsonServerException  if no name provided
     */
    public function __construct($spec)
    {
        ...
        if (null == $this->getName()) {
            ...
            throw new  JsonServerException ('SMD service description requires a name; none provided');
        }
    }
    ...
    public function setName($name)
    {
        ...
        if (!preg_match($this->_nameRegex, $name)) {
            require_once 'Zend/Json/Server/Exception.php';
            throw new  JsonServerException (sprintf('Invalid name "%s" provided \for service; must follow PHP method naming conventions', $name));
        }
        $this->_name = $name;
        return $this;
    }
    ...
    public function setTransport($transport)
    {
        if (!in_array($transport, $this->_transportTypes)) {
            require_once 'Zend/Json/Server/Exception.php';
            throw new  JsonServerException (sprintf('Invalid transport "%s"; please select one \of (%s)', $transport, implode(', ', $this->_transportTypes)));
        }
    ...
    public function setEnvelope($envelopeType)
    {
        if (!in_array($envelopeType, $this->_envelopeTypes)) {
            require_once 'Zend/Json/Server/Exception.php';
            throw new  JsonServerException (sprintf('Invalid envelope type "%s"; please specify one \of (%s)', $envelopeType, implode(', ', $this->_envelopeTypes)));
        }
    ...
    public function addParam($type, array $options = array(), $order = null)
    {
        if (is_string($type)) {
            ...
            require_once 'Zend/Json/Server/Exception.php';
            throw new  JsonServerException ('Invalid param type provided');
        }
    ...
    public function setReturn($type)
    {
        if (is_string($type)) {
            ...
            require_once 'Zend/Json/Server/Exception.php';
            throw new  JsonServerException ('Invalid param type provided ("' . gettype($type) .'")');
        }
        $this->_return = $type;
        return $this;
    }
    ...
    protected function _validateParamType($type, $isReturn = false)
    {
        if (!is_string($type)) {
            require_once 'Zend/Json/Server/Exception.php';
            throw new  JsonServerException ('Invalid param type provided ("' . $type .'")');
        }
