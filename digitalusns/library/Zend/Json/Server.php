...
use Zend\Json\Server\Response\Http as JsonServerResponseHttp;
use Zend\Json\Server\Request\Http as JsonServerRequestHttp;
use Zend\Json\Server\Exception as JsonServerException;
use Zend\Json\Server\Response as Response;
use Zend\Json\Server\Request as Request;
use Zend\Server\Reflection as Reflection;
use Zend\Json\Server\Error as Error;
...
class  Server  implements  ServerInterface 
{
    ...
    public function addFunction($function, $namespace = '')
    {
        if (!is_string($function) && (!is_array($function) || (2 > count($function)))) {
            ...
            throw new  JsonServerException ('Unable to attach function; invalid');
        }

        if (!is_callable($function)) {
            require_once 'Zend/Json/Server/Exception.php';
            throw new  JsonServerException ('Unable to attach function; does not exist');
        }

        require_once 'Zend/Server/Reflection.php';
        if (is_string($function)) {
            $method =  Reflection ::reflectFunction($function);
        } else {
            $class  = array_shift($function);
            $action = array_shift($function);
            $reflection =  Reflection ::reflectClass($class);
    ...
    public function handle($request = false)
    {
        if ((false !== $request) && (!$request instanceof  Request )) {
            require_once 'Zend/Json/Server/Exception.php';
            throw new  JsonServerException ('Invalid request type provided; cannot handle');
        } elseif ($request) {
    ...
    public function getRequest()
    {
        if (null === ($request = $this->_request)) {
            ...
            $this->setRequest(new  JsonServerRequestHttp ());
        }
    ...
    public function getResponse()
    {
        if (null === ($response = $this->_response)) {
            ...
            $this->setResponse(new  JsonServerResponseHttp ());
        }
     ...
     * 
     * @param   ServerReflectionFunction  $method 
     * @return void
     */
    protected function _addMethodServiceMap( ServerReflectionFunctionAbstract  $method)
    {
        $serviceInfo = array(
            'name'   => $method->getName(),
     ...
     * @return string|array
     */
    protected function _getParams( ServerReflectionFunctionAbstract  $method)
    {
        $params = array();
     ...
     * @return string|array
     */
    protected function _getReturnType( ServerReflectionFunctionAbstract  $method)
    {
        $return = array();
        foreach ($method->getPrototypes() as $prototype) {
     ...
     * Dispatch method
     * 
     * @param   ServerReflectionFunctionAbstract  $invocable 
     * @param  array $params 
     * @return mixed
     */
    protected function _dispatch( ServerReflectionFunctionAbstract  $invocable, array $params)
    {
        if ($invocable instanceof  ServerReflectionFunction ) {
