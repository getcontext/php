...
use Zend\Rest\Server\Exception as RestServerException;
use Zend\Server\Reflection as Reflection;
use Zend\Server\ServerInterface as ServerInterface;
use Zend\Server\ServerAbstract as ServerAbstract;
...
class  Server  extends  ServerAbstract  implements  ServerInterface 
{
     ...
     *
     * @param boolean $flag
     * @return boolean| Server  Returns  Server  when used to set the flag; returns boolean flag value otherwise.
     */
    ...
    /**
     * Implement  ServerInterface ::handle()
     *
     * @param array $request
     */
    public function handle($request = false)
    {
        ...
        if (isset($request['method'])) {
            ...
            if (isset($this->_functions[$this->_method])) {
                if ($this->_functions[$this->_method] instanceof  ServerReflectionFunction  || $this->_functions[$this->_method] instanceof  Method  && $this->_functions[$this->_method]->isPublic()) {
                    ...
                    if (count($calling_args) < count($func_args)) {
                        $result = $this->fault(new  RestServerException ('Invalid Method Call to ' . $this->_method . '. Requires ' . count($func_args) . ', ' . count($calling_args) . ' given.'), 400);
                    }

                    if (!$result && $this->_functions[$this->_method] instanceof  Method ) {
                        // Get class
    ...
    /**
     * Implement  ServerInterface ::setClass()
     *
    ...
    /**
     * Implement  ServerInterface ::fault()
     *
    ...
    public function fault($exception = null, $code = null)
    {
        ...
        if ($function instanceof  ServerReflectionFunctionAbstract ) {
            $method = $function->getName();
        } else {
            $method = $function;
        }
    ...
    /**
     * Implement  ServerInterface ::addFunction()
     *
    ...
    public function addFunction($function, $namespace = '')
    {
        ...
        foreach ($function as $func) {
            if (is_callable($func) && !in_array($func, self::$magic_methods)) {
                ...
                throw new  RestServerException ("Invalid Method Added to Service.");
            }
        }
    }

    /**
     * Implement  ServerInterface ::getFunctions()
     *
     * @return array An array \of  Reflection _Method's
     */
    ...
    /**
     * Implement  ServerInterface ::loadFunctions()
     *
     * @todo Implement
    ...
    /**
     * Implement  ServerInterface ::setPersistence()
     *
     * @todo Implement
     * @param int $mode
     */
     ...
     * @param  array $args
     * @return mixed
     * @throws  RestServerException  For invalid class name
     */
    protected function _callObjectMethod($class, array $args)
    {
        try {
            ...
            echo $e->getMessage();
            throw new  RestServerException ('Error instantiating class ' . $class . ' to invoke method ' . $this->_functions[$this->_method]->getName(), 500);
        }
