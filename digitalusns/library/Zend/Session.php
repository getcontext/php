...
use Zend\Session\Validator\ValidatorInterface as SessionValidatorInterface;
use Zend\Session\Exception as SessionException;
use Zend\Session\SessionNamespace as SessionNamespace;
use Zend\Session\SessionAbstract as SessionAbstract;
...
class  Session  extends  SessionAbstract 
{
     ...
     *
     * @param  SessionSaveHandlerInterface  $interface
     * @return void
     */
    ...
    public static function start($options = false)
    {
        ...
        if (!self::$_unitTestEnabled) {
            require_once 'Zend/Session/Exception.php';
            set_error_handler(array('\Zend\Session\Exception', 'handleSessionStartError'), E_ALL);
            session_start();
            restore_error_handler();
            if ( SessionException ::$sessionStartError !== null) {
            set_error_handler(array('\Zend\Session\Exception', 'handleSilentWriteClose'), E_ALL);
            session_write_close();
            restore_error_handler();
     ...
     *
     * @param bool $readonly - OPTIONAL remove write access (i.e. throw error if \Zend\Session's attempt writes)
     * @return void
     */
     ...
     * Deprecated: Use getIterator() in  SessionNamespace .
     *
     * @param string $namespace
     * @return array
     */
