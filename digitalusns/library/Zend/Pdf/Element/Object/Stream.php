...
use Zend\Pdf\Filter\Compression\Flate as Flate;
use Zend\Pdf\Filter\Compression\Lzw as Lzw;
use Zend\Pdf\Element\Dictionary as Dictionary;
use Zend\Pdf\Filter\AsciiHex as AsciiHex;
use Zend\Pdf\Element\Numeric as Numeric;
use Zend\Pdf\Element\Object as Object;
use Zend\Pdf\Filter\Ascii85 as Ascii85;
use Zend\Pdf\ElementFactory as ElementFactory;
use Zend\Pdf\Element\Stream as PdfElementStream;
...
class  Stream  extends  Object 
{
     ...
     * @throws  PdfException 
     */
    public function __construct($val, $objNum, $genNum,  ElementFactory  $factory, $dictionary = null)
    {
        parent::__construct(new  PdfElementStream ($val), $objNum, $genNum, $factory);

        if ($dictionary === null) {
            $this->_dictionary    = new  Dictionary ();
            $this->_dictionary->Length = new  Numeric (strlen( $val ));
    ...
    private function _storeOriginalDictionary()
    {
        ...
        if ($this->_dictionary->Filter === null) {
            // Do nothing.
        } else if ($this->_dictionary->Filter->getType() ==  Element ::TYPE_ARRAY) {
        ...
        } else if ($this->_dictionary->Filter->getType() !=  Element ::TYPE_NULL) {
        ...
        if ($this->_dictionary->FFilter === null) {
            // Do nothing.
        } else if ($this->_dictionary->FFilter->getType() ==  Element ::TYPE_ARRAY) {
    ...
    private function _decodeStream()
    {
        ...
        if (isset($this->_originalDictionary['F'])) {
            ...
            throw new  PdfException ('External filters are not supported now.');
        }
        ...
        foreach ($this->_originalDictionary['Filter'] as $id => $filterName ) {
            ...
            switch ($filterName) {
                case 'ASCIIHexDecode':
                    $valueRef =  AsciiHex ::decode($valueRef);
                    break;

                case 'ASCII85Decode':
                    $valueRef =  Ascii85 ::decode($valueRef);
                    break;

                case 'FlateDecode':
                    $valueRef =  Flate ::decode($valueRef,
                                                                          $this->_originalDictionary['DecodeParms'][$id]);
                    break;

                case 'LZWDecode':
                    $valueRef =  Lzw ::decode($valueRef,
                                                                        $this->_originalDictionary['DecodeParms'][$id]);
                    break;

                default:
                    throw new  PdfException ('Unknown stream filter: \'' . $filterName . '\'.');
            }
        }
    ...
    private function _encodeStream()
    {
        ...
        foreach ($filters as $id => $filterName ) {
            ...
            switch ($filterName) {
                case 'ASCIIHexDecode':
                    $valueRef =  AsciiHex ::encode($valueRef);
                    break;

                case 'ASCII85Decode':
                    $valueRef =  Ascii85 ::encode($valueRef);
                    break;

                case 'FlateDecode':
                    $valueRef =  Flate ::encode($valueRef,
                                                                          $this->_originalDictionary['DecodeParms'][$id]);
                    break;

                case 'LZWDecode':
                    $valueRef =  Lzw ::encode($valueRef,
                                                                        $this->_originalDictionary['DecodeParms'][$id]);
                    break;
    ...
    public function __get($property)
    {
        ...
        throw new  PdfException ('Unknown stream object property requested.');
    }
    ...
    public function __set($property, $value)
    {
        ...
        throw new  PdfException ('Unknown stream object property: \'' . $property . '\'.');
    }
