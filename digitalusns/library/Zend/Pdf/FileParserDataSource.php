...
/**
 * Abstract helper class \for {@link \Zend\Pdf\FileParser} that provides the
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
abstract class \Zend\Pdf\FileParserDataSource
{
  /**** Instance Variables ****/
  ...
  /* Primitive Methods */
    ...
    public function moveToOffset($offset)
    {
        ...
        if ($offset < 0) {
            throw new \Zend\Pdf\Exception('Attempt to move before start \of data source',
                                         \Zend\Pdf\Exception::MOVE_BEFORE_START_OF_FILE);
        }
        if ($offset >= $this->_size) {    // Offsets are zero-based.
            throw new \Zend\Pdf\Exception('Attempt to move beyond end \of data source',
