...
use Zend\Pdf\Trailer\Keeper as Keeper;
...
class  Parser 
{
    ...
    private function _loadXRefTable($offset)
    {
        ...
        $refTable = new  Table ();
        $context  = new  Context ($this->_stringParser, $refTable);
        ...
        if ($nextLexeme == 'xref') {
            ...
            while ( ($nextLexeme = $this->_stringParser->readLexeme()) != 'trailer' ) {
                if (!ctype_digit($nextLexeme)) {
                    throw new  PdfException (sprintf('PDF file syntax error. Offset - 0x%X. Cross-reference table subheader values must contain only digits.', $this->_stringParser->offset-strlen($nextLexeme)));
                }
                $objNum = (int)$nextLexeme;

                $refCount = $this->_stringParser->readLexeme();
                if (!ctype_digit($refCount)) {
                    throw new  PdfException (sprintf('PDF file syntax error. Offset - 0x%X. Cross-reference table subheader values must contain only digits.', $this->_stringParser->offset-strlen($refCount)));
                }
                ...
                while ($refCount > 0) {
                    ...
                    if (!ctype_digit($objectOffset)) {
                        throw new  PdfException (sprintf('PDF file cross-reference table syntax error. Offset - 0x%X. Offset must contain only digits.', $this->_stringParser->offset));
                    }
                    ...
                    if ( ! StringParser ::isWhiteSpace(ord( $this->_stringParser->data[$this->_stringParser->offset] )) ) {
                        throw new  PdfException (sprintf('PDF file cross-reference table syntax error. Offset - 0x%X. Value separator must be white space.', $this->_stringParser->offset));
                    }
            ...
            if (!$trailerDict instanceof  Dictionary ) {
                throw new  PdfException (sprintf('PDF file syntax error. Offset - 0x%X.  Dictionary expected after \'trailer\' keyword.', $trailerDictOffset));
            }
        } else {
            $xrefStream = $this->_stringParser->getObject($offset, $context);

            if (!$xrefStream instanceof  Stream ) {
                throw new  PdfException (sprintf('PDF file syntax error. Offset - 0x%X.  Cross-reference stream expected.', $offset));
            }

            $trailerDict = $xrefStream->dictionary;
            if ($trailerDict->Type->value != 'XRef') {
                throw new  PdfException (sprintf('PDF file syntax error. Offset - 0x%X.  Cross-reference stream object must have /Type property assigned to /XRef.', $offset));
            }
            if ($trailerDict->W === null  || $trailerDict->W->getType() !=  Element ::TYPE_ARRAY) {
                throw new  PdfException (sprintf('PDF file syntax error. Offset - 0x%X. Cross reference stream dictionary doesn\'t have W entry or it\'s not an array.', $offset));
            }
            ...
            if ($entryField2Size == 0 || $entryField3Size == 0) {
                throw new  PdfException (sprintf('PDF file syntax error. Offset - 0x%X. Wrong W dictionary entry. Only type field \of stream entries has default value and could be zero length.', $offset));
            }

            $xrefStreamData = &$xrefStream->value;

            if ($trailerDict->Index !== null) {
                if ($trailerDict->Index->getType() !=  Element ::TYPE_ARRAY) {
                    throw new  PdfException (sprintf('PDF file syntax error. Offset - 0x%X. Cross reference stream dictionary Index entry must be an array.', $offset));
                }
            ...
            // "$entries\n";
            throw new  PdfException ('Cross-reference streams are not supported yet.');
        }


        $trailerObj = new  Keeper ($trailerDict, $context);
        if ($trailerDict->Prev instanceof  Numeric  ||
            $trailerDict->Prev instanceof  PdfElementReference  ) {
         ...
         */
        $trailerObj->Prev = new  Numeric ($offset);
     ...
     */
    public function __construct($source,  PdfElementFactoryInterface  $factory, $load)
    {
        if ($load) {
            if (($pdfFile = @fopen($source, 'rb')) === false ) {
                throw new  PdfException ( "Can not open '$source' file \for reading." );
            }
        ...
        if (substr($pdfVersionComment, 0, 5) != '%PDF-') {
            throw new  PdfException ('File is not a PDF.');
        }
        ...
        if ($pdfVersion < 0.9 || $pdfVersion >= 1.61) {
             ...
             */
            throw new  PdfException (sprintf('Unsupported PDF version. \Zend\Pdf supports PDF 1.0-1.4. Current version - \'%f\'', $pdfVersion));
        }
        ...
        if ($this->_stringParser->offset === false ||
            ...
            throw new  PdfException ('Pdf file syntax error. End-\of-fle marker expected at the end \of file.');
        }
        ...
        if ($nextLexeme != 'startxref') {
            throw new  PdfException (sprintf('Pdf file syntax error. \'startxref\' keyword expected. Offset - 0x%X.', $this->_stringParser->offset-strlen($nextLexeme)));
        }

        $startXref = $this->_stringParser->readLexeme();
        if (!ctype_digit($startXref)) {
            throw new  PdfException (sprintf('Pdf file syntax error. Cross-reference table offset must contain only digits. Offset - 0x%X.', $this->_stringParser->offset-strlen($nextLexeme)));
        }
