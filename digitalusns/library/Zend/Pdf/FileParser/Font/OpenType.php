...
use Zend\Pdf\FileParser\Font as PdfFileParserFont;
use Zend\Pdf\Exception as PdfException;
use Zend\Pdf\Font as PdfFont;
use Zend\Pdf\Cmap as Cmap;



abstract class  OpenType  extends  PdfFileParserFont 
{
  /**** Instance Variables ****/
  ...
  /* Parser Methods */
    ...
    protected function _parseTableDirectory()
    {
        ...
        if (($tableCount < 7) || ($tableCount > 50)) {
            throw new  PdfException ('Table count not within expected range',
                                          PdfException ::BAD_TABLE_COUNT);
        }
        ...
        \for ($tableIndex = 0; $tableIndex < $tableCount; $tableIndex++) {
            ...
            if (($tableOffset < 0) || ($tableOffset > $fileSize)) {
                throw new  PdfException ("Table offset ($tableOffset) not within expected range",
                                              PdfException ::INDEX_OUT_OF_RANGE);
            }
            if (($tableLength < 0) || (($tableOffset + $tableLength) > $fileSize)) {
                throw new  PdfException ("Table length ($tableLength) not within expected range",
                                              PdfException ::INDEX_OUT_OF_RANGE);
            }
    ...
    protected function _parseHeadTable()
    {
        ...
        if ($magicNumber != 0x5f0f3cf5) {
            throw new  PdfException ('Wrong magic number. Expected: 0x5f0f3cf5; actual: '
                                       . sprintf('%x', $magicNumber),
                                          PdfException ::BAD_MAGIC_NUMBER);
        }
    ...
    protected function _parseNameTable()
    {
        ...
        if ($tableFormat != 0) {
            throw new  PdfException ("Unable to read format $tableFormat table",
                                          PdfException ::DONT_UNDERSTAND_TABLE_VERSION);
        }
    ...
    protected function _parseOs2Table()
    {
        if (! $this->numberHMetrics) {
            throw new  PdfException ("hhea table must be parsed prior to calling this method",
                                          PdfException ::PARSED_OUT_OF_ORDER);
        }
        ...
        try {
             ...
             */
            if ($exception->getCode() ==  PdfException ::REQUIRED_TABLE_NOT_FOUND) {
                $this->_debugLog('No OS/2 table found. Using default values');
                $this->fontWeight =  PdfFont ::WEIGHT_NORMAL;
                $this->fontWidth =  PdfFont ::WIDTH_NORMAL;
        ...
        if (($tableVersion < 0) || ($tableVersion > 3)) {
            throw new  PdfException ("Unable to read version $tableVersion table",
                                          PdfException ::DONT_UNDERSTAND_TABLE_VERSION);
        }
         ...
         * and WIDTH_ constants defined in  PdfFont .
         */
    ...
    protected function _parseHmtxTable()
    {
        ...
        if ($this->metricDataFormat != 0) {
            throw new  PdfException ("Unable to read format $this->metricDataFormat table.",
                                          PdfException ::DONT_UNDERSTAND_TABLE_VERSION);
        }
    ...
    protected function _parseCmapTable()
    {
        ...
        if ($cmapType == -1) {
            throw new  PdfException ('Unable to find usable cmap table',
                                          PdfException ::CANT_FIND_GOOD_CMAP);
        }

        /* Now extract the subtable data and create a  PdfFont Cmap object.
         */
        $this->_debugLog('Using cmap type %d; offset: 0x%x; length: %d',
                         $cmapType, $cmapOffset, $cmapLength);
        $this->moveToOffset($cmapOffset);
        $cmapData = $this->readBytes($cmapLength);
        $this->cmap =  Cmap ::cmapWithTypeData($cmapType, $cmapData);
    }
    ...
    protected function _readScalerType()
    {
        ...
        switch ($this->_scalerType) {
            ...
            case 0x74797031:    // 'typ1'
                throw new  PdfException ('Unsupported font type: PostScript in sfnt wrapper',
                                              PdfException ::WRONG_FONT_TYPE);

            default:
                throw new  PdfException ('Not an OpenType font file',
                                              PdfException ::WRONG_FONT_TYPE);
        }
        return $this->_scalerType;
    }
    ...
    protected function _jumpToTable($tableName)
    {
        if (empty($this->_tableDirectory[$tableName])) {    // do not allow NULL or zero
            throw new  PdfException ("Required table '$tableName' not found!",
                                          PdfException ::REQUIRED_TABLE_NOT_FOUND);
        }
