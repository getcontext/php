...
use Zend\Pdf\FileParser\Image as PdfFileParserImage;
...
class  Png  extends  PdfFileParserImage 
{
     protected $_isPNG;
     protected $_width;
     protected $_height;
  ...
  /* Semi-Concrete Class Implementation */
    ...
    protected function _parseChunks() {
         ...
         while($size - $this->getOffset() >= 8) {
              ...
              if($chunkLength < 0 || ($chunkLength + $this->getOffset() + 4) > $size) {
                   throw new  PdfException ("PNG Corrupt: Invalid Chunk Size In File.");
              }
         ...
         if(empty($this->_imageData)) {
              throw new  PdfException  ( "This PNG is corrupt. All png must contain IDAT chunks." );
         }
    }
    ...
    protected function _parseIHDRChunk() {
         ...
         if(!$this->readBytes(4) == 'IHDR') {
              throw new  PdfException ( "This PNG is corrupt. The first chunk in a PNG file must be IHDR." );
         }
         ...
         if($this->_interlacing !=  PdfImage ::PNG_INTERLACING_DISABLED) {
    ...
    protected function _parseTRNSChunk($chunkOffset, $chunkLength) {
         ...
         switch($this->_color) {
             ...
             case  PdfImage ::PNG_CHANNEL_INDEXED:
             ...
             case  PdfImage ::PNG_CHANNEL_GRAY_ALPHA:
                  //Fall through to the next case
             case  PdfImage ::PING_CHANNEL_RGB_ALPHA:
