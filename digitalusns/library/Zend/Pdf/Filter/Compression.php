...
abstract class  Compression  implements  PdfFilterInterface 
{
    /**
    ...
    private static function _getPredictorValue(&$params)
    {
        if (isset($params['Predictor'])) {
            ...
            if ($predictor != 1   &&  $predictor != 2   &&
                ...
                throw new  PdfException ('Invalid value \of \'Predictor\' decode param - ' . $predictor . '.' );
            }
            return $predictor;
        } else {
            return 1;
    ...
    private static function _getColorsValue(&$params)
    {
        if (isset($params['Colors'])) {
            ...
            if ($colors != 1  &&  $colors != 2  &&  $colors != 3  &&  $colors != 4) {
                throw new  PdfException ('Invalid value \of \'Color\' decode param - ' . $colors . '.' );
            }
            return $colors;
        } else {
            return 1;
    ...
    private static function _getBitsPerComponentValue(&$params)
    {
        if (isset($params['BitsPerComponent'])) {
            ...
            if ($bitsPerComponent != 1  &&  $bitsPerComponent != 2  &&
                ...
                $bitsPerComponent != 16 ) {
                throw new  PdfException ('Invalid value \of \'BitsPerComponent\' decode param - ' . $bitsPerComponent . '.' );
            }
    ...
    protected static function _applyEncodeParams($data, $params) {
        ...
        if ($predictor == 10 ||  /** None \of prediction */
            ...
            if($bitsPerComponent == 16) {
                throw new  PdfException ("PNG Prediction with bit depth greater than 8 not yet supported.");
            }
            ...
            if (!is_integer($rows)) {
                throw new  PdfException ('Wrong data length.');
            }
        ...
        throw new  PdfException ('Unknown prediction algorithm - ' . $predictor . '.' );
    }
        ...
        if ($predictor == 10 ||  /** None \of prediction */
            ...
            \for ($count = 0; $count < $rows; $count++) {
                ...
                switch (ord($data[$offset++])) {
                    ...
                    default:
                        throw new  PdfException ('Unknown prediction tag.');
                }
            }
            return $output;
        }
