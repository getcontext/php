 ...
 *
 * File access is managed via a {@link  FileParserDataSource } object.
...
abstract class  FileParser 
{
  ...
  /* Object Lifecycle */
    ...
    public function __construct( FileParserDataSource  $dataSource)
    {
        if ($dataSource->getSize() == 0) {
            throw new  PdfException ('The data source has not been properly initialized',
                                          PdfException ::BAD_DATA_SOURCE);
        }
  ...
  /* Parser Methods */
     ...
     *   Use the BYTE_ORDER_ constants defined in {@link  FileParser }.
     *   If omitted, uses big-endian.
     * @return integer
     * @throws  PdfException 
     */
    public function readInt($size, $byteOrder =  FileParser ::BYTE_ORDER_BIG_ENDIAN)
    {
        if (($size < 1) || ($size > 4)) {
            throw new  PdfException ("Invalid signed integer size: $size",
                                          PdfException ::INVALID_INTEGER_SIZE);
        }
        ...
        if ($byteOrder ==  FileParser ::BYTE_ORDER_BIG_ENDIAN) {
            ...
            throw new  PdfException ("Invalid byte order: $byteOrder",
                                          PdfException ::INVALID_BYTE_ORDER);
        }
        return $number;
    }
     ...
     * @throws  PdfException 
     */
    public function readUInt($size, $byteOrder =  FileParser ::BYTE_ORDER_BIG_ENDIAN)
    {
        if (($size < 1) || ($size > 4)) {
            throw new  PdfException ("Invalid unsigned integer size: $size",
                                          PdfException ::INVALID_INTEGER_SIZE);
        }
        $bytes = $this->_dataSource->readBytes($size);
    ...
    public function readFixed($mantissaBits, $fractionBits,
                              $byteOrder =  FileParser ::BYTE_ORDER_BIG_ENDIAN)
    {
        $bitsToRead = $mantissaBits + $fractionBits;
        if (($bitsToRead % 8) !== 0) {
            throw new  PdfException ('Fixed-point numbers are whole bytes',
                                          PdfException ::BAD_FIXED_POINT_SIZE);
        }
