 ...
 *
 * This table type is preferred over the {@link  Cmap _SegmentToDelta}
...
class  TrimmedTable  extends  Cmap 
{
  ...
  /* Object Lifecycle */
    ...
    public function __construct($cmapData)
    {
        ...
        if ($actualLength < 9) {
            throw new  PdfException ('Insufficient table data',
                                          PdfException ::CMAP_TABLE_DATA_TOO_SMALL);
        }
        ...
        if ($type !=  Cmap ::TYPE_TRIMMED_TABLE) {
            throw new  PdfException ('Wrong cmap table type',
                                          PdfException ::CMAP_WRONG_TABLE_TYPE);
        }

        $length = $this->_extractUInt2($cmapData, 2);
        if ($length != $actualLength) {
            throw new  PdfException ("Table length ($length) does not match actual length ($actualLength)",
                                          PdfException ::CMAP_WRONG_TABLE_LENGTH);
        }
        ...
        if ($entryCount != $expectedCount) {
            throw new  PdfException ("Entry count is wrong; expected: $expectedCount; actual: $entryCount",
                                          PdfException ::CMAP_WRONG_ENTRY_COUNT);
        }
         ...
         */
        if ($offset != $length) {
            throw new  PdfException ("Ending offset ($offset) does not match length ($length)",
                                          PdfException ::CMAP_FINAL_OFFSET_NOT_LENGTH);
        }
    }
