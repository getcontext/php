 ...
 * {@link  PdfFont ::fontWithName} and {@link  PdfFont ::fontWithPath}.
 *
 * @package    \Zend\Pdf
 * @subpackage Fonts
 * @copyright  Copyright (c) 2005-2008 Zend Technologies USA Inc. (http://www.zend.com)
...
abstract class  CidFont  extends  PdfResourceFont 
{
    /**
     ...
     * Object constructor
     *
     * @param  OpenType  $fontParser Font parser object
    ...
    public function __construct( OpenType  $fontParser)
    {
        ...
        $baseFont = $this->getFontName( PdfFont ::NAME_POSTSCRIPT, 'en', 'UTF-8');
        ...
        $this->_resource->DW = new  Numeric ($this->toEmSpace($defaultWidth));
            ...
            foreach ($widthsSequence as $width) {
            	if ($lastWidth != $width) {
            		...
            		if ($widthsInSequence != 0) {
                        ...
                        $pdfCharsWidths[] = new  Numeric ($startCode);                         // First character code
            			$pdfCharsWidths[] = new  Numeric ($startCode + $widthsInSequence - 1); // Last character code
                        $pdfCharsWidths[] = new  Numeric ($this->toEmSpace($lastWidth));       // Width 
            		...
            		if (count($pdfWidths) != 0) {
            			...
            			if (count($pdfWidths) != 0) {
                            ...
                            $pdfCharsWidths[] = new  Numeric ($startCode); // First character code
            				$pdfCharsWidths[] = new  PdfElementArray ($pdfWidths);   // Widths array
         ...
         */
        $widthsArrayElement = new  PdfElementArray ($pdfCharsWidths);
        ...
        $cidSystemInfo->Registry   = new  PdfElementString ('Adobe');
        $cidSystemInfo->Ordering   = new  PdfElementString ('UCS');
        $cidSystemInfo->Supplement = new  Numeric (0);
    ...
    public function glyphNumbersForCharacters($characterCodes)
    {
         ...
         * Throw an exception.
         */
        throw new  PdfException ('CIDFont PDF objects could not be used as the operand \of the text drawing operators');
    }
