...
use Zend\Pdf\FileParser\Font\OpenType\TrueType as TrueType;
use Zend\Pdf\FileParser\Font\OpenType\Type1 as Type1;
...
use Zend\Pdf\Resource\Font as PdfResourceFont;
...
class  FontDescriptor 
{
	...
	{
		throw new  PdfException ('\Zend\Pdf\Resource\Font\FontDescriptor is not intended to be instantiated');
	} 
     ...
     * the EMBED_ constants defined in {@link  PdfFont } \for the list \of
     ...
     * 
     * 
     * @param  PdfResourceFont  $font Font
     * @param  OpenType  $fontParser Font parser object containing parsed TrueType file.
     * @param integer $embeddingOptions Options \for font embedding.
     * @return  Dictionary 
     * @throws  PdfException 
     */
    static public function factory( PdfResourceFont  $font,  OpenType  $fontParser, $embeddingOptions)
    {
         ...
         */
        $fontDescriptor = new  Dictionary ();

        $fontDescriptor->Type     = new  Name ('FontDescriptor');
        $fontDescriptor->FontName = new  Name ($font->getResource()->BaseFont->value);
        ...
        $fontDescriptor->Flags = new  Numeric ($flags);

        $fontBBox = array(new  Numeric ($font->toEmSpace($fontParser->xMin)),
                          new  Numeric ($font->toEmSpace($fontParser->yMin)),
                          new  Numeric ($font->toEmSpace($fontParser->xMax)),
                          new  Numeric ($font->toEmSpace($fontParser->yMax)));
        $fontDescriptor->FontBBox     = new  PdfElementArray ($fontBBox);

        $fontDescriptor->ItalicAngle  = new  Numeric ($fontParser->italicAngle);

        $fontDescriptor->Ascent       = new  Numeric ($font->toEmSpace($fontParser->ascent));
        $fontDescriptor->Descent      = new  Numeric ($font->toEmSpace($fontParser->descent));

        $fontDescriptor->CapHeight    = new  Numeric ($fontParser->capitalHeight);
        /**
         ...
         */
        $fontDescriptor->StemV        = new  Numeric (0);
        
        $fontDescriptor->MissingWidth = new  Numeric ($fontParser->glyphWidths[0]);
         ...
         */
        if (!($embeddingOptions &  PdfFont ::EMBED_DONT_EMBED)) {
            ...
            if (! $fontParser->isEmbeddable) {
                 ...
                 */
                if (!($embeddingOptions &  PdfFont ::EMBED_SUPPRESS_EMBED_EXCEPTION)) {
                    $message = 'This font cannot be embedded in the PDF document. If you would like to use '
                             . 'it anyway, you must pass  PdfFont ::EMBED_SUPPRESS_EMBED_EXCEPTION '
                             . 'in the $options parameter \of the font constructor.';
                    throw new  PdfException ($message,  PdfException ::FONT_CANT_BE_EMBEDDED);
                }
                ...
                $fontFileObject->dictionary->Length1 = new  Numeric (strlen($fontFile));
                if (!($embeddingOptions &  PdfFont ::EMBED_DONT_COMPRESS)) {
                    /* Compress the font file using Flate. This generally cuts file
                     * sizes by about half!
                     */
                    $fontFileObject->dictionary->Filter = new  Name ('FlateDecode');
                }
                if ($fontParser instanceof  Type1  /* not implemented now */) {
                    $fontDescriptor->FontFile  = $fontFileObject;
                } else if ($fontParser instanceof  TrueType ) {
