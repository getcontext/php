...
class  Movable  extends  Container  {
     ...
     * @param string $value
     */
    public function __construct( Manager  $memoryManager, $id, $value)
    {
        $this->_memManager = $memoryManager;
        $this->_id    = $id;
        $this->_state = self::LOADED;
        $this->_value = new  Value ($value, $this);
    }
    ...
    {
        if ($property != 'value') {
            throw new  MemoryException ('Unknown property: \Zend\Memory_container::$' . $property);
        }
