...
use Zend\Controller\Response\HttpTestCase as HttpTestCase;
...
class  Redirect  extends  Constraint 
{
    /**#@+
    ...
    public function evaluate($other, $assertType = null)
    {
        if (!$other instanceof  ControllerResponseAbstract ) {
            ...
            throw new  TestPHPUnitConstraintException ('Redirect constraint assertions require a response object');
        }
        ...
        switch ($assertType) {
            case self::ASSERT_REDIRECT_TO:
                if (3 > $argc) {
                    require_once 'Zend/Test/PHPUnit/Constraint/Exception.php';
                    throw new  TestPHPUnitConstraintException ('No redirect URL provided against which to match');
                }
            ...
            case self::ASSERT_REDIRECT_REGEX:
                if (3 > $argc) {
                    require_once 'Zend/Test/PHPUnit/Constraint/Exception.php';
                    throw new  TestPHPUnitConstraintException ('No pattern provided against which to match redirect');
                }
                $this->_match = $match = $argv[2];
                return ($this->_negate)
