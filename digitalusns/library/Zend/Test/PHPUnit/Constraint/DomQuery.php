...
/**
 *  Query -based PHPUnit Constraint
 * 
 * @uses        Constraint 
 * @package    Zend_Test
 * @subpackage PHPUnit
...
use Zend\Dom\Query\Result as Result;
use Zend\Dom\Query as Query;




class  DomQuery  extends  Constraint 
{
    /**#@+
    ...
    public function evaluate($other, $assertType = null)
    {
        ...
        switch ($assertType) {
            case self::ASSERT_CONTENT_CONTAINS:
                if (3 > $argc) {
                    require_once 'Zend/Test/PHPUnit/Constraint/Exception.php';
                    throw new  TestPHPUnitConstraintException ('No content provided against which to match');
                }
            ...
            case self::ASSERT_CONTENT_REGEX:
                if (3 > $argc) {
                    require_once 'Zend/Test/PHPUnit/Constraint/Exception.php';
                    throw new  TestPHPUnitConstraintException ('No pattern provided against which to match');
                }
                $this->_content = $content = $argv[2];
                return ($this->_negate)
            ...
            case self::ASSERT_CONTENT_COUNT_MAX:
                if (3 > $argc) {
                    require_once 'Zend/Test/PHPUnit/Constraint/Exception.php';
                    throw new  TestPHPUnitConstraintException ('No count provided against which to compare');
                }
                $this->_content = $content = $argv[2];
