...
use Zend\Test\PHPUnit\Constraint\Exception as TestPHPUnitConstraintException;
...
class  ResponseHeader  extends  Constraint 
{
     ...
     * 
     * @param   ControllerResponseAbstract  $other String to examine
     * @param  null|string Assertion type
     * @return bool
     */
    public function evaluate($other, $assertType = null)
    {
        if (!$other instanceof  ControllerResponseAbstract ) {
            require_once 'Zend/Test/PHPUnit/Constraint/Exception.php';
            throw new  TestPHPUnitConstraintException ('Header constraint assertions require a response object');
        }
        ...
        if (!in_array($assertType, $this->_assertTypes)) {
            require_once 'Zend/Test/PHPUnit/Constraint/Exception.php';
            throw new  TestPHPUnitConstraintException (sprintf('Invalid assertion type "%s" provided to %s constraint', $assertType, __CLASS__));
        }
        ...
        switch ($assertType) {
            case self::ASSERT_RESPONSE_CODE:
                if (3 > $argc) {
                    require_once 'Zend/Test/PHPUnit/Constraint/Exception.php';
                    throw new  TestPHPUnitConstraintException ('No response code provided against which to match');
                }
            ...
            case self::ASSERT_HEADER:
                if (3 > $argc) {
                    require_once 'Zend/Test/PHPUnit/Constraint/Exception.php';
                    throw new  TestPHPUnitConstraintException ('No header provided against which to match');
                }
            ...
            case self::ASSERT_HEADER_CONTAINS:
                if (4 > $argc) {
                    require_once 'Zend/Test/PHPUnit/Constraint/Exception.php';
                    throw new  TestPHPUnitConstraintException ('Both a header name and content to match are required \for ' . __FUNCTION__);
                }
                $this->_header = $header = $argv[2];
            ...
            default:
                require_once 'Zend/Test/PHPUnit/Constraint/Exception.php';
                throw new  TestPHPUnitConstraintException ('Invalid assertion type ' . __FUNCTION__);
        }
    }
    ...
    public function fail($other, $description, $not = false)
    {
        ...
        throw new  TestPHPUnitConstraintException ($failure);
    }
     ...
     * @return bool
     */
    protected function _code( ControllerResponseAbstract  $response, $code)
    {
     ...
     * @return bool
     */
    protected function _notCode( ControllerResponseAbstract  $response, $code)
    {
        $test = $this->_getCode($response);
        return ($test != $code);
    }
     ...
     * @return int
     */
    protected function _getCode( ControllerResponseAbstract  $response)
    {
     ...
     * @return bool
     */
    protected function _header( ControllerResponseAbstract  $response, $header)
    {
     ...
     * @return bool
     */
    protected function _notHeader( ControllerResponseAbstract  $response, $header)
    {
        return (null === $this->_getHeader($response, $header));
    }
     ...
     * @return string|null
     */
    protected function _getHeader( ControllerResponseAbstract  $response, $header)
    {
     ...
     * @return bool
     */
    protected function _headerContains( ControllerResponseAbstract  $response, $header, $match)
    {
     ...
     * @return bool
     */
    protected function _notHeaderContains( ControllerResponseAbstract  $response, $header, $match)
    {
        if (null === ($fullHeader = $this->_getHeader($response, $header))) {
            return true;
        }
     ...
     * @return bool
     */
    protected function _headerRegex( ControllerResponseAbstract  $response, $header, $pattern)
    {
        if (null === ($fullHeader = $this->_getHeader($response, $header))) {
            return false;
        }
     ...
     * @return bool
     */
    protected function _notHeaderRegex( ControllerResponseAbstract  $response, $header, $pattern)
    {
        if (null === ($fullHeader = $this->_getHeader($response, $header))) {
            return true;
        }
