...
class  Callback  extends \\Zend\Form_Decorator_Abstract
{
    /**
     * Callback
     * @var string|array
    ...
    public function setCallback($callback)
    {
        if (!is_string($callback) && !is_array($callback)) {
            ...
            throw new  FormException ('Invalid callback provided to callback decorator');
        }

        if (is_array($callback)) {
            if (2 !== count($callback)) {
                require_once 'Zend/Form/Exception.php';
                throw new  FormException ('Invalid method callback provided to callback decorator');
            }
        }
