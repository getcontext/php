...
class  Input 
{
    ...
    /**
     * @var  FilterInterface  The filter object that is run on values
     ...
     * @return  Input 
     * @throws  FilterException  on invalid type
     */
    public function setPluginLoader( LoaderPluginLoaderInterface  $loader, $type)
    {
        ...
        switch ($type) {
            ...
            default:
                ...
                throw new  FilterException (sprintf('Invalid type "%s" provided to setPluginLoader()', $type));
        }
    ...
    public function getPluginLoader($type)
    {
        ...
        if (!isset($this->_loaders[$type])) {
            switch ($type) {
                case self::FILTER:
                    $prefixSegment = '\Zend\Filter_';
                    $pathSegment   = 'Zend/Filter/';
                    break;
                case self::VALIDATE:
                    $prefixSegment = '\Zend\Validate_';
                    $pathSegment   = 'Zend/Validate/';
                    break;
                default:
                    require_once 'Zend/Filter/Exception.php';
                    throw new  FilterException (sprintf('Invalid type "%s" provided to getPluginLoader()', $type));
            }
    ...
    public function setDefaultEscapeFilter($escapeFilter)
    {
        ...
        if (!$escapeFilter instanceof  FilterInterface ) {
            require_once 'Zend/Filter/Exception.php';
            throw new  FilterException ('Escape filter specified does not implement  Filter _Interface');
        }
     ...
     * @param array $options
     * @return  Input 
     * @throws  FilterException  if an unknown option is given
     */
    ...
    protected function _filter()
    {
        foreach ($this->_filterRules as $ruleName => &$filterRule) {
             ...
             * Don't typecast to (array) because it might be a  Filter  object
             */
    ...
    protected function _validate()
    {
        ...
        foreach ($this->_validatorRules as $ruleName => &$validatorRule) {
             ...
             * Don't typecast to (array) because it might be a  Validate  object
             */
            ...
            if (!isset($validatorRule[self::VALIDATOR_CHAIN])) {
                $validatorRule[self::VALIDATOR_CHAIN] = new  Validate ();
                $i = 0;
     ...
     * @param string $type
     * @param mixed $classBaseName
     * @return  FilterInterface | Validate _Interface
     * @throws  FilterException 
     */
    protected function _getFilterOrValidator($type, $classBaseName)
    {
        ...
        if (!$class->implementsInterface($interfaceName)) {
            require_once 'Zend/Filter/Exception.php';
            throw new  FilterException ("Class based on basename '$classBaseName' must implement the '$interfaceName' interface");
        }
