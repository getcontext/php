...
{
    /**
     * @var  LoaderPluginLoaderInterface 
     */
    ...
    {
        if (!$this->_pluginLoader instanceof  LoaderPluginLoaderInterface ) {
            $this->_pluginLoader = new  PluginLoader (array('\Zend\Filter_' => 'Zend/Filter/'), __CLASS__);
        }
     ...
     * Set PluginLoader
     *
     * @param  LoaderPluginLoaderInterface  $pluginLoader
     * @return  Inflector 
     */
    public function setPluginLoader( LoaderPluginLoaderInterface  $pluginLoader)
    {
    ...
    /**
     * Use  Config  object to set object state
     * 
     * @param   Config  $config 
     * @return  Inflector 
     */
    ...
    public function filter($source)
    {
    	...
    	if ($this->_throwTargetExceptionsOn && (preg_match('#(?='.$pregQuotedTargetReplacementIdentifier.'[A-Za-z]{1})#', $inflectedTarget) == true)) {
    	    ...
    	    throw new  FilterException ('A replacement identifier ' . $this->_targetReplacementIdentifier . ' was found inside the inflected target, perhaps a rule was not satisfied with a target source?  Unsatisfied inflected target: ' . $inflectedTarget);
    	}
    ...
    protected function _getRule($rule)
    {
        ...
        if (!$ruleObject instanceof  FilterInterface ) {
            require_once 'Zend/Filter/Exception.php';
            throw new  FilterException ('No class named ' . $rule . ' implementing  FilterInterface  could be found');
        }
