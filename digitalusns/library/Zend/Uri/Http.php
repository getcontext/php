...
use Zend\Validate\Hostname as Hostname;
...
class  Http  extends  Uri 
{
     ...
     * @throws  UriException  When the URI is not valid
     */
    protected function __construct($scheme, $schemeSpecific = '')
    {
        ...
        if ($this->valid() === false) {
            ...
            throw new  UriException ('Invalid URI supplied');
        }
    }
     ...
     * @throws  UriException         When the given $uri is invalid
     * @return  Http 
     */
     ...
     * @throws  UriException  When scheme-specific decoposition fails
     * @throws  UriException  When authority decomposition fails
     * @return void
     */
    protected function _parseUri($schemeSpecific)
    {
        ...
        if ($status === false) {
            require_once 'Zend/Uri/Exception.php';
            throw new  UriException ('Internal error: scheme-specific decomposition failed');
        }
        ...
        if ($status === false) {
            require_once 'Zend/Uri/Exception.php';
            throw new  UriException ('Internal error: authority decomposition failed');
        }
     ...
     *
     * @throws  UriException  When one or more parts \of the URI are invalid
     * @return string
     */
    public function getUri()
    {
        if ($this->valid() === false) {
            require_once 'Zend/Uri/Exception.php';
            throw new  UriException ('One or more parts \of the URI are invalid');
        }
     ...
     * @throws  UriException  When username validation fails
     * @return boolean
    ...
    public function validateUsername($username = null)
    {
        ...
        if ($status === false) {
            require_once 'Zend/Uri/Exception.php';
            throw new  UriException ('Internal error: username validation failed');
        }
     ...
     *
     * @param  string $username The HTTP username
     * @throws  UriException  When $username is not a valid HTTP username
     * @return string
     */
    public function setUsername($username)
    {
        if ($this->validateUsername($username) === false) {
            require_once 'Zend/Uri/Exception.php';
            throw new  UriException ("Username \"$username\" is not a valid HTTP username");
        }
     ...
     * @throws  UriException  When password validation fails
     * @return boolean
     * @link   http://www.faqs.org/rfcs/rfc2396.html
     */
    public function validatePassword($password = null)
    {
        ...
        if ($status === false) {
            require_once 'Zend/Uri/Exception.php';
            throw new  UriException ('Internal error: password validation failed.');
        }
     ...
     *
     * @param  string $password The HTTP password
     * @throws  UriException  When $password is not a valid HTTP password
     * @return string
     */
    public function setPassword($password)
    {
        if ($this->validatePassword($password) === false) {
            require_once 'Zend/Uri/Exception.php';
            throw new  UriException ("Password \"$password\" is not a valid HTTP password.");
        }
    ...
    public function validateHost($host = null)
    {
        ...
        // Check the host against the allowed values; delegated to \Zend\Filter.
        $validate = new  Hostname (\Zend\Validate\Hostname::ALLOW_ALL);
     ...
     *
     * @param  string $host The HTTP host
     * @throws  UriException  When $host is nota valid HTTP host
     * @return string
     */
    public function setHost($host)
    {
        if ($this->validateHost($host) === false) {
            require_once 'Zend/Uri/Exception.php';
            throw new  UriException ("Host \"$host\" is not a valid HTTP host");
        }
     ...
     *
     * @param  string $port The HTTP port
     * @throws  UriException  When $port is not a valid HTTP port
     * @return string
     */
    public function setPort($port)
    {
        if ($this->validatePort($port) === false) {
            require_once 'Zend/Uri/Exception.php';
            throw new  UriException ("Port \"$port\" is not a valid HTTP port.");
        }
     ...
     * @throws  UriException  When path validation fails
     * @return boolean
     */
    public function validatePath($path = null)
    {
        ...
        if ($status === false) {
            require_once 'Zend/Uri/Exception.php';
            throw new  UriException ('Internal error: path validation failed');
        }
     ...
     *
     * @param  string $path The HTTP path
     * @throws  UriException  When $path is not a valid HTTP path
     * @return string
     */
    public function setPath($path)
    {
        if ($this->validatePath($path) === false) {
            require_once 'Zend/Uri/Exception.php';
            throw new  UriException ("Path \"$path\" is not a valid HTTP path");
        }
     ...
     * @throws  UriException  When query validation fails
     * @return boolean
     * @link   http://www.faqs.org/rfcs/rfc2396.html
     */
    public function validateQuery($query = null)
    {
        ...
        if ($status === false) {
            require_once 'Zend/Uri/Exception.php';
            throw new  UriException ('Internal error: query validation failed');
        }
     ...
     * @throws  UriException  When $query is not a valid query string
    ...
    public function setQuery($query)
    {
        ...
        if ($this->validateQuery($query) === false) {
            require_once 'Zend/Uri/Exception.php';
            throw new  UriException ("'$query' is not a valid query string");
        }
     ...
     * @throws  UriException  When fragment validation fails
     * @return boolean
     * @link   http://www.faqs.org/rfcs/rfc2396.html
     */
    public function validateFragment($fragment = null)
    {
        ...
        if ($status === false) {
            require_once 'Zend/Uri/Exception.php';
            throw new  UriException ('Internal error: fragment validation failed');
        }
     ...
     * @throws  UriException  When $fragment is not a valid HTTP fragment
     * @return string
     */
    public function setFragment($fragment)
    {
        if ($this->validateFragment($fragment) === false) {
            require_once 'Zend/Uri/Exception.php';
            throw new  UriException ("Fragment \"$fragment\" is not a valid HTTP fragment");
        }
