...
/**
 * Specifically grab the  Method  \for manually setting up
 ...
 *  Fault ::attachFaultException('My_Exception');
 *  Fault ::attachObserver('My_Fault_Observer');
 *
 * // Get or build dispatch table:
 * if (! Cache ::get($filename, $server)) {
...
use Zend\Server\Reflection\ReflectionFunction as ServerReflectionFunction;
use Zend\Server\Reflection\Method as Method;
...
use Zend\XmlRpc\Server\Fault as Fault;
use Zend\XmlRpc\Server\Cache as Cache;
use Zend\XmlRpc\Request\Http as Http;
use Zend\Server\Reflection as Reflection;
use Zend\Server\ServerInterface as ServerInterface;
use Zend\Server\Exception as ServerException;
use Zend\XmlRpc\Request as Request;
use Zend\XmlRpc\Value as Value;
...
class  Server  implements  ServerInterface 
{
    ...
    public function __construct()
    {
        ...
        $class =  Reflection ::reflectClass($this);
        foreach ($system as $method) {
            $reflection = new  Method ($class, new ReflectionMethod($this, $method), 'system');
     ...
     *
     * @param  ServerReflectionFunctionAbstract  $method
     * @return void
     */
    protected function _fixTypes( ServerReflectionFunctionAbstract  $method)
    {
     ...
     *  ServerReflectionFunctionAbstract  pairs
     *
     * @return void
     */
    protected function _buildDispatchTable()
    {
        ...
        foreach ($this->_methods as $dispatchable) {
            if ($dispatchable instanceof  ServerReflectionFunctionAbstract ) {
                ...
                if (isset($table[$name])) {
                    throw new  XmlRpcServerException ('Duplicate method registered: ' . $name);
                }
            ...
            if ($dispatchable instanceof  ServerReflectionClass ) {
    ...
    public function addFunction($function, $namespace = '')
    {
        if (!is_string($function) && !is_array($function)) {
            throw new  XmlRpcServerException ('Unable to attach function; invalid', 611);
        }
     ...
     * results pulled from {@link  Cache ::get()}.
     *
     * @param  array $definition
     * @return void
     * @throws  XmlRpcServerException  on invalid input
     */
    public function loadFunctions($definition)
    {
        if (!is_array($definition)) {
            throw new  XmlRpcServerException ('Unable to load array; not an array', 612);
        }

        foreach ($definition as $key => $value) {
            if (!$value instanceof  ServerReflectionFunctionAbstract 
                && !$value instanceof  ServerReflectionClass )
            {
                throw new  XmlRpcServerException ('One or more method records are corrupt or otherwise unusable', 613);
            }
    ...
    /**
     * Do nothing; persistence is handled via {@link  Cache }
     *
     * @param  mixed $mode
     * @return void
     */
    ...
    public function setClass($class, $namespace = '', $argv = null)
    {
        if (is_string($class) && !class_exists($class)) {
            if (!class_exists($class)) {
                throw new  XmlRpcServerException ('Invalid method class', 610);
            }
        }
     ...
     * @return  Server 
     * @throws  XmlRpcServerException  on invalid request class or object
     */
    public function setRequest($request)
    {
        if (is_string($request) && class_exists($request)) {
            ...
            if (!$request instanceof  Request ) {
                throw new  XmlRpcServerException ('Invalid request class');
            }
            $request->setEncoding($this->getEncoding());
        } elseif (!$request instanceof  Request ) {
            throw new  XmlRpcServerException ('Invalid request object');
        }
    ...
    public function fault($fault = null, $code = 404)
    {
        if (!$fault instanceof \Exception) {
            ...
            $fault = new  XmlRpcServerException ($fault, $code);
        }
    ...
    protected function _handle( Request  $request)
    {
        ...
        if (!isset($this->_table[$method])) {
            throw new  XmlRpcServerException ('Method "' . $method . '" does not exist', 620);
        }
        ...
        if (!$matched) {
            throw new  XmlRpcServerException ('Calling parameters do not match signature', 623);
        }

        if ($info instanceof  ServerReflectionFunction ) {
            $func = $info->getName();
            $return = call_user_func_array($func, $params);
        } elseif (($info instanceof  Method ) && $info->system) {
            // System methods
            $return = $info->invokeArgs($this, $params);
        } elseif ($info instanceof  Method ) {
            // Get class
            ...
            if ('static' == $info->isStatic()) {
                ...
                try {
                    ...
                    throw new  XmlRpcServerException ('Error instantiating class ' . $class . ' to invoke method ' . $info->getName(), 621);
                }

                $return = $info->invokeArgs($object, $params);
            }
        } else {
            throw new  XmlRpcServerException ('Method missing implementation ' . get_class($info), 622);
        }
    ...
    public function setResponseClass($class)
    {
        if (class_exists($class)) {
            ...
            if ($reflection->isSubclassOf(new \ReflectionClass('\Zend\XmlRpc\Response'))) {
     ...
     *
     * Returns an array \of dispatchables ( ServerReflectionFunction ,
    ...
    public function getFunctions()
    {
        ...
        foreach ($this->_methods as $method) {
            if ($method instanceof  ServerReflectionClass 
                && ($method->system))
            {
                continue;
            }
    ...
    {
        if (!isset($this->_table[$method])) {
            throw new  ServerException ('Method "' . $method . '"does not exist', 640);
        }
