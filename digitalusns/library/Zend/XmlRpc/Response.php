...
use Zend\XmlRpc\Fault as Fault;
    ...
    public function loadXml($response)
    {
        ...
        try {
            if (!isset($xml->params) || !isset($xml->params->param) || !isset($xml->params->param->value)) {
                throw new  XmlRpcValueException ('Missing XML-RPC value in XML');
            }
            $valueXml = $xml->params->param->value->asXML();
            $valueXml = preg_replace('/<\?xml version=.*?\?>/i', '', $valueXml);
            $value =  Value ::getXmlRpcValue(trim($valueXml),  Value ::XML_STRING);
        } catch ( XmlRpcValueException  $e) {
