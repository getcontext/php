...
class  Fault 
{
     ...
     * @throws  XmlRpcException  if no or faulty XML provided, or if fault
     * response does not contain either code or message
     */
    public function loadXml($fault)
    {
        if (!is_string($fault)) {
            throw new  XmlRpcException ('Invalid XML provided to fault');
        }
        ...
        try {
            ...
            // Not valid XML
            throw new  XmlRpcException ('Failed to parse XML fault: ' .  $e->getMessage(), 500);
        }
        ...
        if (!$xml->fault->value->struct) {
            // not a proper fault
            throw new  XmlRpcException ('Invalid fault structure', 500);
        }

        $structXml = $xml->fault->value->asXML();
        $structXml = preg_replace('/<\?xml version=.*?\?>/i', '', $structXml);
        $struct    =  Value ::getXmlRpcValue(trim($structXml),  Value ::XML_STRING);
        ...
        if (empty($code) && empty($message)) {
            throw new  XmlRpcException ('Fault code and string required');
        }
