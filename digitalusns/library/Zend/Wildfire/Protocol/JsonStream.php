...
use Zend\Wildfire\Channel\HttpHeaders as HttpHeaders;
use Zend\Wildfire\Channel\ChannelInterface as WildfireChannelInterface;
use Zend\Wildfire\Plugin\PluginInterface as WildfirePluginInterface;
use Zend\Wildfire\Exception as WildfireException;
use Zend\Json\Encoder as Encoder;
...
class  JsonStream 
{
     ...
     * 
     * @param  WildfirePluginInterface  $plugin The plugin to be registered
     * @return boolean Returns TRUE if plugin was registered, false if it was already registered
     */
    public function registerPlugin( WildfirePluginInterface  $plugin)
    {
     ...
     * 
     * @param  WildfirePluginInterface  $plugin The plugin recording the message
     ...
     */
    public function recordMessage( WildfirePluginInterface  $plugin, $structure, $data)
    {
     ...
     * 
     * @param  WildfirePluginInterface  $plugin The plugin \for which to clear messages
     * @return boolean Returns TRUE if messages were present
     */
    public function clearMessages( WildfirePluginInterface  $plugin)
    {
        $uri = $plugin->getUri();
    ...
    {
        return  Encoder ::encode($value, true, array('silenceCyclicalExceptions'=>true));
    }

    /**
     * Retrieves all formatted data ready to be sent by the channel.
     * 
     * @param  WildfireChannelInterface  $channel The instance \of the channel that will be transmitting the data
     * @return mixed Returns the data to be sent by the channel.
     * @throws  WildfireException 
     */
    public function getPayload( WildfireChannelInterface  $channel)
    {
        if (!$channel instanceof  HttpHeaders ) {
            throw new  WildfireException ('The '.get_class($channel).' channel is not supported by the '.get_class($this).' protocol.');          
        }
                    ...
                    foreach (explode("\n",chunk_split($message, 4998, "\n")) as $part) {
                        ...
                        if ($part) {
                            ...
                            if ($message_index > 99999) {
                                throw new  WildfireException ('Maximum number (99,999) \of messages reached!');             
                            }
                        }
                    }
                }
