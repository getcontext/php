...
use Zend\Wildfire\Plugin\FirePhp\Message as Message;
...
class  FirePhp  implements  WildfirePluginInterface 
{
    /**
     * Plain log style.
     */
     ...
     */
    const PROTOCOL_URI =  JsonStream ::PROTOCOL_URI;
     ...
     *
     * @param string $class OPTIONAL Subclass \of  FirePhp 
     * @return  FirePhp  Returns the singleton  FirePhp  instance
     * @throws  WildfireException 
     */
    public static function init($class = null)
    {
        if (self::$_instance!==null) {
            throw new  WildfireException ('Singleton instance \of  FirePhp  already exists!');
        }
        if ($class!==null) {
            if (!is_string($class)) {
            ...
            if (!self::$_instance instanceof  FirePhp ) {
                self::$_instance = null;
                throw new  WildfireException ('Invalid class to third argument. Must be subclass \of  FirePhp .');
            }
        } else {
    ...
    {
        $this->_channel =  HttpHeaders ::getInstance();
    ...
    public static function send($var, $label=null, $style=null)
    {
        ...
        if ($style==self::TRACE) {
            ...
            \for ( $i=0 ; $i<sizeof($trace) ; $i++ ) {
                if (isset($trace[$i]['class']) &&
                    substr($trace[$i]['class'],0,8)!='\Zend\Log' &&
        ...
        switch ($style) {
            ...
            case self::DUMP:
                break;
            default:
                throw new  WildfireException ('Log style "'.$style.'" not recognized!');
                break;
        }
    ...
    protected function _recordMessage($structure, $data)
    {
        switch($structure) {
            ...
            case self::STRUCTURE_URI_DUMP:
                ...
                if (!isset($data['key'])) {
                    throw new  WildfireException ('You must supply a key.');
                }
                if (!isset($data['data'])) {
                    throw new  WildfireException ('You must supply data.');
                }
            ...
            case self::STRUCTURE_URI_FIREBUGCONSOLE:
                ...
                if (!isset($data['meta']) ||
                    ...
                    throw new  WildfireException ('You must supply a "Type" in the meta information.');
                }
                if (!isset($data['data'])) {
                    throw new  WildfireException ('You must supply data.');
                }
            ...
            default:
                throw new  WildfireException ('Structure \of name "'.$structure.'" is not recognized.');
                break;  
        }
        return false;      
    }
