...
use Zend\Search\Lucene\Index\SegmentInfoPriorityQueue as SegmentInfoPriorityQueue;
use Zend\Search\Lucene\Storage\Directory\Filesystem as Filesystem;
...
use Zend\Search\Lucene\Search\QueryHit as QueryHit;
use Zend\Search\Lucene\Index\TermInfo as TermInfo;
use Zend\Search\Lucene\Search\Query as Query;
use Zend\Search\Lucene\Index\Writer as Writer;
use Zend\Search\Lucene\LockManager as LockManager;
...
use Zend\Search\Lucene\Proxy as Proxy;
...
class  Lucene  implements  SearchLuceneInterface 
{
    /**
        ...
        try {
            \for ($count = 0; $count < self::GENERATION_RETRIEVE_COUNT; $count++) {
                ...
                if ($format != (int)0xFFFFFFFE) {
                    throw new  SearchLuceneException ('Wrong segments.gen file format');
                }
            ...
            throw new  SearchLuceneException ('Index is under processing now');
        } catch ( SearchLuceneException  $e) {
    ...
    public function setFormatVersion($formatVersion)
    {
    	if ($formatVersion != self::FORMAT_PRE_2_1  &&
    		...
    		throw new  SearchLuceneException ('Unsupported index format');
    	}
    ...
    private function _readPre21SegmentsFile()
    {
        ...
        if ($format != (int)0xFFFFFFFF) {
            throw new  SearchLuceneException ('Wrong segments file format');
        }
    ...
    private function _readSegmentsFile()
    {
        ...
        if ($format == (int)0xFFFFFFFC) {
            ...
            throw new  SearchLuceneException ('Unsupported segments file format');
        }
        ...
        \for ($count = 0; $count < $segments; $count++) {
            ...
            if ($numField != (int)0xFFFFFFFF) {
                ...
                throw new  SearchLuceneException ('Separate norm files are not supported. Optimize index to use it with  Lucene .');
            }
    ...
    {
        if ($directory === null) {
            throw new  SearchException ('No index directory specified');
        }
        ...
        if ($create) {
        	try {
        		...
        		if (strpos($e->getMessage(), 'Can\'t obtain exclusive index lock') === false) {
        			...
        			throw new  SearchLuceneException ('Can\'t create index. It\'s under processing now');
        		}
        	}
        ...
        if ($this->_generation == -1) {
            throw new  SearchLuceneException ('Index doesn\'t exists in the specified directory.');
    ...
    {
        if (!$this->_writer instanceof  Writer ) {
     ...
     * @param integer $id
     * @return boolean
     * @throws  SearchLuceneException     \Exception is thrown if $id is out \of the range
     */
    public function isDeleted($id)
    {
        if ($id >= $this->_docCount) {
            throw new  SearchLuceneException ('Document id is out \of the range.');
        }
    ...
    public function find($query)
    {
        ...
        if (!$query instanceof  Query ) {
            throw new  SearchLuceneException ('Query must be a string or  Query  object');
        }
        ...
        if (func_num_args() == 1) {
            ...
            \for ($count = 1; $count < count($argList); $count++) {
                ...
                if (!is_string($fieldName)) {
                    throw new  SearchLuceneException ('Field name must be a string.');
                }

                if (!in_array($fieldName, $fieldNames)) {
                    throw new  SearchLuceneException ('Wrong field name.');
                }
    ...
    public function hasTerm( Term  $term)
    {
        foreach ($this->_segmentInfos as $segInfo) {
            if ($segInfo->getTermInfo($term) instanceof  TermInfo ) {
                return true;
            }
        }
     ...
     * @return integer
     */
    public function termFreqs( Term  $term)
    {
        $result = array();
        $segmentStartDocId = 0;
        foreach ($this->_segmentInfos as $segmentInfo) {
     ...
     * @return array
     */
    public function termPositions( Term  $term)
    {
        $result = array();
        $segmentStartDocId = 0;
        foreach ($this->_segmentInfos as $segmentInfo) {
    ...
    public function terms()
    {
        ...
        $segmentInfoQueue = new  SegmentInfoPriorityQueue ();
    ...
    {
        $this->_termsStreamQueue = new  SegmentInfoPriorityQueue ();
