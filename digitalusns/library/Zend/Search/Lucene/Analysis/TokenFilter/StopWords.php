...
class  StopWords  extends  TokenFilter 
{
     ...
     * @throws  SearchException  When the file doesn`t exists or is not readable.
     */
    public function loadFromFile($filepath = null) {
        if (! $filepath || ! file_exists($filepath)) {
            throw new  SearchException ('You have to provide valid file path');
        }
        $fd = fopen($filepath, "r");
        if (! $fd) {
            throw new  SearchException ('Cannot open file ' . $filepath);
        }
        while (!feof ($fd)) {
        ...
        if (!fclose($fd)) {
            throw new  SearchException ('Cannot close file ' . $filepath);
        }
    }
}
