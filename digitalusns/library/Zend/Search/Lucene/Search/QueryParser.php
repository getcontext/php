...
use Zend\Search\Lucene\Search\QueryEntry\Subquery as Subquery;
use Zend\Search\Lucene\Search\QueryParserContext as QueryParserContext;
use Zend\Search\Lucene\Search\QueryEntry\Phrase as Phrase;
use Zend\Search\Lucene\Search\QueryEntry\Term as SearchLuceneSearchQueryEntryTerm;
use Zend\Search\Lucene\Search\Query\MultiTerm as MultiTerm;
use Zend\Search\Lucene\Search\Query\Range as Range;
use Zend\Search\Lucene\Search\QueryLexer as QueryLexer;
...
use Zend\Search\Lucene\FSMAction as FSMAction;
use Zend\Search\Lucene\Exception as SearchLuceneException;
use Zend\Search\Lucene\FSM as FSM;
...
class  QueryParser  extends  FSM 
{
     ...
     * Default is true.
     * 
     * Use {@link  Lucene ::suppressQueryParsingExceptions()},
     * {@link  Lucene ::dontSuppressQueryParsingExceptions()} and
     * {@link  Lucene ::checkQueryParsingExceptionsSuppressMode()} to operate
     * with this setting.
     * 
     * @var boolean
     */
     ...
     * @var integer
     */
    private $_defaultOperator = self::\B\BOR;
        ...
        try {
            ...
            foreach (self::$_instance->_tokens as $token) {
                try {
                    ...
                    if (strpos($e->getMessage(), 'There is no any rule \for') !== false) {
                        throw new  QueryParserException ( 'Syntax error at char position ' . $token->position . '.' );
                    }
            ...
            if (self::$_instance->_suppressQueryParsingExceptions) {
                ...
                foreach ($queryTokens as $token) {
                    $query->addTerm(new  SearchLuceneIndexTerm ($token->getTermText()), $termsSign);
                }
    ...
    {
        $entry = new  SearchLuceneSearchQueryEntryTerm ($this->_currentToken->text, $this->_context->getField());
    ...
    public function processModifierParameter()
    {
        ...
        switch ($this->_lastToken->type) {
            ...
            default:
                ...
                throw new  SearchLuceneException ('Lexeme modifier parameter must follow lexeme modifier. Char position 0.' );
        }
    }
    ...
    public function openedRQLastTerm()
    {
        ...
        if (count($tokens) > 1) {
            ...
            $from = new  SearchLuceneIndexTerm (reset($tokens)->getTermText(), $this->_context->getField());
        } else {
            $from = null;
        }
        ...
        if (count($tokens) > 1) {
            ...
            $to = new  SearchLuceneIndexTerm (reset($tokens)->getTermText(), $this->_context->getField());
        } else {
            $to = null;
        }
