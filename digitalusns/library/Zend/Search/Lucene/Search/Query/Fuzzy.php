...
class  Fuzzy  extends  Query 
{
    ...
    /**
     *  Query _Wildcard constructor.
     *
     * @param  SearchLuceneIndexTerm  $pattern
     * @throws  SearchLuceneException 
     */
    public function __construct( SearchLuceneIndexTerm  $term, $minimumSimilarity = self::DEFAULT_MIN_SIMILARITY, $prefixLength = 0)
    {
        if ($minimumSimilarity < 0) {
            throw new  SearchLuceneException ('minimumSimilarity cannot be less than 0');
        }
    	if ($minimumSimilarity >= 1) {
    		throw new  SearchLuceneException ('minimumSimilarity cannot be greater than or equal to 1');
    	}
        if ($prefixLength < 0) {
            throw new  SearchLuceneException ('prefixLength cannot be less than 0');
        }
    ...
    public function rewrite( SearchLuceneInterface  $index)
    {
        ...
        $prefix           =  SearchLuceneIndexTerm ::getPrefix($this->_term->text, $this->_prefixLength);
        $prefixByteLength = strlen($prefix);
        $prefixUtf8Length =  SearchLuceneIndexTerm ::getLength($prefix);
        
        $termLength       =  SearchLuceneIndexTerm ::getLength($this->_term->text);
        ...
        if (count($this->_matches) == 0) {
            ...
            $rewrittenQuery = new  SearchLuceneSearchQueryBoolean ();
            ...
            foreach ($this->_matches as $id => $matchedTerm) {
            	$subquery = new  SearchLuceneSearchQueryTerm ($matchedTerm);
