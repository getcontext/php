...
/**  SearchLuceneSearchWeightBoolean  */
...
use Zend\Search\Lucene\Search\Weight\WeightBoolean as SearchLuceneSearchWeightBoolean;
use Zend\Search\Lucene\Search\Query\QueryEmpty as SearchLuceneSearchQueryEmpty;
use Zend\Search\Lucene\Search\Query\Term as Term;
...
class  QueryBoolean  extends  Query 
{
    ...
    public function optimize( SearchLuceneInterface  $index)
    {
        ...
        foreach ($subqueries as $id => $subquery) {
            if ($subquery instanceof  SearchLuceneSearchQueryEmpty ) {
                if ($signs[$id] === true) {
        ...
        $optimizedQuery = new  QueryBoolean ($subqueries, $signs);
    ...
    {
        $this->_weight = new  SearchLuceneSearchWeightBoolean ($this, $reader);
        return $this->_weight;
    }
     ...
     * @return float
     */
    public function _nonConjunctionScore($docId,  SearchLuceneInterface  $reader)
    {
        if ($this->_coord === null) {
            $this->_coord = array();
