...
/**
 * \Zend\Search\Lucene\Search\Weight\MultiTerm
 */
...
use Zend\Search\Lucene\Search\Weight\Phrase as SearchLuceneSearchWeightPhrase;
...
class  Phrase  extends  Query 
{
    ...
    public function __construct($terms = null, $offsets = null, $field = null)
    {
        ...
        if (is_array($terms)) {
            ...
            foreach ($terms as $termId => $termText) {
                $this->_terms[$termId] = ($field !== null)? new  SearchLuceneIndexTerm ($termText, $field):
                                                            new  SearchLuceneIndexTerm ($termText);
            }
        } else if ($terms === null) {
            $this->_terms = array();
        } else {
            throw new  SearchLuceneException ('terms argument must be array \of strings or null');
        }

        if (is_array($offsets)) {
            if (count($this->_terms) != count($offsets)) {
                throw new  SearchLuceneException ('terms and offsets arguments must have the same size.');
            }
            ...
            throw new  SearchLuceneException ('offsets argument must be array \of strings or null');
        }
    }
     ...
     */
    public function addTerm( SearchLuceneIndexTerm  $term, $position = null) {
        if ((count($this->_terms) != 0)&&(end($this->_terms)->field != $term->field)) {
            throw new  SearchLuceneException ('All phrase terms must be in the same field: ' .
    ...
    public function rewrite( SearchLuceneInterface  $index)
    {
        if (count($this->_terms) == 0) {
            ...
            foreach ($index->getFieldNames(true) as $fieldName) {
                ...
                foreach ($this->_terms as $termId => $term) {
                    $qualifiedTerm = new  SearchLuceneIndexTerm ($term->text, $fieldName);
    ...
    public function optimize( SearchLuceneInterface  $index)
    {
        ...
        if (count($this->_terms) == 1) {
            // It's one term query
            $optimizedQuery = new  SearchLuceneSearchQueryTerm (reset($this->_terms));
     ...
     *
     * @param integer $num
     * @param  SearchLuceneSearchWeightTerm  $weight
     */
    ...
    {
        $this->_weight = new  SearchLuceneSearchWeightPhrase ($this, $reader);
        return $this->_weight;
    }
     ...
     * @return float
     */
    public function _sloppyPhraseFreq($docId,  SearchLuceneInterface  $reader)
    {
        $freq = 0;
