...
class  LockManager 
{
    ...
    public static function obtainWriteLock( Directory  $lockDirectory)
    {
        ...
        if (!$lock->lock(LOCK_EX)) {
            throw new  SearchLuceneException ('Can\'t obtain exclusive index lock');
        }
        return $lock;
    }
    ...
    private static function _startReadLockProcessing( Directory  $lockDirectory)
    {
        ...
        if (!$lock->lock(LOCK_EX)) {
            throw new  SearchLuceneException ('Can\'t obtain exclusive lock \for the read lock processing file');
        }
        return $lock;
    }
    ...
    public static function obtainReadLock( Directory  $lockDirectory)
    {
        ...
        if (!$lock->lock(LOCK_SH)) {
            ...
            throw new  SearchLuceneException ('Can\'t obtain shared reading index lock');
        }
        return $lock;
    }
