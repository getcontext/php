...
/**  SearchLuceneStorageFileFilesystem  */
...
use Zend\Search\Lucene\Storage\File\Filesystem as SearchLuceneStorageFileFilesystem;
...
class  Filesystem  extends  Directory 
{
    ...
    /**
     * Cache \for  SearchLuceneStorageFileFilesystem  objects
     * Array: filename => \Zend\Search\Lucene\Storage\File object
     *
     * @var array
    ...
    public function __construct($path)
    {
        if (!is_dir($path)) {
            if (file_exists($path)) {
                ...
                throw new  SearchLuceneException ('Path exists, but it\'s not a directory');
            } else {
                if (!self::mkdirs($path)) {
                    require_once 'Zend/Search/Lucene/Exception.php';
                    throw new  SearchLuceneException ("Can't create directory '$path'.");
                }
            }
        }
        $this->_dirPath = $path;
    ...
    public function createFile($filename)
    {
        ...
        $this->_fileHandlers[$filename] = new  SearchLuceneStorageFileFilesystem ($this->_dirPath . '/' . $filename, 'w+b');
    ...
    public function deleteFile($filename)
    {
        ...
        if (!@unlink($this->_dirPath . '/' . $filename)) {
            ...
            require_once 'Zend/Search/Lucene/Exception.php';
            throw new  SearchLuceneException ('Can\'t delete file: ' . $php_errormsg);
        }
        ini_set('track_errors', $trackErrors);
    }
    ...
    public function renameFile($from, $to)
    {
        ...
        if (file_exists($this->_dirPath . '/' . $to)) {
            if (!unlink($this->_dirPath . '/' . $to)) {
                require_once 'Zend/Search/Lucene/Exception.php';
                throw new  SearchLuceneException ('Delete operation failed');
            }
        }
        ...
        if (!$success) {
            ini_set('track_errors', $trackErrors);
            require_once 'Zend/Search/Lucene/Exception.php';
            throw new  SearchLuceneException ($php_errormsg);
        }
    ...
    /**
     * Returns a \Zend\Search\Lucene\Storage\File object \for a given $filename in the directory.
     *
    ...
    public function getFileObject($filename, $shareHandler = true)
    {
        ...
        if (!$shareHandler) {
            return new  SearchLuceneStorageFileFilesystem ($fullFilename);
        }
        ...
        $this->_fileHandlers[$filename] = new  SearchLuceneStorageFileFilesystem ($fullFilename);
        return $this->_fileHandlers[$filename];
    }
}
