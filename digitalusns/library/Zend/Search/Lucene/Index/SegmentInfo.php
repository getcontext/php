...
/** \Zend\Search\Lucene\LockManager */
...
use Zend\Search\Lucene\Index\DictionaryLoader as DictionaryLoader;
use Zend\Search\Lucene\Search\Similarity as Similarity;
use Zend\Search\Lucene\Storage\Directory as Directory;
use Zend\Search\Lucene\Index\FieldInfo as FieldInfo;
...
class  SegmentInfo 
{
    ...
    public function openCompoundFile($extension, $shareHandler = true)
    {
        if (($extension == '.fdx'  || $extension == '.fdt')  &&  $this->_usesSharedDocStore) {
            ...
            if( !isset($this->_sharedDocStoreOptions['files'][$fdxFName]) ) {
                throw new  SearchLuceneException ('Shared doc storage segment compound file doesn\'t contain '
                                       . $fdxFName . ' file.' );
            }
        ...
        if( !isset($this->_segFiles[$filename]) ) {
            throw new  SearchLuceneException ('Segment compound file doesn\'t contain '
                                       . $filename . ' file.' );
        }
    ...
    public function compoundFileLength($extension)
    {
        if (($extension == '.fdx'  || $extension == '.fdt')  &&  $this->_usesSharedDocStore) {
            ...
            if( !isset($this->_sharedDocStoreOptions['fileSizes'][$filename]) ) {
                throw new  SearchLuceneException ('Shared doc store compound file doesn\'t contain '
                                           . $filename . ' file.' );
            }
        ...
        if( !isset($this->_segFileSizes[$filename]) ) {
            throw new  SearchLuceneException ('Index compound file doesn\'t contain '
                                       . $filename . ' file.' );
        }
    ...
    public function getTermInfo( Term  $term)
    {
        ...
        if ($tiVersion != (int)0xFFFFFFFE /* pre-2.1 format */  &&
            ...
            throw new  SearchLuceneException ('Wrong TermInfoFile file format');
        }
     ...
     * @return  TermInfo 
     */
    public function termFreqs( Term  $term, $shift = 0)
    {
     ...
     * @return  TermInfo 
     */
    public function termPositions( Term  $term, $shift = 0)
    {
        $termInfo = $this->getTermInfo($term);
    ...
    private function _loadNorm($fieldNum)
    {
        if ($this->_hasSingleNormFile) {
            ...
            if ($header != 'NRM'  ||  $headerFormatVersion != (int)0xFF) {
                throw new   SearchLuceneException ('Wrong norms file format.');
            }
    ...
    public function reset($startId = 0, $mode = self::SM_TERMS_ONLY)
    {
        ...
        switch ($mode) {
            ...
            default:
                throw new  SearchLuceneException ('Wrong terms scaning mode specified.');
                break;
        }
