...
class  DictionaryLoader 
{
     ...
     *
     * See \Zend\Search\Lucene_Index_SegmintInfo class \for details
     *
     * @param string $data
     * @return array
    ...
    public static function load($data)
    {
        ...
        if ($tiVersion != (int)0xFFFFFFFE /* pre-2.1 format */ &&
            ...
            throw new  SearchLuceneException ('Wrong TermInfoIndexFile file format');
        }
        ...
        if (PHP_INT_SIZE > 4) {
            ...
            if ((ord($data[$pos])            != 0) ||
                ...
                ((ord($data[$pos+4]) & 0x80) != 0)) {
                     throw new  SearchLuceneException ('Largest supported segment size (\for 32-bit mode) is 2Gb');
                 }
        ...
        $pos += 4;
        if ($indexTermCount < 1) {
            throw new  SearchLuceneException ('Wrong number \of terms in a term dictionary index');
        }
        ...
        \for ($count = 0; $count < $indexTermCount; $count++) {
            ...
            // $termValue        =  Term ::getPrefix($prevTerm, $termPrefixLength) . $termSuffix;
            $pb = 0; $pc = 0;
            ...
            // $this->_termDictionary[] =  new  Term ($termValue, $termFieldNum);
            $termDictionary[] = array($termFieldNum, $termValue);

            $termInfos[] =
                 // new  TermInfo ($docFreq, $freqPointer, $proxPointer, $skipDelta, $indexPointer);
