...
/** \Zend\Search\Lucene\Index\SegmentInfo */
...
// not supported yet            0x08 /* term offsets are stored with the term vectors */   |
    ...
    public function addTerm($termEntry, $termDocs)
    {
        ...
        $term = new  Term ($termEntry->text,
                                                  $this->_fields[$termEntry->field]->number);
        $termInfo = new  TermInfo (count($termDocs),
    ...
    protected function _dumpTermDictEntry( File  $dicFile,
                                        &$prevTerm,      Term      $term,
                                        &$prevTermInfo,  TermInfo  $termInfo)
    {
