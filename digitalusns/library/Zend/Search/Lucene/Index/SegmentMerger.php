...
use Zend\Search\Lucene\Index\SegmentWriter\StreamWriter as StreamWriter;
...
class  SegmentMerger 
{
    ...
    {
        if ($this->_mergeDone) {
            throw new  SearchLuceneException ('Merge is already done.');
        }

        if (count($this->_segmentInfos) < 1) {
            throw new  SearchLuceneException ('Wrong number \of segments to be merged ('
    ...
    private function _mergeTerms()
    {
        ...
        $segmentStartId = 0;
        foreach ($this->_segmentInfos as $segName => $segmentInfo) {
            $segmentStartId = $segmentInfo->reset($segmentStartId,  SegmentInfo ::SM_MERGE_INFO);
