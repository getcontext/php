...
abstract class  FSM 
{
    ...
    public function setState($state)
    {
        if (!isset($this->_states[$state])) {
            throw new  SearchException ('State \'' . $state . '\' is not on \of the possible FSM states.');
        }
    ...
    public function addRule($sourceState, $input, $targetState, $inputAction = null)
    {
        if (!isset($this->_states[$sourceState])) {
            throw new  SearchException ('Undefined source state (' . $sourceState . ').');
        }
        if (!isset($this->_states[$targetState])) {
            throw new  SearchException ('Undefined target state (' . $targetState . ').');
        }
        if (!isset($this->_inputAphabet[$input])) {
            throw new  SearchException ('Undefined input symbol (' . $input . ').');
        }
        ...
        if (isset($this->_rules[$sourceState][$input])) {
            throw new  SearchException ('Rule \for {state,input} pair (' . $sourceState . ', '. $input . ') is already defined.');
        }
    ...
    {
        if (!isset($this->_states[$state])) {
            throw new  SearchException ('Undefined state (' . $state. ').');
        }
    ...
    public function addInputAction($state, $inputSymbol,  FSMAction  $action)
    {
        ...
        if (!isset($this->_inputAphabet[$inputSymbol])) {
            throw new  SearchException ('Undefined input symbol (' . $inputSymbol. ').');
        }
    ...
    public function addTransitionAction($sourceState, $targetState,  FSMAction  $action)
    {
        ...
        if (!isset($this->_states[$targetState])) {
            throw new  SearchException ('Undefined source state (' . $targetState. ').');
        }
    ...
    public function process($input)
    {
        if (!isset($this->_rules[$this->_currentState])) {
            throw new  SearchException ('There is no any rule \for current state (' . $this->_currentState . ').');
        }
        if (!isset($this->_rules[$this->_currentState][$input])) {
            throw new  SearchException ('There is no any rule \for {current state, input} pair (' . $this->_currentState . ', ' . $input . ').');
        }
    ...
    public function reset()
    {
        if (count($this->_states) == 0) {
            throw new  SearchException ('There is no any state defined \for FSM.');
        }
