...
/**
 * \Zend\Layout_View_Helper_HeadLink
 *
...
use Zend\View\ViewAbstract as ViewAbstract;




class  HeadLink  extends  Standalone 
{
    /**
     * $_validAttributes
     *
     ...
     */
    protected $_regKey = '\Zend\View\Helper\HeadLink';
     ...
     * @return  HeadLink 
     */
    public function headLink(array $attributes = null, $placement =  ViewHelperPlaceholderContainerAbstract ::APPEND)
    {
        if (null !== $attributes) {
    ...
    public function __call($method, $args)
    {
        if (preg_match('/^(?P<action>set|(ap|pre)pend|offsetSet)(?P<type>Stylesheet|Alternate)$/', $method, $matches)) {
            ...
            if (1 > $argc) {
                ...
                throw new  ViewException (sprintf('%s requires at least one argument', $method));
            }
    ...
    public function append($value)
    {
        if (!$this->_isValid($value)) {
            require_once 'Zend/View/Exception.php';
            throw new  ViewException ('append() expects a data token; please use one \of the custom append*() methods');
        }
    ...
    public function offsetSet($index, $value) 
    {
        if (!$this->_isValid($value)) {
            require_once 'Zend/View/Exception.php';
            throw new  ViewException ('offsetSet() expects a data token; please use one \of the custom offsetSet*() methods');
        }
     ...
     *
     * @param  array $value
     * @return \Zend\Layout_ViewHelper_HeadLink
     */
    public function prepend($value)
    {
        if (!$this->_isValid($value)) {
            require_once 'Zend/View/Exception.php';
            throw new  ViewException ('prepend() expects a data token; please use one \of the custom prepend*() methods');
        }
    ...
    public function set($value)
    {
        if (!$this->_isValid($value)) {
            require_once 'Zend/View/Exception.php';
            throw new  ViewException ('set() expects a data token; please use one \of the custom set*() methods');
        }
    ...
    public function createDataAlternate(array $args)
    {
        if (3 > count($args)) {
            require_once 'Zend/View/Exception.php';
            throw new  ViewException (sprintf('Alternate tags require 3 arguments; %s provided', count($args)));
        }
