...
use Zend\View\Helper\Placeholder\Container\Standalone as Standalone;
use Zend\View\Helper\Placeholder\Container\Exception as ViewHelperPlaceholderContainerException;
...
class  HeadScript  extends  Standalone 
{
    /**#@+
     ...
     * @var string
     */
    protected $_regKey = '\Zend\View\Helper\HeadScript';
     ...
     * @return  HeadScript 
     */
    public function headScript($mode =  HeadScript ::FILE, $spec = null, $placement = 'APPEND', array $attrs = array(), $type = 'text/javascript')
    {
     ...
     * @return void
     */
    public function captureStart($captureType =  ViewHelperPlaceholderContainerAbstract ::APPEND, $type = 'text/javascript', $attrs = array())
    {
        if ($this->_captureLock) {
            require_once 'Zend/View/Helper/Placeholder/Container/Exception.php';
            throw new  ViewHelperPlaceholderContainerException ('Cannot nest headScript captures');
        }
    ...
    public function captureEnd()
    {
        ...
        switch ($this->_captureType) {
            case  ViewHelperPlaceholderContainerAbstract ::SET:
            case  ViewHelperPlaceholderContainerAbstract ::PREPEND:
            case  ViewHelperPlaceholderContainerAbstract ::APPEND:
     ...
     * @param  array $args 
     * @return  HeadScript 
     * @throws  ViewException  if too few arguments or invalid method
     */
    public function __call($method, $args)
    {
        if (preg_match('/^(?P<action>set|(ap|pre)pend|offsetSet)(?P<mode>File|Script)$/', $method, $matches)) {
            if (1 > count($args)) {
                ...
                throw new  ViewException (sprintf('Method "%s" requires at least one argument', $method));
            }
            ...
            if ('offsetSet' == $action) {
                ...
                if (1 > count($args)) {
                    require_once 'Zend/View/Exception.php';
                    throw new  ViewException (sprintf('Method "%s" requires at least two arguments, an index and source', $method));
                }
            }
    ...
    public function append($value)
    {
        if (!$this->_isValid($value)) {
            require_once 'Zend/View/Exception.php';
            throw new  ViewException ('Invalid argument passed to append(); please use one \of the helper methods, appendScript() or appendFile()');
        }
    ...
    public function prepend($value)
    {
        if (!$this->_isValid($value)) {
            require_once 'Zend/View/Exception.php';
            throw new  ViewException ('Invalid argument passed to prepend(); please use one \of the helper methods, prependScript() or prependFile()');
        }
    ...
    public function set($value)
    {
        if (!$this->_isValid($value)) {
            require_once 'Zend/View/Exception.php';
            throw new  ViewException ('Invalid argument passed to set(); please use one \of the helper methods, setScript() or setFile()');
        }
    ...
    public function offsetSet($index, $value)
    {
        if (!$this->_isValid($value)) {
            require_once 'Zend/View/Exception.php';
            throw new  ViewException ('Invalid argument passed to offsetSet(); please use one \of the helper methods, offsetSetScript() or offsetSetFile()');
        }
