...
/**
 * \Zend\Layout_View_Helper_HeadMeta
 *
...
class  HeadMeta  extends  Standalone 
{
    /**
     * Types \of attributes
     * @var array
     ...
     */
    protected $_regKey = '\Zend\View\Helper\HeadMeta';
     ...
     * @return  HeadMeta 
     */
    public function headMeta($content = null, $keyValue = null, $keyType = 'name', $modifiers = array(), $placement =  ViewHelperPlaceholderContainerAbstract ::APPEND)
    {
    ...
    protected function _normalizeType($type)
    {
        switch ($type) {
            ...
            default:
                require_once 'Zend/View/Exception.php';
                throw new  ViewException (sprintf('Invalid type "%s" passed to _normalizeType', $type));
        }
    }
    ...
    public function __call($method, $args)
    {
        if (preg_match('/^(?P<action>set|(pre|ap)pend|offsetSet)(?P<type>Name|HttpEquiv)$/', $method, $matches)) {
            ...
            if (2 > $argc) {
                require_once 'Zend/View/Exception.php';
                throw new  ViewException ('Too few arguments provided; requires key value, and content');
            }
    ...
    public function append($value)
    {
        if (!$this->_isValid($value)) {
            require_once 'Zend/View/Exception.php';
            throw new  ViewException ('Invalid value passed to append; please use appendMeta()');
        }
    ...
    public function offsetSet($index, $value)
    {
        if (!$this->_isValid($value)) {
            require_once 'Zend/View/Exception.php';
            throw new  ViewException ('Invalid value passed to offsetSet; please use offsetSetMeta()');
        }
    ...
    public function offsetUnset($index)
    {
        if (!in_array($index, $this->getContainer()->getKeys())) {
            require_once 'Zend/View/Exception.php';
            throw new  ViewException ('Invalid index passed to offsetUnset.');
        }
    ...
    public function prepend($value)
    {
        if (!$this->_isValid($value)) {
            require_once 'Zend/View/Exception.php';
            throw new  ViewException ('Invalid value passed to prepend; please use prependMeta()');
        }
    ...
    public function set($value)
    {
        if (!$this->_isValid($value)) {
            require_once 'Zend/View/Exception.php';
            throw new  ViewException ('Invalid value passed to set; please use setMeta()');
        }
    ...
    public function itemToString(\stdClass $item)
    {
        if (!in_array($item->type, $this->_typeKeys)) {
            require_once 'Zend/View/Exception.php';
            throw new  ViewException (sprintf('Invalid type "%s" provided \for meta', $item->type));
        }
        $type = $item->type;
