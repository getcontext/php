...
/**
 * Abstract class \for  ZendView  to help enforce private constructs.
 *
 * @category   Zend
 * @package     ZendView 
 * @copyright  Copyright (c) 2005-2008 Zend Technologies USA Inc. (http://www.zend.com)
...
use Zend\Loader\Exception as LoaderException;
...
abstract class  ViewAbstract  implements  ViewInterface 
{
    ...
    /**
     * Stack \of  ZendView _Filter names to apply as filters.
     * @var array
     */
    private $_filter = array();

    /**
     * Stack \of  ZendView _Filter objects that have been loaded
     * @var array
     */
    ...
    /**
     * Allow custom object initialization when extending  ViewAbstract  or
     *  ZendView 
     *
     ...
     * @return void
     * @throws  ViewException  if an attempt to set a private or protected
     * member is detected
     */
    public function __set($key, $val)
    {
        ...
        throw new  ViewException ('Setting private or protected class members is not allowed', $this);
    }
     ...
     *
     * To clear all paths, use  ZendView ::setScriptPath(null).
     *
     ...
     * @throws  ViewException  if no script directory set
     */
    ...
    public function setPluginLoader( PluginLoader  $loader, $type)
    {
        ...
        if (!in_array($type, $this->_loaderTypes)) {
            require_once 'Zend/View/Exception.php';
            throw new  ViewException (sprintf('Invalid plugin loader type "%s"', $type));
        }
    ...
    public function getPluginLoader($type)
    {
        ...
        if (!in_array($type, $this->_loaderTypes)) {
            require_once 'Zend/View/Exception.php';
            throw new  ViewException (sprintf('Invalid plugin loader type "%s"; cannot retrieve', $type));
        }
     ...
     * directory; defaults to  ZendView _Helper
     * @return  ViewAbstract 
     */
    public function addHelperPath($path, $classPrefix = '\Zend\View_Helper_')
    {
     ...
     *
     * To clear all paths, use  ZendView ::setHelperPath(null).
     *
     ...
     * $path; defaults to  ZendView _Helper
     * @return  ViewAbstract 
     */
    public function setHelperPath($path, $classPrefix = '\Zend\View_Helper_')
    {
     ...
     * @param string|array The directory (-ies) to add.
     * @param string $classPrefix Class prefix to use with classes in this
     * directory; defaults to  ZendView _Filter
     * @return  ViewAbstract 
     */
    public function addFilterPath($path, $classPrefix = '\Zend\View_Filter_')
    {
     ...
     *
     * To clear all paths, use  ZendView ::setFilterPath(null).
     *
     * @param string|array The directory (-ies) to set as the path.
     * @param string $classPrefix The class prefix to apply to all elements in
     * $path; defaults to  ZendView _Filter
     * @return  ViewAbstract 
     */
    public function setFilterPath($path, $classPrefix = '\Zend\View_Filter_')
    {
     ...
     *
     * To clear all filters, use  ZendView ::setFilter(null).
     *
     ...
     *
     *  ZendView ::assign('name', $value) assigns a variable called 'name'
     * with the corresponding $value.
     *
     *  ZendView ::assign($array) assigns the array keys as variable
     ...
     * as the value.
     * @return  ViewAbstract  Fluent interface
     * @throws  ViewException  if $spec is neither a string nor an array,
    ...
    public function assign($spec, $value = null)
    {
        ...
        if (is_string($spec)) {
            ...
            require_once 'Zend/View/Exception.php';
            throw new  ViewException ('assign() expects a string or array, received ' . gettype($spec), $this);
        }
     ...
     *
     * Clears all variables assigned to  ZendView  either via {@link assign()} or
     ...
     *
     * Use in conjunction with {@link  ZendView _Helper_DeclareVars the declareVars() helper}
    ...
    protected function _script($name)
    {
        if (0 == count($this->_path['script'])) {
            require_once 'Zend/View/Exception.php';
            throw new  ViewException ('no view script directory set; unable to determine location \for view script',
                $this);
        }
     ...
     *
     *  ZendView ::_addPath($type, 'dirname') adds one directory
     * to the path stack.
     *
     *  ZendView ::_addPath($type, $array) adds one directory \for
        ...
        switch ($type) {
            ...
            default:
                $this->_path[$type] = array(array(
                    'prefix' => '\Zend\View_' . ucfirst($type) . '_',
     ...
     * @throws  ViewException  if unable to load class
     */
    private function _loadClass($type, $name)
    {
        ...
        foreach ($this->_path[$type] as $info) {
            ...
            if (class_exists($class, false)) {
                ...
                return $class;
            } elseif ( Loader ::isReadable($dir . $file)) {
        ...
        require_once 'Zend/View/Exception.php';
        throw new  ViewException ("$type '$name' not found in path", $this);
    }
