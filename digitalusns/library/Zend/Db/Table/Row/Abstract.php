...
use Zend\Db\Table\Row\Exception as DbTableRowException;
use Zend\Db\Table\TableAbstract as DbTableAbstract;
use Zend\Db\Table\Select as Select;
...
abstract class  RowAbstract 
{
    ...
    /**
     *  DbTableAbstract  parent class or instance.
     *
     * @var  DbTableAbstract 
     */
     ...
     * the database schema (e.g. evaluated columns/ Db _Expr columns). This can also be passed
    ...
    /**
     * Name \of the class \of the  DbTableAbstract  object.
     *
     * @var string
     */
     ...
     * - table       = class name or object \of type  DbTableAbstract 
     * - data        = values \of columns in this row.
     *
     * @param  array $config OPTIONAL Array \of user-specified config options.
     * @return void
     * @throws  DbTableRowException 
     */
    public function __construct(array $config = array())
    {
        if (isset($config['table']) && $config['table'] instanceof  DbTableAbstract ) {
        ...
        if (isset($config['data'])) {
            if (!is_array($config['data'])) {
                ...
                throw new  DbTableRowException ('\Data must be an array');
            }
     ...
     * @throws  DbTableRowException  if the $columnName is not a string.
     */
    protected function _transformColumn($columnName)
    {
        if (!is_string($columnName)) {
            require_once 'Zend/Db/Table/Row/Exception.php';
            throw new  DbTableRowException ('Specified column is not a string');
        }
     ...
     * @throws  DbTableRowException  if the $columnName is not a column in the row.
     */
    public function __get($columnName)
    {
        ...
        if (!array_key_exists($columnName, $this->_data)) {
            require_once 'Zend/Db/Table/Row/Exception.php';
            throw new  DbTableRowException ("Specified column \"$columnName\" is not in the row");
        }
     ...
     * @throws  DbTableRowException 
     */
    public function setTable( DbTableAbstract  $table = null)
    {
        if ($table == null) {
            $this->_table = null;
            $this->_connected = false;
        ...
        if (! $table instanceof $this->_tableClass) {
            require_once 'Zend/Db/Table/Row/Exception.php';
            throw new  DbTableRowException ("The specified Table is \of class $tableClass, expecting class to be instance \of $this->_tableClass");
        }
        ...
        if ($info['cols'] != array_keys($this->_data)) {
            require_once 'Zend/Db/Table/Row/Exception.php';
            throw new  DbTableRowException ('The specified Table does not have the same columns as the Row');
        }

        if (! array_intersect((array) $this->_primary, $info['primary']) == (array) $this->_primary) {

            require_once 'Zend/Db/Table/Row/Exception.php';
            throw new  DbTableRowException ("The specified Table '$tableClass' does not have the same primary key as the Row");
        }
    ...
    /**
     * Returns an instance \of the parent table's  Select  object.
     *
     * @return  Select 
     */
    ...
    protected function _doInsert()
    {
        ...
        if ($this->_readOnly === true) {
            require_once 'Zend/Db/Table/Row/Exception.php';
            throw new  DbTableRowException ('This row has been marked read-only');
        }
        ...
        if (count($pkDiffData) > 0) {
            ...
            if (!empty($depTables)) {
                ...
                foreach ($depTables as $tableClass) {
                    try {
                        @ Loader ::loadClass($tableClass);
                    } catch (\Zend_Exception $e) {
                        require_once 'Zend/Db/Table/Row/Exception.php';
                        throw new  DbTableRowException ($e->getMessage());
                    }
     ...
     *
     * @param  array $data
     * @return  RowAbstract  Provides a fluent interface
     */
    ...
    protected function _getTable()
    {
        if (!$this->_connected) {
            require_once 'Zend/Db/Table/Row/Exception.php';
            throw new  DbTableRowException ('Cannot save a Row unless it is connected');
        }
        return $this->_table;
    }
    ...
    protected function _getPrimaryKey($useDirty = true)
    {
        if (!is_array($this->_primary)) {
            require_once 'Zend/Db/Table/Row/Exception.php';
            throw new  DbTableRowException ("The primary key must be set as an array");
        }
        ...
        if (count($primary) != count($array)) {
            require_once 'Zend/Db/Table/Row/Exception.php';
            throw new  DbTableRowException ("The specified Table '$this->_tableClass' does not have the same primary key as the Row");
        }
        return $array;
    }
    ...
    protected function _getWhereQuery($useDirty = true)
    {
        ...
        $metadata = $info[ DbTableAbstract ::METADATA];

        // retrieve recently updated row using primary keys
        $where = array();
        foreach ($primaryKey as $column => $value) {
            $tableName = $db->quoteIdentifier($info[ DbTableAbstract ::NAME]);
    ...
    protected function _refresh()
    {
        ...
        if (null === $row) {
            require_once 'Zend/Db/Table/Row/Exception.php';
            throw new  DbTableRowException ('Cannot refresh row as parent is missing');
        }
     ...
     *
     * @param  DbTableAbstract  $dependentTable
     * @param  DbTableAbstract  $parentTable
     * @param string                 $ruleKey
     * @return array
     */
    protected function _prepareReference( DbTableAbstract  $dependentTable,  DbTableAbstract  $parentTable, $ruleKey)
    {
        $map = $dependentTable->getReference(get_class($parentTable), $ruleKey);

        if (!isset($map[ DbTableAbstract ::REF_COLUMNS])) {
            $parentInfo = $parentTable->info();
            $map[ DbTableAbstract ::REF_COLUMNS] = array_values((array) $parentInfo['primary']);
        }

        $map[ DbTableAbstract ::COLUMNS] = (array) $map[ DbTableAbstract ::COLUMNS];
        $map[ DbTableAbstract ::REF_COLUMNS] = (array) $map[ DbTableAbstract ::REF_COLUMNS];
     ...
     *
     * @param string| DbTableAbstract   $dependentTable
     * @param string                         OPTIONAL $ruleKey
     * @param  Select            OPTIONAL $select
     * @return  Db _Table_Rowset_Abstract Query result from $dependentTable
     * @throws  DbTableRowException  If $dependentTable is not a table or is not loadable.
     */
    public function findDependentRowset($dependentTable, $ruleKey = null,  Select  $select = null)
    {
        ...
        if (is_string($dependentTable)) {
            try {
                @ Loader ::loadClass($dependentTable);
            } catch (\Zend_Exception $e) {
                require_once 'Zend/Db/Table/Row/Exception.php';
                throw new  DbTableRowException ($e->getMessage());
            }
        ...
        if (! $dependentTable instanceof  DbTableAbstract ) {
            $type = gettype($dependentTable);
            if ($type == 'object') {
                $type = get_class($dependentTable);
            }
            require_once 'Zend/Db/Table/Row/Exception.php';
            throw new  DbTableRowException ("Dependent table must be a  DbTableAbstract , but it is $type");
        }
        ...
        \for ($i = 0; $i < count($map[ DbTableAbstract ::COLUMNS]); ++$i) {
            $parentColumnName = $db->foldCase($map[ DbTableAbstract ::REF_COLUMNS][$i]);
            ...
            $dependentColumnName = $dependentDb->foldCase($map[ DbTableAbstract ::COLUMNS][$i]);
            $dependentColumn = $dependentDb->quoteIdentifier($dependentColumnName, true);
            $dependentInfo = $dependentTable->info();
            $type = $dependentInfo[ DbTableAbstract ::METADATA][$dependentColumnName]['DATA_TYPE'];
     ...
     *
     * @param string| DbTableAbstract  $parentTable
     * @param string                        OPTIONAL $ruleKey
     * @return  RowAbstract    Query result from $parentTable
     * @throws  DbTableRowException  If $parentTable is not a table or is not loadable.
     */
    public function findParentRow($parentTable, $ruleKey = null,  Select  $select = null)
    {
        ...
        if (! $parentTable instanceof  DbTableAbstract ) {
            $type = gettype($parentTable);
            if ($type == 'object') {
                $type = get_class($parentTable);
            }
            require_once 'Zend/Db/Table/Row/Exception.php';
            throw new  DbTableRowException ("Parent table must be a  DbTableAbstract , but it is $type");
        }
        ...
        \for ($i = 0; $i < count($map[ DbTableAbstract ::COLUMNS]); ++$i) {
            $dependentColumnName = $db->foldCase($map[ DbTableAbstract ::COLUMNS][$i]);
            ...
            $parentColumnName = $parentDb->foldCase($map[ DbTableAbstract ::REF_COLUMNS][$i]);
            $parentColumn = $parentDb->quoteIdentifier($parentColumnName, true);
            $parentInfo = $parentTable->info();
            $type = $parentInfo[ DbTableAbstract ::METADATA][$parentColumnName]['DATA_TYPE'];
    ...
    /**
     * @param  string| DbTableAbstract   $matchTable
     * @param  string| DbTableAbstract   $intersectionTable
     * @param  string                         OPTIONAL $primaryRefRule
     * @param  string                         OPTIONAL $matchRefRule
     * @return  Db _Table_Rowset_Abstract Query result from $matchTable
     * @throws  DbTableRowException  If $matchTable or $intersectionTable is not a table class or is not loadable.
     */
                                         ...
                                         $matchRefRule = null,  Select  $select = null)
    {
        ...
        if (is_string($intersectionTable)) {
            try {
                @ Loader ::loadClass($intersectionTable);
            } catch (\Zend_Exception $e) {
                require_once 'Zend/Db/Table/Row/Exception.php';
                throw new  DbTableRowException ($e->getMessage());
            }
        ...
        if (! $intersectionTable instanceof  DbTableAbstract ) {
            $type = gettype($intersectionTable);
            if ($type == 'object') {
                $type = get_class($intersectionTable);
            }
            require_once 'Zend/Db/Table/Row/Exception.php';
            throw new  DbTableRowException ("Intersection table must be a  DbTableAbstract , but it is $type");
        }

        if (is_string($matchTable)) {
            try {
                @ Loader ::loadClass($matchTable);
            } catch (\Zend_Exception $e) {
                require_once 'Zend/Db/Table/Row/Exception.php';
                throw new  DbTableRowException ($e->getMessage());
            }
        ...
        if (! $matchTable instanceof  DbTableAbstract ) {
            $type = gettype($matchTable);
            if ($type == 'object') {
                $type = get_class($matchTable);
            }
            require_once 'Zend/Db/Table/Row/Exception.php';
            throw new  DbTableRowException ("Match table must be a  DbTableAbstract , but it is $type");
        }
        ...
        \for ($i = 0; $i < count($matchMap[ DbTableAbstract ::COLUMNS]); ++$i) {
            $interCol = $interDb->quoteIdentifier('i' . '.' . $matchMap[ DbTableAbstract ::COLUMNS][$i], true);
            $matchCol = $interDb->quoteIdentifier('m' . '.' . $matchMap[ DbTableAbstract ::REF_COLUMNS][$i], true);
        ...
        \for ($i = 0; $i < count($callerMap[ DbTableAbstract ::COLUMNS]); ++$i) {
            $callerColumnName = $db->foldCase($callerMap[ DbTableAbstract ::REF_COLUMNS][$i]);
            $value = $this->_data[$callerColumnName];
            $interColumnName = $interDb->foldCase($callerMap[ DbTableAbstract ::COLUMNS][$i]);
            $interCol = $interDb->quoteIdentifier("i.$interColumnName", true);
            $matchColumnName = $interDb->foldCase($matchMap[ DbTableAbstract ::REF_COLUMNS][$i]);
            $interInfo = $intersectionTable->info();
            $type = $interInfo[ DbTableAbstract ::METADATA][$interColumnName]['DATA_TYPE'];
     ...
     * @return  RowAbstract | Db _Table_Rowset_Abstract
     * @throws  DbTableRowException  If an invalid method is called.
     */
    public function __call($method, array $args)
    {
        ...
        require_once 'Zend/Db/Table/Row/Exception.php';
        throw new  DbTableRowException ("Unrecognized method '$method()'");
    }
