...
use Zend\Db\Adapter\Pdo\Pgsql as Pgsql;
use Zend\Db\Adapter\AdapterAbstract as DbAdapterAbstract;
use Zend\Db\Table\Exception as DbTableException;
...
abstract class  TableAbstract 
{
    ...
    /**
     * Default  DbAdapterAbstract  object.
     *
    ...
    /**
     * The table column names derived from  DbAdapterAbstract ::describeTable().
     *
     * @var array
     */
    protected $_cols;
     ...
     * @return  TableAbstract  Provides a fluent interface
     */
    ...
    public function getReference($tableClassname, $ruleKey = null)
    {
        ...
        if ($ruleKey !== null) {
            if (!isset($refMap[$ruleKey])) {
                ...
                throw new  DbTableException ("No reference rule \"$ruleKey\" from table $thisClass to table $tableClassname");
            }
            if ($refMap[$ruleKey][self::REF_TABLE_CLASS] != $tableClassname) {
                require_once "Zend/Db/Table/Exception.php";
                throw new  DbTableException ("\Reference rule \"$ruleKey\" does not reference table $tableClassname");
            }
        ...
        require_once "Zend/Db/Table/Exception.php";
        throw new  DbTableException ("No reference from table $thisClass to table $tableClassname");
    }
    ...
    /**
     * Sets the default  DbAdapterAbstract  \for all \ Db _Table objects.
     *
    ...
    /**
     * Gets the default  DbAdapterAbstract  \for all \ Db _Table objects.
     *
     * @return  DbAdapterAbstract  or null
     */
    ...
    /**
     * Gets the  DbAdapterAbstract  \for this particular \ Db _Table object.
     *
    ...
    protected static final function _setupAdapter($db)
    {
        ...
        if (!$db instanceof  DbAdapterAbstract ) {
            require_once 'Zend/Db/Table/Exception.php';
            throw new  DbTableException ('Argument must be \of type  DbAdapterAbstract , or a Registry key where a  DbAdapterAbstract  object is stored');
        }
        return $db;
    }

    /**
     * Sets the default metadata cache \for information returned by  DbAdapterAbstract ::describeTable().
     *
    ...
    /**
     * Gets the default metadata cache \for information returned by  DbAdapterAbstract ::describeTable().
     *
     * @return  Core  or null
     */
    ...
    /**
     * Sets the metadata cache \for information returned by  DbAdapterAbstract ::describeTable().
     *
    ...
    /**
     * Gets the metadata cache \for information returned by  DbAdapterAbstract ::describeTable().
     *
     * @return  Core  or null
     */
    ...
    protected static final function _setupMetadataCache($metadataCache)
    {
        ...
        if (!$metadataCache instanceof  Core ) {
            require_once 'Zend/Db/Table/Exception.php';
            throw new  DbTableException ('Argument must be \of type  Core , or a Registry key where a  Core  object is stored');
        }
     ...
     * @return  Db _Table_Adapter_Abstract Provides a fluent interface
     */
    ...
    protected function _setupDatabaseAdapter()
    {
        if (! $this->_db) {
            ...
            if (!$this->_db instanceof  DbAdapterAbstract ) {
                require_once 'Zend/Db/Table/Exception.php';
                throw new  DbTableException ('No adapter found \for ' . get_class($this));
            }
        }
    }
    ...
    protected function _setupMetadata()
    {
        ...
        if (null === $this->_metadataCache || !($metadata = $this->_metadataCache->load($cacheId))) {
            ...
            if (null !== $this->_metadataCache && !$this->_metadataCache->save($metadata, $cacheId)) {
                /**
                 * @see  DbTableException 
                 */
                require_once 'Zend/Db/Table/Exception.php';
                throw new  DbTableException ('Failed saving metadata to metadataCache');
            }
        }
    ...
    protected function _setupPrimaryKey()
    {
        if (!$this->_primary) {
            ...
            if (empty($this->_primary)) {
                require_once 'Zend/Db/Table/Exception.php';
                throw new  DbTableException ('A table must have a primary key, but none was found');
            }
        ...
        if (! array_intersect((array) $this->_primary, $this->_cols) == (array) $this->_primary) {
            require_once 'Zend/Db/Table/Exception.php';
            throw new  DbTableException ("Primary key column(s) ("
    ...
    public function info($key = null)
    {
        ...
        if (!array_key_exists($key, $info)) {
            require_once 'Zend/Db/Table/Exception.php';
            throw new  DbTableException ('There is no table information \for the key "' . $key . '"');
        }
    ...
    {
        /**
         * \ Db _Table assumes that if you have a compound primary key
     ...
     * @return  Db _Table_Rowset_Abstract Row(s) matching the criteria.
     * @throws  DbTableException 
     */
    public function find()
    {
        ...
        if (count($args) < count($keyNames)) {
            require_once 'Zend/Db/Table/Exception.php';
            throw new  DbTableException ("Too few columns \for the primary key");
        }

        if (count($args) > count($keyNames)) {
            require_once 'Zend/Db/Table/Exception.php';
            throw new  DbTableException ("Too many columns \for the primary key");
        }
        ...
        foreach ($args as $keyPosition => $keyValues) {
            ...
            if ($numberTerms == 0) {
                ...
                require_once 'Zend/Db/Table/Exception.php';
                throw new  DbTableException ("Missing value(s) \for the primary key");
            }
     ...
     * Fetches all rows.
     *
     * Honors the  Db _Adapter fetch mode.
     *
     * @param string|array| Select  $where  OPTIONAL An SQL WHERE clause or  Select  object.
     * @param string|array                      $order  OPTIONAL An SQL ORDER clause.
     * @param int                               $count  OPTIONAL An SQL LIMIT count.
     * @param int                               $offset OPTIONAL An SQL LIMIT offset.
     * @return  Db _Table_Rowset_Abstract The row results per the  Db _Adapter fetch mode.
     */
    public function fetchAll($where = null, $order = null, $count = null, $offset = null)
    {
        ...
        @ Loader ::loadClass($this->_rowsetClass);
        return new $this->_rowsetClass($data);
    }

    /**
     * Fetches one row in an object \of type  Db _Table_Row_Abstract,
     * or returns Boolean false if no row matches the specified criteria.
     *
     * @param string|array| Select  $where  OPTIONAL An SQL WHERE clause or  Select  object.
     * @param string|array                      $order  OPTIONAL An SQL ORDER clause.
     * @return  Db _Table_Row_Abstract The row results per the
     *      Db _Adapter fetch mode, or null if no row found.
     */
