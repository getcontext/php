...
/**
 *  Cookie  is a class describing an HTTP cookie and all it's parameters.
 *
 *  Cookie  is a class describing an HTTP cookie and all it's parameters. The
...
class  Cookie 
{
    ...
    public function __construct($name, $value, $domain, $expires = null, $path = null, $secure = false)
    {
        if (preg_match("/[=,; \t\r\n\013\014]/", $name)) {
            ...
            throw new  HttpException ("Cookie name cannot contain these characters: =,; \\t\\r\\n\\013\\014 ({$name})");
        }

        if (! $this->name = (string) $name) {
            require_once 'Zend/Http/Exception.php';
            throw new  HttpException ('Cookies must have a name');
        }

        if (! $this->domain = (string) $domain) {
            require_once 'Zend/Http/Exception.php';
            throw new  HttpException ('Cookies must have a domain');
        }
    ...
    {
        if (is_string ($uri)) {
            $uri =  Http ::factory($uri);
        }

        // Make sure we have a valid  Http  object
        if (! ($uri->valid() && ($uri->getScheme() == 'http' || $uri->getScheme() =='https'))) {
            require_once 'Zend/Http/Exception.php';    
            throw new  HttpException ('Passed URI is not a valid HTTP or HTTPS URI');
        }
     ...
     * @param  Http |string $ref_uri \Reference URI \for default values (domain, path)
    ...
    public static function fromString($cookieStr, $ref_uri = null)
    {
        ...
        if ($name !== '') {
            return new  Cookie ($name, $value, $domain, $expires, $path, $secure);
        } else {
            return false;
        }
    }
