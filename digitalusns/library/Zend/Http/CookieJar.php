...
/**
 * A  CookieJar  object is designed to contain and maintain HTTP cookies, and should
 * be used along with \Zend\Http\Client in order to manage cookies across HTTP requests and
 * responses.
 *
 * The class contains an array \of  Cookie  objects. Cookies can be added to the jar
 ...
 * can be returned either in their native form (as  Cookie  objects) or as strings -
 ...
 * (by passing  CookieJar ::COOKIE_STRING_ARRAY) or one unified string \for all cookies
 * (by passing  CookieJar ::COOKIE_STRING_CONCAT).
 *
...
{
    /**
     * Return cookie(s) as a  Cookie  object
     *
     */
    ...
    /**
     * Add a cookie to the jar. Cookie should be passed either as a  Cookie  object
     * or as a string - in which case an object is created from the string.
     *
     * @param  Cookie |string $cookie
     * @param  Http |string    $ref_uri Optional reference URI (\for domain, path, secure)
     */
    public function addCookie($cookie, $ref_uri = null)
    {
        ...
        if ($cookie instanceof  Cookie ) {
            ...
            throw new  HttpException ('Supplient argument is not a valid cookie string or object');
        }
    }
     ...
     * @param  Http |string $ref_uri Requested URI
     */
    public function addCookiesFromResponse($response, $ref_uri)
    {
        if (! $response instanceof  Response ) {
            require_once 'Zend/Http/Exception.php';        
            throw new  HttpException ('$response is expected to be a Response object, ' .
     ...
     *
     * @param int $ret_as Whether to return cookies as objects \of  Cookie  or as strings
     * @return array|string
     */
     ...
     *
     * @param string| Http  $uri URI to check against (secure, domain, path)
    ...
    {
        if (is_string($uri)) $uri =  Uri ::factory($uri);
        if (! $uri instanceof  Http ) {
            require_once 'Zend/Http/Exception.php';    
            throw new  HttpException ("Invalid URI string or object passed");
        }
     ...
     *
     * @param  Http |string $uri The uri (domain and path) to match
    ...
    public function getCookie($uri, $cookie_name, $ret_as = self::COOKIE_OBJECT)
    {
        ...
        if (! $uri instanceof  Http ) {
            require_once 'Zend/Http/Exception.php';
            throw new  HttpException ('Invalid URI specified');
        }
        ...
        if (isset($this->cookies[$uri->getHost()][$path][$cookie_name])) {
            ...
            switch ($ret_as) {
                ...
                default:
                    require_once 'Zend/Http/Exception.php';
                    throw new  HttpException ("Invalid value passed \for \$ret_as: {$ret_as}");
                    break;
            }
        } else {
            return false;
     ...
     * @param  Http |string $uri The requested URI
     * @return  CookieJar 
