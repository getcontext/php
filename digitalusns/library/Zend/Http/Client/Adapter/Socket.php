...
/**
 * A sockets based (stream_socket_client) adapter class \for  Client . Can be used
...
use Zend\Http\Client\Adapter\Exception as HttpClientAdapterException;
...
class  Socket  implements  HttpClientAdapterInterface 
{
    /**
    ...
    public function connect($host, $port = 80, $secure = false)
    {
        ...
        if (! is_resource($this->socket) || ! $this->config['keepalive']) {
            ...
            if ($secure) {
                if ($this->config['sslcert'] !== null) {
                    if (! stream_context_set_option($context, 'ssl', 'local_cert',
                                                    $this->config['sslcert'])) {
                        require_once 'Zend/Http/Client/Adapter/Exception.php';
                        throw new  HttpClientAdapterException ('Unable to set sslcert option');
                    }
                }
                if ($this->config['sslpassphrase'] !== null) {
                    if (! stream_context_set_option($context, 'ssl', 'passphrase',
                        ...
                        require_once 'Zend/Http/Client/Adapter/Exception.php';
                        throw new  HttpClientAdapterException ('Unable to set sslpassphrase option');
                    }
                }
            }
            ...
            if (! stream_set_timeout($this->socket, (int) $this->config['timeout'])) {
                require_once 'Zend/Http/Client/Adapter/Exception.php';
                throw new  HttpClientAdapterException ('Unable to set the connection timeout');
            }
    ...
    public function write($method, $uri, $http_ver = '1.1', $headers = array(), $body = '')
    {
        ...
        if (! $this->socket) {
            require_once 'Zend/Http/Client/Adapter/Exception.php';
            throw new  HttpClientAdapterException ('Trying to write but we are not connected');
        }
        ...
        if ($this->connected_to[0] != $host || $this->connected_to[1] != $uri->getPort()) {
            require_once 'Zend/Http/Client/Adapter/Exception.php';
            throw new  HttpClientAdapterException ('Trying to write but we are connected to the wrong host');
        }
        ...
        if (! @fwrite($this->socket, $request)) {
            require_once 'Zend/Http/Client/Adapter/Exception.php';
            throw new  HttpClientAdapterException ('Error writing request to server');
        }
    ...
    public function read()
    {
        ...
        if ($statusCode == 304 || $statusCode == 204 || 
            $this->method ==  Client ::HEAD) return $response;
            ...
            if ($headers['transfer-encoding'] == 'chunked') {
                do {
                    ...
                    if (dechex($chunksize) != $hexchunksize) {
                        @fclose($this->socket);
                        require_once 'Zend/Http/Client/Adapter/Exception.php';
                        throw new  HttpClientAdapterException ('Invalid chunk size "' .
                ...
                throw new  HttpClientAdapterException ('Cannot handle "' .
