...
/**
 * @see  HttpClientAdapterInterface 
 */
...
/**
 *  Client  is an implemetation \of an HTTP client in PHP. The client
 * supports basic features like sending different HTTP requests and handling
 * redirections, as well as more advanced features like proxy settings, HTTP
 * authentication and cookie persistance (using a  CookieJar  object)
 *
...
use Zend\Http\Client\Adapter\AdapterInterface as HttpClientAdapterInterface;
use Zend\Http\Client\Exception as HttpClientException;
use Zend\Http\CookieJar as CookieJar;
use Zend\Http\Response as Response;
use Zend\Http\Cookie as Cookie;
...
class  Client 
{
    ...
    public function setUri($uri)
    {
        ...
        if (!$uri instanceof  Http ) {
            ...
            throw new  HttpClientException ('Passed parameter is not a valid HTTP URI.');
        }
    ...
    {
        if ($as_string && $this->uri instanceof  Http ) {
    ...
    public function setConfig($config = array())
    {
        if (! is_array($config)) {
            /** @see  HttpClientException  */
            require_once 'Zend/Http/Client/Exception.php';
            throw new  HttpClientException ('Expected array parameter, given ' . gettype($config));
        }
    ...
    public function setMethod($method = self::GET)
    {
        ...
        if (! preg_match($regex, $method)) {
            /** @see  HttpClientException  */
            require_once 'Zend/Http/Client/Exception.php';
            throw new  HttpClientException ("'{$method}' is not a valid HTTP request method.");
        }
    ...
    public function setHeaders($name, $value = null)
    {
        ...
        if (is_array($name)) {
            ...
            if ($this->config['strict'] && (! preg_match('/^[a-zA-Z0-9-]+$/', $name))) {
                /** @see  HttpClientException  */
                require_once 'Zend/Http/Client/Exception.php';
                throw new  HttpClientException ("{$name} is not a valid HTTP header name");
            }
     ...
     * <code>
     * $this->setAuth('shahar', 'secret',  Client ::AUTH_BASIC);
     * </code>
     *
    ...
    public function setAuth($user, $password = '', $type = self::AUTH_BASIC)
    {
        ...
        // Else, set up authentication
            ...
            if (! defined('self::AUTH_' . strtoupper($type))) {
                /** @see  HttpClientException  */
                require_once 'Zend/Http/Client/Exception.php';
                throw new  HttpClientException ("Invalid or not supported authentication type: '$type'");
            }
     ...
     * and responses.
     *
     * @param  CookieJar |boolean $cookiejar Existing cookiejar object, true to create a new one, false to disable
     * @return  Client 
     * @throws  HttpClientException 
     */
    public function setCookieJar($cookiejar = true)
    {
        if (! class_exists('\Zend\Http\CookieJar'))
        ...
        if ($cookiejar instanceof  CookieJar ) {
            ...
            /** @see  HttpClientException  */
            require_once 'Zend/Http/Client/Exception.php';
            throw new  HttpClientException ('Invalid parameter type passed as CookieJar');
        }
    ...
    {
        if (! class_exists('\Zend\Http\Cookie'))
        ...
        if (isset($this->cookiejar)) {
            if ($cookie instanceof  Cookie ) {
                ...
                $cookie =  Cookie ::fromString("{$cookie}={$value}", $this->uri);
                $this->cookiejar->addCookie($cookie);
            }
        } else {
            if ($cookie instanceof  Cookie ) {
            ...
            if (preg_match("/[=,; \t\r\n\013\014]/", $cookie)) {
                /** @see  HttpClientException  */
                require_once 'Zend/Http/Client/Exception.php';
                throw new  HttpClientException ("Cookie name cannot contain these characters: =,; \t\r\n\013\014 ({$cookie})");
            }
    ...
    public function setFileUpload($filename, $formname, $data = null, $ctype = null)
    {
        if ($data === null) {
            if (($data = @file_get_contents($filename)) === false) {
                /** @see  HttpClientException  */
                require_once 'Zend/Http/Client/Exception.php';
                throw new  HttpClientException ("Unable to read file '{$filename}' \for upload");
            }
     ...
     *
     * @return  Response  or null if none
     */
     ...
     *
     * @param  HttpClientAdapterInterface |string $adapter
     * @return null
     * @throws  HttpClientException 
     */
    public function setAdapter($adapter)
    {
        if (is_string($adapter)) {
            try {
                ...
                /** @see  HttpClientException  */
                require_once 'Zend/Http/Client/Exception.php';
                throw new  HttpClientException ("Unable to load adapter '$adapter': {$e->getMessage()}");
            }
        ...
        if (! $adapter instanceof  HttpClientAdapterInterface ) {
            /** @see  HttpClientException  */
            require_once 'Zend/Http/Client/Exception.php';
            throw new  HttpClientException ('Passed adapter is not a HTTP connection adapter');
        }
    ...
    {
        if (! $this->uri instanceof  Http ) {
            /** @see  HttpClientException  */
            require_once 'Zend/Http/Client/Exception.php';
            throw new  HttpClientException ('No valid URI has been passed to the client');
        }
        ...
        do {
            ...
            if (! $response) {
                /** @see  HttpClientException  */
                require_once 'Zend/Http/Client/Exception.php';
                throw new  HttpClientException ('Unable to read response, or response is empty');
            }
            ...
            switch($this->enctype) {
                ...
                default:
                    ...
                    throw new  HttpClientException ("Cannot handle content type '{$this->enctype}' automatically." .
                        " Please use  Client ::setRawData to send this kind \of content.");
                    break;
            }
        }
        ...
        switch ($type) {
            case self::AUTH_BASIC:
                ...
                if (strpos($user, ':') !== false) {
                    /** @see  HttpClientException  */
                    require_once 'Zend/Http/Client/Exception.php';
                    throw new  HttpClientException ("The user name cannot contain ':' in 'Basic' HTTP authentication");
                }
            ...
            default:
                /** @see  HttpClientException  */
                require_once 'Zend/Http/Client/Exception.php';
                throw new  HttpClientException ("Not a supported HTTP authentication type: '$type'");
        }
