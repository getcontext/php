...
/**
 *  Post  represents a post \of a user that can be edited
 *
 * @category   Zend
 * @package    Zend_Service
 * @subpackage Delicious
...
class  Post  extends  SimplePost 
{
    ...
    public function __construct( Delicious  $service, $values)
    {
        ...
        if (!is_array($values) || !isset($values['url']) || !isset($values['title'])) {
            ...
            throw new  ServiceDeliciousException ("Second argument must be array with at least 2 keys ('url' and"
                                                     . " 'title')");
        }
        ...
        if (isset($values['date']) && ! $values['date'] instanceof  Date ) {
             ...
             */
            require_once 'Zend/Service/Delicious/Exception.php';
            throw new  ServiceDeliciousException ("Date has to be an instance \of \Zend\Date");
        }
