...
use Zend\Http\Client\Adapter\Test as Test;
...
class  Audioscrobbler 
{
    ...
    protected function _getInfo($service, $params = null)
    {
        ...
        if (preg_match('/No such path/', $responseBody)) {
            ...
            throw new  HttpClientException ('Could not find: ' . $this->_client->getUri());
        } elseif (preg_match('/No user exists with this name/', $responseBody)) {
            /**
             * @see  HttpClientException 
             */
            require_once 'Zend/Http/Client/Exception.php';
            throw new  HttpClientException ('No user exists with this name');
        } elseif (!$response->isSuccessful()) {
            /**
             * @see  HttpClientException 
             */
            require_once 'Zend/Http/Client/Exception.php';
            throw new  HttpClientException ('The web service ' . $this->_client->getUri() . ' returned the following status code: ' . $response->getStatus());
        }
        ...
        if (!$simpleXmlElementResponse = simplexml_load_string($responseBody)) {
            ...
            $exception = new  ServiceException ('Response failed to load with SimpleXML');
