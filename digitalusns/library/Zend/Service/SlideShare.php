...
/**
 * The  SlideShare  component is used to interface with the
 ...
 * @package    Zend_Service
 * @subpackage SlideShare
 * @throws      ServiceSlideShareException 
 * @copyright  Copyright (c) 2005-2008 Zend Technologies USA Inc. (http://www.zend.com)
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */


use Zend\Service\SlideShare\SlideShow as SlideShow;
use Zend\Service\SlideShare\Exception as ServiceSlideShareException;
...
class  SlideShare 
{
    ...
    /**
     * Sets the  Client  object to use in requests. If not provided a default will
     * be used.
     *
     * @param  Client  $client The HTTP client instance to use
     * @return  SlideShare 
     */
    ...
    /**
     * Returns the instance \of the  Client  which will be used. Creates an instance
     * \of  Client  if no previous client was set.
     *
     * @return  Client  The HTTP client which will be used
     */
    ...
    /**
     * Sets the  Cache  object to use to cache the results \of API queries
     *
     * @param  Core  $cacheobject The  Cache  object used
     * @return  SlideShare 
     */
    public function setCacheObject( Core  $cacheobject)
    {
    ...
    /**
     * Gets the  Cache  object which will be used to cache API queries. If no cache object
     * was previously set the the default will be used (Filesystem caching in /tmp with a life
     * time \of 43200 seconds)
     *
     * @return  Core  The object used in caching
     */
    public function getCacheObject()
    {

        if(!($this->_cacheobject instanceof  Core )) {
            $cache =  Cache ::factory('Core', 'File', array('lifetime' => 43200,
     ...
     *
     * @param  SlideShow  $ss The slide show object representing the slide show to upload
     * @param boolean $make_src_public Determines if the the slide show's source file is public or not upon upload
     * @return  SlideShow  The passed Slide show object, with the new assigned ID provided
     */
    public function uploadSlideShow( SlideShow  $ss, $make_src_public = true)
    {
        ...
        if(!file_exists($filename) || !is_readable($filename)) {
            throw new  ServiceSlideShareException ("Specified Slideshow \for upload not found or unreadable");
        }
        ...
        try {
            ...
            throw new  ServiceSlideShareException ("Service Request Failed: {$e->getMessage()}");
        }
        ...
        if($sxe->getName() == "SlideShareServiceError") {
            ...
            throw new  ServiceSlideShareException (trim($error_str), $code);
        }

        if(!$sxe->getName() == "SlideShowUploaded") {
            throw new  ServiceSlideShareException ("Unknown XML Respons Received");
        }
    ...
    public function getSlideShow($ss_id)
    {
        ...
        if(!$retval = $cache->load($cache_key)) {
            ...
            if(!$sxe->getName() == 'Slideshows') {
                throw new  ServiceSlideShareException ('Unknown XML Repsonse Received');
            }
    ...
    protected function _getSlideShowsByType($key, $value, $offset = null, $limit = null)
    {
        ...
        switch($key) {
            ...
            case 'tag':
                ...
                break;
            default:
                throw new  ServiceSlideShareException ("Invalid SlideShare Query");
        }
        ...
        if(!$retval = $cache->load($cache_key)) {
            ...
            if(!$sxe->getName() == $responseTag) {
                throw new  ServiceSlideShareException ('Unknown or Invalid XML Repsonse Received');
            }
    ...
    protected function _slideShowNodeToObject(SimpleXMLElement $node)
    {
        ...
        throw new  ServiceSlideShareException ("Was not provided the expected XML Node \for processing");
    }
}
