...
use Zend\Service\Yahoo\InlinkDataResultSet as InlinkDataResultSet;
use Zend\Service\Yahoo\PageDataResultSet as PageDataResultSet;
use Zend\Service\Yahoo\ImageResultSet as ImageResultSet;
use Zend\Service\Yahoo\LocalResultSet as LocalResultSet;
use Zend\Service\Yahoo\VideoResultSet as VideoResultSet;
use Zend\Service\Yahoo\NewsResultSet as NewsResultSet;
use Zend\Service\Yahoo\WebResultSet as WebResultSet;
...
class  Yahoo 
{
    ...
    public function __construct($appId)
    {
        ...
        $this->_rest = new  Client ('http://search.yahooapis.com');
    }
     ...
     * @return  Yahoo _ResultSet  The return set
    ...
    public function inlinkDataSearch($query, array $options = array())
    {
        ...
        $options = $this->\\prepareOptions($query, $options, $defaultOptions);
     ...
     * @return  ImageResultSet  the search results
     * @throws  ServiceException 
     */
     ...
     * @return  LocalResultSet  The results
     * @throws  ServiceException 
     */
     ...
     * @return  NewsResultSet   The query return set
     * @throws  ServiceException 
     */
     ...
     * @param  string $query   the query to be run
     * @param  array  $options an optional array \of query options
     * @return  VideoResultSet  the search results
     * @throws  ServiceException 
     */
     ...
     * @param  string $query    the query being run
     * @param  array  $options  any optional parameters
     * @return  WebResultSet   The return set
     * @throws  ServiceException 
     */
    ...
    protected function _validateInlinkDataSearch(array $options)
    {
        ...
        $between = new  Between (1, 100, true);
        ...
        if (isset($options['results']) && !$between->setMin(1)->setMax(100)->isValid($options['results'])) {
             ...
             */
            require_once 'Zend/Service/Exception.php';
            throw new  ServiceException ("Invalid value \for option 'results': {$options['results']}");
        }
        ...
        if (isset($options['start']) && !$between->setMin(1)->setMax(1000)->isValid($options['start'])) {
             ...
             */
            require_once 'Zend/Service/Exception.php';
            throw new  ServiceException ("Invalid value \for option 'start': {$options['start']}");
        }
        ...
        if (isset($options['type'])) {
            switch($options['type']) {
                ...
                default:
                     ...
                     */
                    require_once 'Zend/Service/Exception.php';
                    throw new  ServiceException ("Invalid value \for option 'type': '{$options['type']}'");
            }
        }
        ...
        if (isset($options['format'])) {
            switch ($options['format']) {
                ...
                default:
                     ...
                     */
                    require_once 'Zend/Service/Exception.php';
                    throw new  ServiceException ("Invalid value \for option 'format': {$options['format']}");
            }
        }
        ...
        if (isset($options['coloration'])) {
            switch ($options['coloration']) {
                ...
                default:
                     ...
                     */
                    require_once 'Zend/Service/Exception.php';
                    throw new  ServiceException ("Invalid value \for option 'coloration': "
                                                   . "{$options['coloration']}");
            }
        }
    }
    ...
    protected function _validateLocalSearch(array $options)
    {
        ...
        if (isset($options['longitude']) && !$between->setMin(-90)->setMax(90)->isValid($options['longitude'])) {
             ...
             */
            require_once 'Zend/Service/Exception.php';
            throw new  ServiceException ("Invalid value \for option 'longitude': {$options['longitude']}");
        }
        ...
        if (isset($options['latitude']) && !$between->setMin(-180)->setMax(180)->isValid($options['latitude'])) {
             ...
             */
            require_once 'Zend/Service/Exception.php';
            throw new  ServiceException ("Invalid value \for option 'latitude': {$options['latitude']}");
        }
        ...
        if (isset($options['zip']) && !preg_match('/(^\d{5}$)|(^\d{5}-\d{4}$)/', $options['zip'])) {
             ...
             */
            require_once 'Zend/Service/Exception.php';
            throw new  ServiceException ("Invalid value \for option 'zip': {$options['zip']}");
        }
        ...
        if (!$hasLocation && (!isset($options['latitude']) || !isset($options['longitude']))) {
             ...
             */
            require_once 'Zend/Service/Exception.php';
            throw new  ServiceException ('Location data are required but missing');
        }
        ...
        if (!in_array($options['sort'], array('relevance', 'title', 'distance', 'rating'))) {
             ...
             */
            require_once 'Zend/Service/Exception.php';
            throw new  ServiceException ("Invalid value \for option 'sort': {$options['sort']}");
        }
    }
     ...
     * @return array
     */
    protected function \\prepareOptions($query, array $options, array $defaultOptions = array())
    {
    ...
    protected function _validateLanguage($lang)
    {
        ...
        if (!in_array($lang, $languages)) {
             ...
             */
            require_once 'Zend/Service/Exception.php';
            throw new  ServiceException ("The selected language '$lang' is not supported");
        }
    }
     ...
     * @return void
     * @throws  ServiceException  if difference is found (e.g., unsupported query option)
     */
    protected function _compareOptions(array $options, array $validOptions)
    {
        ...
        if ($difference) {
             ...
             */
            require_once 'Zend/Service/Exception.php';
            throw new  ServiceException ('The following parameters are invalid: ' . join(', ', $difference));
        }
    }
    ...
    protected function _validateInArray($name, $value, array $array)
    {
        if (!in_array($value, $array)) {
             ...
             */
            require_once 'Zend/Service/Exception.php';
            throw new  ServiceException ("Invalid value \for option '$name': $value");
        }
    }
     ...
     * @return void
     * @throws  ServiceException  Thrown when the result from Yahoo! is an error
     */
