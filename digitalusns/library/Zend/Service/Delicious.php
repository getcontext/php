...
/**
 *  Delicious  is a concrete implementation \of the del.icio.us web service
 *
 * @category   Zend
 * @package    Zend_Service
 * @subpackage Delicious
...
use Zend\Service\Delicious\Exception as ServiceDeliciousException;
use Zend\Service\Delicious\PostList as PostList;
use Zend\Service\Delicious\Post as Post;
use Zend\Json\Decoder as Decoder;
...
class  Delicious 
{
     ...
     *
     * @throws  ServiceDeliciousException 
     * @return  Date 
     */
    public function getLastUpdate()
    {
        ...
        if ($rootNode && $rootNode->nodeName == 'update') {
            /**
             * @todo replace strtotime() with  Date  equivalent
             */
            return new  Date (strtotime($rootNode->getAttribute('time')));
        } else {
            /**
             * @see  ServiceDeliciousException 
             */
            require_once 'Zend/Service/Delicious/Exception.php';
            throw new  ServiceDeliciousException ('del.icio.us web service has returned something odd!');
        }
    }
     ...
     *
     * @param  string    $tag Optional filtering by tag
     * @param   Date  $dt  Optional filtering by date
    ...
    public function makeRequest($path, array $parms = array(), $type = 'xml')
    {
        ...
        switch ($type) {
            ...
            default:
                 ...
                 */
                require_once 'Zend/Service/Delicious/Exception.php';
                throw new  ServiceDeliciousException ('Unknown request type');
        }
        ...
        if (!$response->isSuccessful()) {
             ...
             */
            require_once 'Zend/Service/Delicious/Exception.php';
            throw new  ServiceDeliciousException ("Http client reported an error: '{$response->getMessage()}'");
        }
        ...
        switch ($type) {
            case 'xml':
                ...
                if (!@$dom->loadXML($responseBody)) {
                     ...
                     */
                    require_once 'Zend/Service/Delicious/Exception.php';
                    throw new  ServiceDeliciousException ('XML Error');
                }
    ...
    /**
     * Constructs  PostList  from XML response
     *
     * @param   DOMDocument $response
     * @return   PostList 
     * @throws   ServiceDeliciousException 
    ...
    private static function _evalXmlResult(DOMDocument $response)
    {
        ...
        if ($rootNode && $rootNode->nodeName == 'result') {
            ...
            if ($strResponse != 'done' && $strResponse != 'ok') {
                 ...
                 */
                require_once 'Zend/Service/Delicious/Exception.php';
                throw new  ServiceDeliciousException ("del.icio.us web service: '{$strResponse}'");
            }
        } else {
            /**
             * @see  ServiceDeliciousException 
