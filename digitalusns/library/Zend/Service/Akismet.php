...
class  Akismet  extends  ServiceAbstract 
{
    /**
     * Akismet API key
     * @var string
    ...
    public function __construct($apiKey, $blog)
    {
        $this->setBlogUrl($blog)
             ->setApiKey($apiKey)
             ->setUserAgent('Zend Framework/' .  Version ::VERSION . ' | Akismet/1.11');
    }
     ...
     * @return  Akismet 
     * @throws  ServiceException  if invalid URL provided
     */
    public function setBlogUrl($blogUrl)
    {
        require_once 'Zend/Uri.php';
        if (! Uri ::check($blogUrl)) {
            require_once 'Zend/Service/Exception.php';
            throw new  ServiceException ('Invalid url provided \for blog');
        }
     ...
     * @param int $port
     * @return  Akismet 
     * @throws  ServiceException  if non-integer value provided
     */
    public function setPort($port)
    {
        if (!is_int($port)) {
            require_once 'Zend/Service/Exception.php';
            throw new  ServiceException ('Invalid port');
        }
     ...
     * @return  Akismet 
     * @throws  ServiceException  with invalid user agent string
     */
    public function setUserAgent($userAgent)
    {
        ...
        {
            require_once 'Zend/Service/Exception.php';
            throw new  ServiceException ('Invalid \User Agent string; must be \of format "Application name/version | Akismet/version"');
        }
     ...
     * @throws  ServiceException  if missing user_ip or user_agent fields
     */
    protected function _makeApiCall($path, $params)
    {
        if (empty($params['user_ip']) || empty($params['user_agent'])) {
            require_once 'Zend/Service/Exception.php';
            throw new  ServiceException ('Missing required Akismet fields (user_ip and user_agent are required)');
        }
     ...
     * @param array $params
     * @return boolean
     * @throws  ServiceException  with invalid API key
     */
