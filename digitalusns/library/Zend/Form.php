...
use Zend\Loader\PluginLoader\PluginLoaderInterface as LoaderPluginLoaderInterface;
...
use Zend\Translate as Translate;
...
class  Form  implements \Iterator, \Countable,  ValidateInterface 
{
    /**#@+
     ...
     * 
     * @param   LoaderPluginLoaderInterface  $loader 
     * @param  string $type 'decorator' or 'element'
     * @return  Form 
     * @throws  FormException  on invalid type
     */
    public function setPluginLoader( LoaderPluginLoaderInterface  $loader, $type = null)
    {
        ...
        switch ($type) {
            ...
            default:
                ...
                throw new  FormException (sprintf('Invalid type "%s" provided to setPluginLoader()', $type));
        }
    }
     ...
     * 
     * @param  string $type 
     * @return  LoaderPluginLoaderInterface 
     */
    public function getPluginLoader($type = null)
    {
        ...
        if (!isset($this->_loaders[$type])) {
            switch ($type) {
                ...
                default:
                    require_once 'Zend/Form/Exception.php';
                    throw new  FormException (sprintf('Invalid type "%s" provided to getPluginLoader()', $type));
            }
     ...
     * {@link  Element ::addPrefixPath()} is called on each element.
     * 
     * @param  string $path 
     * @return  Form 
     * @throws  FormException  \for invalid type
     */
    ...
    public function setMethod($method)
    {
        ...
        if (!in_array($method, $this->_methods)) {
            require_once 'Zend/Form/Exception.php';
            throw new  FormException (sprintf('"%s" is an invalid form method', $method));
        }
    ...
    public function setName($name)
    {
        ...
        if (('0' !== $name) && empty($name)) {
            require_once 'Zend/Form/Exception.php';
            throw new  FormException ('Invalid name provided; must contain only valid variable characters and be non-empty');
        }
     ...
     *  Element . If a string element type is provided, $name must be 
     * provided, and $options may be optionally provided \for configuring the 
     * element.
     *
     * If a  Element  is provided, $name may be optionally provided, 
    ...
    public function addElement($element, $name = null, $options = null)
    {
        if (is_string($element)) {
            if (null === $name) {
                require_once 'Zend/Form/Exception.php';
                throw new  FormException ('Elements specified by string must have an accompanying name');
            }
    ...
    public function createElement($type, $name, $options = null)
    {
        if (!is_string($type)) {
            require_once 'Zend/Form/Exception.php';
            throw new  FormException ('Element type must be a string indicating type');
        }

        if (!is_string($name)) {
            require_once 'Zend/Form/Exception.php';
            throw new  FormException ('Element name must be a string');
        }
     ...
     * @param  int $order 
     * @return  Form 
     * @throws  FormException  if no valid elements provided
     */
    public function addDisplayGroup(array $elements, $name, $options = null)
    {
        ...
        if (empty($group)) {
            require_once 'Zend/Form/Exception.php';
            throw new  FormException ('No valid elements specified \for display group');
        }
    ...
    protected function _addDisplayGroupObject( DisplayGroup  $group, $name = null)
    {
        if (null === $name) {
            ...
            if (empty($name)) {
                require_once 'Zend/Form/Exception.php';
                throw new  FormException ('Invalid display group added; requires name');
            }
        }
    ...
    public function isValid($data)
    {
        if (!is_array($data)) {
            require_once 'Zend/Form/Exception.php';
            throw new  FormException (__CLASS__ . '::' . __METHOD__ . ' expects an array');
        }
     ...
     * 
     * @param  string| FormDecoratorInterface  $decorator 
     * @param  array| Config  $options Options with which to initialize decorator
     * @return  Form 
     */
    public function addDecorator($decorator, $options = null)
    {
        if ($decorator instanceof  FormDecoratorInterface ) {
            ...
            if (is_numeric($name)) {
                require_once 'Zend/Form/Exception.php';
                throw new  FormException ('Invalid alias provided to addDecorator; must be alphanumeric string');
            }
            if (is_string($spec)) {
                $decorator = array(
                    'decorator' => $spec,
            ...
            } elseif ($spec instanceof  FormDecoratorInterface ) {
                $decorator = $spec;
            }
        } else {
            require_once 'Zend/Form/Exception.php';
            throw new  FormException ('Invalid decorator provided to addDecorator; must be string or  Form _Decorator_Interface');
        }
    ...
    public function addDecorators(array $decorators)
    {
        foreach ($decorators as $decoratorInfo) {
            if (is_string($decoratorInfo)) {
            ...
            } elseif ($decoratorInfo instanceof  FormDecoratorInterface ) {
                $this->addDecorator($decoratorInfo);
                ...
                require_once 'Zend/Form/Exception.php';
                throw new  FormException ('Invalid decorator passed to addDecorators()');
            }
        }
     ...
     * 
     * @param  string $name 
     * @return false|\ Form _Decorator_Abstract
     */
     ...
     * 
     * @param   Translate | Adapter |null $translator 
     * @return  Form 
     */
    public function setTranslator($translator = null)
    {
        ...
        } elseif ($translator instanceof  Adapter ) {
            $this->_translator = $translator;
        } elseif ($translator instanceof  Translate ) {
            $this->_translator = $translator->getAdapter();
        } else {
            require_once 'Zend/Form/Exception.php';
            throw new  FormException ('Invalid translator specified');
        }
    ...
    public static function getDefaultTranslator()
    {
        if (null === self::$_translatorDefault) {
            ...
            if ( Registry ::isRegistered('\Zend\Translate')) {
                $translator =  Registry ::get('\Zend\Translate');
                if ($translator instanceof  Adapter ) {
                    return $translator;
                } elseif ($translator instanceof  Translate ) {
     ...
     * 
     * @param  string $name 
     * @param   Element | Form  $value 
     * @return void
     * @throws  FormException  \for invalid $value
     */
    public function __set($name, $value)
    {
        ...
        throw new  FormException ('Only form elements and groups may be overloaded; variable \of type "' . $type . '" provided');
    }
     ...
     * 
     * @return  Element | DisplayGroup | Form 
     */
    public function current()
    {
        ...
        if (isset($this->_elements[$key])) {
            ...
            require_once 'Zend/Form/Exception.php';
            throw new  FormException (sprintf('Corruption detected in form; invalid key ("%s") found in internal iterator', (string) $key));
        }
    }
