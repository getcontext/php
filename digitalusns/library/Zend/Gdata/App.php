 ...
 * other components \of  App  are designed to work independently from
 * other  Gdata  components in order to interact with generic APP services.
 *
 * @category   Zend
 * @package     Gdata 
 * @copyright  Copyright (c) 2005-2008 Zend Technologies USA Inc. (http://www.zend.com)
...
class  App 
{
     ...
     *
     * @param string $name The name \of the package (eg  App )
     * @return void
     */
    ...
    /**
     * Get the  Client  object used \for communication
     *
     * @return  Client 
     */
    ...
    public function setHttpClient($client, $applicationId = 'MyCompany-MyApp-1.0')
    {
        ...
        $userAgent = $applicationId . ' Zend_Framework_Gdata/' .  Version ::VERSION;
        ...
        $this->_httpClient = $client;
         Gdata ::setStaticHttpClient($client);
        return $this;
    }
     ...
     * @return void
     */
    public static function setStaticHttpClient( Client  $httpClient)
    {
    ...
    {
        if (!self::$_staticHttpClient instanceof  Client ) {
            $client = new  Client ();
     ...
     *                       request
     * @param mixed $data The  Entry  or XML \for the  
     *                    body \of the request
    ...
    public function performHttpRequest($method, $url, $headers = null, $body = null, $contentType = null, $remainingRedirects = null)
    {
        ...
        if (($method == 'POST' || $method == 'PUT') && $body === null && $headers['x-http-method-override'] != 'DELETE') {
                ...
                throw new  GdataAppInvalidArgumentException (
                        ...
                        'string or a child \of  App _Entry');
        }
        if ($url === null) {
            require_once 'Zend/Gdata/App/InvalidArgumentException.php';
            throw new  GdataAppInvalidArgumentException ('You must specify an URI to which to post.');
        }
     ...
     * @return  App _Feed
     */
    public static function importString($string, $className='\Zend\Gdata\App\Feed')
    {
            ...
            $className='\Zend\Gdata\App\Feed', $useIncludePath = false)
    {
        ...
        if ($feed === false) {
            require_once 'Zend/Gdata/App/Exception.php';
            throw new  GdataAppException ("File could not be loaded: $php_errormsg");
        }
     ...
     * @return  Entry  The entry returned by the service after insertion.
     */
     ...
     * Update an entry
     *
     * @param mixed $data  Entry  or XML (w/ID and link rel='edit')
     * @return  Entry  The entry returned from the server
     * @throws  GdataAppException 
     */
     ...
     *  Gdata _Calendar_Extension_Color, a developer simply needs to do
    ...
    public function __call($method, $args)
    {
        if (preg_match('/^new(\w+)/', $method, $matches)) {
            ...
            require_once 'Zend/Gdata/App/Exception.php';
            throw new  GdataAppException ("No such method ${method}");
        }
    }
     ...
     *  Client _Adapter used \for performing the requests.
