...
class  Transform 
{
    ...
    protected function _findClassbyURI($uri)
    {
        switch($uri) {
            ...
            default:
                throw new  InfoCardXmlSecurityException ("Unknown or Unsupported Transformation Requested");
        }
    }
    ...
    public function applyTransforms($strXmlDocument)
    {
        foreach($this->_transformList as $transform) {
             Loader ::loadClass($transform['class']);
            ...
            if(!($transformer instanceof  InfoCardXmlSecurityTransformInterface )) {
                throw new  InfoCardXmlSecurityException ("Transforms must implement the Transform Interface");
            }
