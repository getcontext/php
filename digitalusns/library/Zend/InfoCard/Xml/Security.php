...
use Zend\InfoCard\Xml\Security\Exception as InfoCardXmlSecurityException;
use Zend\InfoCard\Xml\Security\Transform as Transform;
...
class  Security 
{
     ...
     * @throws  InfoCardXmlSecurityException 
     */
    static public function validateXMLSignature($strXMLInput)
    {
        if(!extension_loaded('openssl')) {
            throw new  InfoCardXmlSecurityException ("You must have the openssl extension installed to use this class");
        }

        $sxe = simplexml_load_string($strXMLInput);

        if(!isset($sxe->Signature)) {
            throw new  InfoCardXmlSecurityException ("Could not identify XML Signature element");
        }

        if(!isset($sxe->Signature->SignedInfo)) {
            throw new  InfoCardXmlSecurityException ("Signature is missing a SignedInfo block");
        }

        if(!isset($sxe->Signature->SignatureValue)) {
            throw new  InfoCardXmlSecurityException ("Signature is missing a SignatureValue block");
        }

        if(!isset($sxe->Signature->KeyInfo)) {
            throw new  InfoCardXmlSecurityException ("Signature is missing a KeyInfo block");
        }

        if(!isset($sxe->Signature->KeyInfo->KeyValue)) {
            throw new  InfoCardXmlSecurityException ("Signature is missing a KeyValue block");
        }
        ...
        switch((string)$sxe->Signature->SignedInfo->CanonicalizationMethod['Algorithm']) {
            case self::CANONICAL_METHOD_C14N_EXC:
                ...
                break;
            default:
                throw new  InfoCardXmlSecurityException ("Unknown or unsupported CanonicalizationMethod Requested");
        }
        ...
        switch((string)$sxe->Signature->SignedInfo->SignatureMethod['Algorithm']) {
            case self::SIGNATURE_METHOD_SHA1:
                ...
                break;
            default:
                throw new  InfoCardXmlSecurityException ("Unknown or unsupported SignatureMethod Requested");
        }
        ...
        switch((string)$sxe->Signature->SignedInfo->\Reference->DigestMethod['Algorithm']) {
            case self::DIGEST_METHOD_SHA1:
                ...
                break;
            default:
                throw new  InfoCardXmlSecurityException ("Unknown or unsupported DigestMethod Requested");
        }
        ...
        if($transformed_xml_binhash != $dValue) {
            throw new  InfoCardXmlSecurityException ("Locally Transformed XML does not match XML Document. Cannot Verify Signature");
        }
        ...
        switch(true) {
            case isset($sxe->Signature->KeyInfo->KeyValue->X509Certificate):
                ...
                if(!$public_key) {
                    throw new  InfoCardXmlSecurityException ("Unable to extract and prcoess X509 Certificate from KeyValue");
                }
            ...
            case isset($sxe->Signature->KeyInfo->KeyValue->RSAKeyValue):
                ...
                if(!isset($sxe->Signature->KeyInfo->KeyValue->RSAKeyValue->Modulus) ||
                   !isset($sxe->Signature->KeyInfo->KeyValue->RSAKeyValue->Exponent)) {
                    throw new  InfoCardXmlSecurityException ("RSA Key Value not in Modulus/Exponent form");
                }
                ...
                break;
            default:
                throw new  InfoCardXmlSecurityException ("Unable to determine or unsupported representation \of the KeyValue block");
        }
    ...
    static protected function _encodeValue($data, $type)
    {
        ...
        switch(true) {
            ...
            default:
                throw new  InfoCardXmlSecurityException ("Could not encode value");
        }

        throw new  InfoCardXmlSecurityException ("Invalid code path");
    }
}
