...
class  SecurityTokenReference  extends  Element 
{
    /**
    ...
    static public function getInstance($xmlData)
    {
        ...
        if($sxe->getName() != "SecurityTokenReference") {
            throw new  InfoCardXmlException ("Invalid XML Block provided \for SecurityTokenReference");
        }
    ...
    protected function _getKeyIdentifier()
    {
        ...
        if(!($keyident instanceof  Element )) {
            throw new  InfoCardXmlException ("Failed to retrieve Key Identifier");
        }
    ...
    public function getKeyThumbprintType()
    {
        ...
        if(!$dom->hasAttribute('ValueType')) {
            throw new  InfoCardXmlException ("Key Identifier did not provide a type \for the value");
        }
    ...
    public function getKeyThumbprintEncodingType()
    {
        ...
        if(!$dom->hasAttribute('EncodingType')) {
            throw new  InfoCardXmlException ("Unable to determine the encoding type \for the key identifier");
        }
    ...
    public function getKeyReference($decode = true)
    {
        ...
        if(empty($encoded)) {
            throw new  InfoCardXmlException ("Could not find the Key \Reference Encoded Value");
        }
        ...
        if($decode) {
            ...
            switch($this->getKeyThumbprintEncodingType()) {
                case self::ENCODING_BASE64BIN:
                    ...
                    break;
                default:
                    throw new  InfoCardXmlException ("Unknown Key \Reference Encoding Type: {$this->getKeyThumbprintEncodingType()}");
            }

            if(!$decoded || empty($decoded)) {
                throw new  InfoCardXmlException ("Failed to decode key reference");
            }
