...
use Zend\Server\Reflection\Prototype as Prototype;
use Zend\Server\Reflection\Node as Node;
...
abstract class  FunctionAbstract 
{
    ...
    public function __construct(Reflector $r, $namespace = null, $argv = array())
    {
        ...
        if ((!$r instanceof ReflectionFunction)
            ...
            throw new  ServerReflectionException ('Invalid reflection class');
        }
        $this->_reflection = $r;
    ...
    protected function _buildSignatures($return, $returnDesc, $paramTypes, $paramDesc)
    {
        ...
        foreach ($signatures as $signature) {
            ...
            foreach ($signature as $key => $type) {
                $param = new  Parameter ($params[$key], $type, $this->_paramDesc[$key]);
    ...
    protected function _reflect()
    {
        ...
        if (count($paramTypesTmp) != $paramCount) {
            throw new  ServerReflectionException (
    ...
    public function __call($method, $args)
    {
        ...
        throw new  ServerReflectionException ('Invalid reflection method ("' .$method. '")');
    }
    ...
    public function setNamespace($namespace)
    {
        ...
        if (!is_string($namespace) || !preg_match('/[a-z0-9_\.]+/i', $namespace)) {
            throw new  ServerReflectionException ('Invalid namespace');
        }
    ...
    {
        if (!is_string($string)) {
            throw new  ServerReflectionException ('Invalid description');
        }
     ...
     * {@link  Prototype  \Zend\Server\Reflection\Prototypes}
     *
     * @return array
     */
