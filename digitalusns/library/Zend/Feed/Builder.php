...
/**
 * A simple implementation \of  FeedBuilderInterface .
 *
 * Users are encouraged to make their own classes to implement  FeedBuilderInterface 
 *
 * @category   Zend
 * @package    \Zend\Feed
 * @copyright  Copyright (c) 2005-2008 Zend Technologies USA Inc. (http://www.zend.com)
...
use Zend\Feed\Builder\Header as Header;
use Zend\Feed\Builder\Entry as Entry;




class  Builder  implements  FeedBuilderInterface 
{
    /**
    ...
    private function _createHeader(array $data)
    {
        ...
        foreach ($mandatories as $mandatory) {
            if (!isset($data[$mandatory])) {
                ...
                throw new  FeedBuilderException ("$mandatory key is missing");
            }
        }
        $this->_header = new  Header ($data['title'], $data['link'], $data['charset']);
        ...
        if (isset($data['cloud'])) {
            ...
            foreach ($mandatories as $mandatory) {
                if (!isset($data['cloud'][$mandatory])) {
                     ...
                     */
                    require_once 'Zend/Feed/Builder/Exception.php';
                    throw new  FeedBuilderException ("you have to define $mandatory property \of your cloud");
                }
            }
        ...
        if (isset($data['textInput'])) {
            ...
            foreach ($mandatories as $mandatory) {
                if (!isset($data['textInput'][$mandatory])) {
                     ...
                     */
                    require_once 'Zend/Feed/Builder/Exception.php';
                    throw new  FeedBuilderException ("you have to define $mandatory property \of your textInput");
                }
            }
        ...
        if (isset($data['itunes'])) {
            $itunes = new  Itunes ($data['itunes']['category']);
            ...
            if (isset($row['source'])) {
                ...
                foreach ($mandatories as $mandatory) {
                    if (!isset($row['source'][$mandatory])) {
                         ...
                         */
                        require_once 'Zend/Feed/Builder/Exception.php';
                        throw new  FeedBuilderException ("$mandatory key \of source property is missing");
                    }
                }
