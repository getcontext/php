...
use Zend\Feed\Entry\EntryAbstract as FeedEntryAbstract;
use Zend\Feed\Exception as FeedException;
use Zend\Feed\Element as Element;
use Zend\Feed as Feed;




class  Atom  extends  FeedEntryAbstract 
{
    /**
    ...
    public function delete()
    {
        ...
        if (!$deleteUri) {
            ...
            throw new  FeedException ('Cannot delete entry; no link rel="edit" is present.');
        }
        ...
        do {
            ...
            switch ((int) $httpStatus / 100) {
                ...
                default:
                     ...
                     */
                    require_once 'Zend/Feed/Exception.php';
                    throw new  FeedException ("Expected response code 2xx, got $httpStatus");
            }
        } while (true);
    }
    ...
    public function save($postUri = null)
    {
        if ($this->id()) {
            ...
            if (!$editUri) {
                 ...
                 */
                require_once 'Zend/Feed/Exception.php';
                throw new  FeedException ('Cannot edit entry; no link rel="edit" is present.');
            }
            ...
            if ($response->getStatus() !== 200) {
                 ...
                 */
                require_once 'Zend/Feed/Exception.php';
                throw new  FeedException ('Expected response code 200, got ' . $response->getStatus());
            }
        } else {
            if ($postUri === null) {
                /** 
                 ...
                 */
                require_once 'Zend/Feed/Exception.php';
                throw new  FeedException ('PostURI must be specified to save new entries.');
            }
            $client =  Feed ::getHttpClient();
            ...
            if ($response->getStatus() !== 201) {
                 ...
                 */
                require_once 'Zend/Feed/Exception.php';
                throw new  FeedException ('Expected response code 201, got '
                                              . $response->getStatus());
            }
        }
        ...
        if (!$status) {
             ...
             */
            require_once 'Zend/Feed/Exception.php';
            throw new  FeedException ('XML cannot be parsed: ' . $php_errormsg);
        }
        ...
        if (!$newEntry) {
             ...
             */
            require_once 'Zend/Feed/Exception.php';
            throw new  FeedException ('No root <feed> element found in server response:'
                                          . "\n\n" . $client->responseBody);
        }
