...
use Zend\OpenId\Provider\User\Session as Session;
use Zend\OpenId\Provider\Storage\File as File;
use Zend\OpenId\Provider\Storage as Storage;
use Zend\OpenId\Provider\User as OpenIdProviderUser;
use Zend\OpenId\Extension as Extension;
use Zend\OpenId as OpenId;
...
class  Provider 
{
    ...
    /**
     * Constructs a  Provider  object with given parameters.
     *
     ...
     * @param  OpenIdProviderUser  $user is an object \for communication
     *  with \User-Agent and store information about logged-in user (it is a
     *   Session  object by default)
     * @param  Storage  $storage is an object \for keeping
     *  persistent database (it is a  File  object
     *  by default)
    ...
    public function allowSite($root, $extensions=null)
    {
        ...
        if ($extensions !== null) {
            $data = array();
             Extension ::forAll($extensions, 'getTrustData', $data);
        } else {
            $data = true;
        }
     ...
     * @param mixed $extensions extension object or array \of extensions objects
     * @param  ControllerResponseAbstract  $response an optional response
     *  object to perform HTTP or HTML form redirection
     * @return mixed
     */
    public function handle($params=null, $extensions=null,
                            ControllerResponseAbstract  $response = null)
    {
        if ($params === null) {
         ...
         ControllerResponseAbstract  $response = null)
    {
        ...
        if (! Extension ::forAll($extensions, 'parseRequest', $params)) {
            $ret['openid.mode'] = ($immediate && $version >= 2.0) ? 'setup_needed': 'cancel';
            return $ret;
        }
        ...
        if (is_array($trusted)) {
            if (! Extension ::forAll($extensions, 'checkTrustData', $trusted)) {
                $trusted = null;
            }
        }
    ...
    protected function _respond($version, $ret, $params, $extensions=null)
    {
        ...
        $ret['openid.mode'] = 'id_res';
         ...
         Extension ::forAll($extensions, 'prepareResponse', $ret);
