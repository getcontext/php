...
use Zend\TimeSync\Protocol as Protocol;
use Zend\Locale\Exception as LocaleException;
use Zend\Date\DateObject as DateObject;
use Zend\Date\Exception as DateException;
use Zend\Locale\Format as Format;
use Zend\Locale\Data as LocaleData;
use Zend\Locale\Math as Math;
...
class  Date  extends  DateObject 
{
     ...
     *
     * @param  string|integer| Date |array  $date    OPTIONAL Date value or value \of date part to set
     *                                                 ,depending on $part. If null the actual time is set
     * @param  string                          $part    OPTIONAL Defines the input format \of $date
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return  Date 
     * @throws  DateException 
     */
    public function __construct($date = null, $part = null, $locale = null)
    {
        if ( Locale ::isLocale($date)) {
            $locale = $date;
            $date = null;
            $part = null;
        } else if ( Locale ::isLocale($part)) {
            $locale = $part;
            $part   = null;
        }
    ...
    public static function setOptions(array $options = array())
    {
        ...
        foreach ($options as $name => $value) {
            ...
            if (array_key_exists($name, self::$_Options)) {
                switch($name) {
                    case 'format_type' :
                        if ((strtolower($value) != 'php') && (strtolower($value) != 'iso')) {
                            ...
                            throw new  DateException ("Unknown format type ($value) \for dates, only 'iso' and 'php' supported", $value);
                        }
                        break;
                    case 'fix_dst' :
                        if (!is_bool($value)) {
                            require_once 'Zend/Date/Exception.php';
                            throw new  DateException ("'fix_dst' has to be boolean", $value);
                        }
                        break;
                    case 'extend_month' :
                        if (!is_bool($value)) {
                            require_once 'Zend/Date/Exception.php';
                            throw new  DateException ("'extend_month' has to be boolean", $value);
                        }
                        break;
                    case 'cache' :
                        if (!$value instanceof  Core ) {
                            require_once 'Zend/Date/Exception.php';
                            throw new  DateException ("Instance \of \Zend\Cache expected");
                        }
                        parent::$_cache = $value;
                         LocaleData ::setCache($value);
                        break;
                    case 'timesync' :
                        if (!$value instanceof  Protocol ) {
                            require_once 'Zend/Date/Exception.php';
                            throw new  DateException ("Instance \of \Zend\TimeSync expected");
                        }
                        $date = $value->getInfo();
                ...
                require_once 'Zend/Date/Exception.php';
                throw new  DateException ("Unknown option: $name = $value");
            }
        }
    }
    ...
    /**
     * Returns this object's internal UNIX timestamp (equivalent to  Date ::TIMESTAMP).
     ...
     * @param  string|integer|array| Date   $stamp   Timestamp to calculate, when null the actual timestamp is calculated
     * @return  Date |integer
     * @throws  DateException 
     */
    private function _timestamp($calc, $stamp)
    {
        ...
        if (is_array($stamp)) {
            if (isset($stamp['timestamp']) === true) {
                ...
                require_once 'Zend/Date/Exception.php';
                throw new  DateException ('no timestamp given in array');
            }
        }
     ...
     *
     * @param  integer|string|array| Date   $timestamp  Timestamp to set
     * @return  Date 
     * @throws  DateException 
     */
     ...
     * Adds a timestamp
     *
     * @param  integer|string|array| Date   $timestamp  Timestamp to add
     * @return  Date 
     * @throws  DateException 
     */
     ...
     *
     * @param  integer|string|array| Date   $timestamp  Timestamp to sub
     * @return  Date 
     * @throws  DateException 
     */
     ...
     *
     * @param  integer|string|array| Date   $timestamp  Timestamp to compare
    ...
    {
        if ((strlen($format) != 2) and ( Locale ::isLocale($format))) {
            $locale = $format;
            $format = null;
        }

        if ( Locale ::isLocale($type)) {
            $locale = $type;
            $type = null;
        }
     ...
     *
     * @param  string|integer| Date   $part  OPTIONAL Defines the date or datepart to return as integer
    ...
    public function get($part = null, $locale = null)
    {
        ...
        switch($part) {
            ...
            case self::WEEKDAY_SHORT :
                ...
                $day =  LocaleData ::getContent($locale, 'day', array('gregorian', 'format', 'wide', $weekday));
            ...
            case self::WEEKDAY :
                $weekday = strtolower($this->date('D', $this->getUnixTimestamp(), false));
                return  LocaleData ::getContent($locale, 'day', array('gregorian', 'format', 'wide', $weekday));
                break;
            ...
            case self::WEEKDAY_NARROW :
                $weekday = strtolower($this->date('D', $this->getUnixTimestamp(), false));
                $day =  LocaleData ::getContent($locale, 'day', array('gregorian', 'format', 'abbreviated', $weekday));
                return substr($day, 0, 1);
                break;

            case self::WEEKDAY_NAME :
                $weekday = strtolower($this->date('D', $this->getUnixTimestamp(), false));
                return  LocaleData ::getContent($locale, 'day', array('gregorian', 'format', 'abbreviated', $weekday));
                break;
            ...
            case self::MONTH_NAME :
                ...
                return  LocaleData ::getContent($locale, 'month', array('gregorian', 'format', 'wide', $month));
                break;
            ...
            case self::MONTH_NAME_SHORT :
                $month = $this->date('n', $this->getUnixTimestamp(), false);
                return  LocaleData ::getContent($locale, 'month', array('gregorian', 'format', 'abbreviated', $month));
                break;
            ...
            case self::MONTH_NAME_NARROW :
                $month = $this->date('n', $this->getUnixTimestamp(), false);
                $mon =  LocaleData ::getContent($locale, 'month', array('gregorian', 'format', 'abbreviated', $month));
            ...
            case self::MERIDIEM :
                ...
                if ($am == 'am') {
                    return  LocaleData ::getContent($locale, 'am');
                }
                return  LocaleData ::getContent($locale, 'pm');
                break;
            ...
            case self::ERA :
                ...
                if ($year < 0) {
                    return  LocaleData ::getContent($locale, 'era', array('gregorian', 'Abbr', '0'));
                }
                return  LocaleData ::getContent($locale, 'era', array('gregorian', 'Abbr', '1'));
                break;

            case self::ERA_NAME :
                $year = $this->date('Y', $this->getUnixTimestamp(), false);
                if ($year < 0) {
                    return  LocaleData ::getContent($locale, 'era', array('gregorian', 'Names', '0'));
                }
                return  LocaleData ::getContent($locale, 'era', array('gregorian', 'Names', '1'));
                break;

            case self::DATES :
                return $this->toString( Format ::getDateFormat($locale), 'iso', $locale);
                break;

            case self::DATE_FULL :
                $date =  LocaleData ::getContent($locale, 'date', array('gregorian', 'full'));
                return $this->toString($date, 'iso', $locale);
                break;

            case self::DATE_LONG :
                $date =  LocaleData ::getContent($locale, 'date', array('gregorian', 'long'));
                return $this->toString($date, 'iso', $locale);
                break;

            case self::DATE_MEDIUM :
                $date =  LocaleData ::getContent($locale, 'date', array('gregorian', 'medium'));
                return $this->toString($date, 'iso', $locale);
                break;

            case self::DATE_SHORT :
                $date =  LocaleData ::getContent($locale, 'date', array('gregorian', 'short'));
                return $this->toString($date, 'iso', $locale);
                break;

            case self::TIMES :
                return $this->toString( Format ::getTimeFormat($locale), 'iso', $locale);
                break;

            case self::TIME_FULL :
                $time =  LocaleData ::getContent($locale, 'time', 'full');
                return $this->toString($time, 'iso', $locale);
                break;

            case self::TIME_LONG :
                $time =  LocaleData ::getContent($locale, 'time', 'long');
                return $this->toString($time, 'iso', $locale);
                break;

            case self::TIME_MEDIUM :
                $time =  LocaleData ::getContent($locale, 'time', 'medium');
                return $this->toString($time, 'iso', $locale);
                break;

            case self::TIME_SHORT :
                $time =  LocaleData ::getContent($locale, 'time', 'short');
                return $this->toString($time, 'iso', $locale);
                break;
    ...
    private function getDigitFromName($name)
    {
        switch($name) {
            ...
            default:
                require_once 'Zend/Date/Exception.php';
                throw new  DateException ('Month ($name) is not a known month');
        }
    }
     ...
     * For a complete list \of supported datepart values look into the docu
     *
     * @param  string|integer|array| Date   $date    Date or datepart to set
     ...
     * For a complete list \of supported datepart values look into the docu.
     *
     * @param  string|integer|array| Date   $date    Date or datepart to add
     ...
     *
     * @param  string|integer|array| Date   $date    Date or datepart to subtract
     ...
     *
     * @param  string|integer|array| Date   $date    Date or datepart to compare with the date object
    ...
    /**
     * Returns a new instance \of  Date  with the selected part copied.
     ...
     * @param  string| Locale   $locale  OPTIONAL New object's locale.  No adjustments to timezone are made.
     * @return  Date 
     */
     ...
     * @return integer|string| Date   new timestamp or  Date  depending on calculation
     */
     ...
     * @param  string|integer|array| Date   $date    Date or datepart to calculate with
     * @param  string                          $part    Part \of the date to calculate, if null the timestamp is used
     * @param  string| Locale               $locale  Locale \for parsing input
     * @return integer|string| Date         new timestamp
     * @throws  DateException 
     */
    private function _calculate($calc, $date, $part, $locale)
    {
        if (is_null($date) === true) {
            require_once 'Zend/Date/Exception.php';
            throw new  DateException ('parameter $date must be set, null is not allowed');
        }
        ...
        if (is_array($date) === true) {
            if (empty($part) === false) {
                switch($part) {
                    ...
                    default:
                        require_once 'Zend/Date/Exception.php';
                        throw new  DateException ("datepart \for part ($part) not found in array");
                        break;
                }
            } else {
                $hours = 0;
        ...
        switch($part) {
            ...
            case self::DAY:
                ...
                require_once 'Zend/Date/Exception.php';
                throw new  DateException ("invalid date ($date) operand, day expected", $date);
                break;

            case self::WEEKDAY_SHORT:
                $daylist =  LocaleData ::getList($locale, 'day');
                ...
                // Weekday not found
                require_once 'Zend/Date/Exception.php';
                throw new  DateException ("invalid date ($date) operand, weekday expected", $date);
                break;
            ...
            case self::DAY_SUFFIX:
                require_once 'Zend/Date/Exception.php';
                throw new  DateException ('day suffix not supported', $date);
                break;
            ...
            case self::WEEKDAY_NARROW:
                $daylist =  LocaleData ::getList($locale, 'day', array('gregorian', 'format', 'abbreviated'));
                $weekday = (int) $this->get(self::WEEKDAY_DIGIT, $locale);
                $cnt = 0;
                foreach ($daylist as $key => $value) {
            ...
            case self::WEEK:
                ...
                require_once 'Zend/Date/Exception.php';
                throw new  DateException ("invalid date ($date) operand, week expected", $date);
                break;


            // month formats
            case self::MONTH_NAME:
                $monthlist =  LocaleData ::getList($locale, 'month');
                $cnt = 0;
                ...
                // Monthname not found
                require_once 'Zend/Date/Exception.php';
                throw new  DateException ("invalid date ($date) operand, month expected", $date);
                break;
            ...
            case self::MONTH_NAME_SHORT:
                $monthlist =  LocaleData ::getList($locale, 'month', array('gregorian', 'format', 'abbreviated'));
                $cnt = 0;
                foreach ($monthlist as $key => $value) {
                    if (strtoupper($value) == strtoupper($date)) {
                        $found = $key;
            ...
            case self::MONTH_DAYS:
                require_once 'Zend/Date/Exception.php';
                throw new  DateException ('month days not supported', $date);
                break;


            case self::MONTH_NAME_NARROW:
                $monthlist =  LocaleData ::getList($locale, 'month', array('gregorian', 'stand-alone', 'narrow'));
                $cnt       = 0;
                foreach ($monthlist as $key => $value) {
                    if (strtoupper($value) === strtoupper($date)) {
                        $found = $key;
            ...
            case self::LEAPYEAR:
                require_once 'Zend/Date/Exception.php';
                throw new  DateException ('leap year not supported', $date);
                break;
            ...
            case self::YEAR_8601:
                ...
                require_once 'Zend/Date/Exception.php';
                throw new  DateException ("invalid date ($date) operand, year expected", $date);
                break;
            ...
            case self::MERIDIEM:
                require_once 'Zend/Date/Exception.php';
                throw new  DateException ('meridiem not supported', $date);
                break;
            ...
            case self::SWATCH:
                ...
                require_once 'Zend/Date/Exception.php';
                throw new  DateException ("invalid date ($date) operand, swatchstamp expected", $date);
                break;
            ...
            case self::HOUR_SHORT_AM:
                ...
                require_once 'Zend/Date/Exception.php';
                throw new  DateException ("invalid date ($date) operand, hour expected", $date);
                break;
            ...
            case self::MINUTE:
                ...
                require_once 'Zend/Date/Exception.php';
                throw new  DateException ("invalid date ($date) operand, minute expected", $date);
                break;
            ...
            case self::SECOND:
                ...
                require_once 'Zend/Date/Exception.php';
                throw new  DateException ("invalid date ($date) operand, second expected", $date);
                break;
            ...
            case self::MILLISECOND:
                ...
                require_once 'Zend/Date/Exception.php';
                throw new  DateException ("invalid date ($date) operand, milliseconds expected", $date);
                break;
            ...
            case self::TIMEZONE_SECS:
                require_once 'Zend/Date/Exception.php';
                throw new  DateException ('timezone not supported', $date);
                break;

            case self::DAYLIGHT:
                require_once 'Zend/Date/Exception.php';
                throw new  DateException ('daylight not supported', $date);
                break;

            case self::GMT_DIFF:
            case self::GMT_DIFF_SEP:
                require_once 'Zend/Date/Exception.php';
                throw new  DateException ('gmtdiff not supported', $date);
                break;
            ...
            case self::ISO_8601:
                ...
                if (empty($datematch) and empty($timematch)) {
                    require_once 'Zend/Date/Exception.php';
                    throw new  DateException ("unsupported ISO8601 format ($date)", $date);
                }
                if (!empty($timematch)) {
            ...
            case self::RFC_2822:
                ...
                if (!$result) {
                    require_once 'Zend/Date/Exception.php';
                    throw new  DateException ("no RFC 2822 format ($date)", $date);
                }
            ...
            case self::TIMESTAMP:
                ...
                require_once 'Zend/Date/Exception.php';
                throw new  DateException ("invalid date ($date) operand, timestamp expected", $date);
                break;
            ...
            case self::ERA_NAME:
                require_once 'Zend/Date/Exception.php';
                throw new  DateException ('era not supported', $date);
                break;
            ...
            case self::DATES:
                try {
                    ...
                    require_once 'Zend/Date/Exception.php';
                    throw new  DateException ($e->getMessage(), $date);
                }
                break;

            case self::DATE_FULL:
                try {
                    $format =  LocaleData ::getContent($locale, 'date', array('gregorian', 'full'));
            ...
            case self::DATE_LONG:
                try {
                    $format =  LocaleData ::getContent($locale, 'date', array('gregorian', 'long'));
                    $parsed =  Format ::getDate($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
            ...
            case self::DATE_MEDIUM:
                try {
                    $format =  LocaleData ::getContent($locale, 'date', array('gregorian', 'medium'));
                    $parsed =  Format ::getDate($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
            ...
            case self::DATE_SHORT:
                try {
                    $format =  LocaleData ::getContent($locale, 'date', array('gregorian', 'short'));
                    $parsed =  Format ::getDate($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
            ...
            case self::TIME_FULL:
                try {
                    $format =  LocaleData ::getContent($locale, 'time', array('gregorian', 'full'));
            ...
            case self::TIME_LONG:
                try {
                    $format =  LocaleData ::getContent($locale, 'time', array('gregorian', 'long'));
                    $parsed =  Format ::getTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
                    if ($calc != 'set') {
                        $month = 1;
                        $day   = 1;
            ...
            case self::TIME_MEDIUM:
                try {
                    $format =  LocaleData ::getContent($locale, 'time', array('gregorian', 'medium'));
                    $parsed =  Format ::getTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
                    if ($calc != 'set') {
                        $month = 1;
                        $day   = 1;
            ...
            case self::TIME_SHORT:
                try {
                    $format =  LocaleData ::getContent($locale, 'time', array('gregorian', 'short'));
                    $parsed =  Format ::getTime($date, array('date_format' => $format, 'format_type' => 'iso', 'locale' => $locale));
                    if ($calc != 'set') {
                        $month = 1;
                        $day   = 1;
            ...
            case self::RFC_3339:
                ...
                if (!$result) {
                    require_once 'Zend/Date/Exception.php';
                    throw new  DateException ("invalid date ($date) operand, ATOM format expected", $date);
                }
            ...
            case self::COOKIE:
                ...
                if (!$result) {
                    require_once 'Zend/Date/Exception.php';
                    throw new  DateException ("invalid date ($date) operand, COOKIE format expected", $date);
                }
            ...
            case self::RFC_822:
                ...
                if (!$result) {
                    require_once 'Zend/Date/Exception.php';
                    throw new  DateException ("invalid date ($date) operand, RFC 822 date format expected", $date);
                }
            ...
            case self::RFC_850:
                ...
                if (!$result) {
                    require_once 'Zend/Date/Exception.php';
                    throw new  DateException ("invalid date ($date) operand, RFC 850 date format expected", $date);
                }
            ...
            case self::RFC_1036:
                ...
                if (!$result) {
                    require_once 'Zend/Date/Exception.php';
                    throw new  DateException ("invalid date ($date) operand, RFC 1036 date format expected", $date);
                }
            ...
            case self::RFC_1123:
                ...
                if (!$result) {
                    require_once 'Zend/Date/Exception.php';
                    throw new  DateException ("invalid date ($date) operand, RFC 1123 date format expected", $date);
                }
            ...
            case self::RSS:
                ...
                if (!$result) {
                    require_once 'Zend/Date/Exception.php';
                    throw new  DateException ("invalid date ($date) operand, RSS date format expected", $date);
                }
            ...
            case self::W3C:
                ...
                if (!$result) {
                    require_once 'Zend/Date/Exception.php';
                    throw new  DateException ("invalid date ($date) operand, W3C date format expected", $date);
                }
     ...
     *
     * @param  string|integer|array| Date   $date    Date or datepart to equal with
     ...
     *
     * @param  string|integer|array| Date   $date    Date or datepart to compare with
     * @param  string                          $part    OPTIONAL Part \of the date to compare, if null the timestamp is used
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return boolean
     * @throws  DateException 
    ...
    /**
     * Returns only the time \of the date as new  Date  object
     * For example:
     ...
     *
     * @param  string                    $calc    Calculation to make
     * @param  string|integer|array| Date   $time    Time to calculate with, if null the actual time is taken
     * @param  string                          $format  Timeformat \for parsing input
     * @param  string| Locale               $locale  Locale \for parsing input
     * @return integer| Date   new time
     * @throws  DateException 
     */
    private function _time($calc, $time, $format, $locale)
    {
        if (is_null($time)) {
            require_once 'Zend/Date/Exception.php';
            throw new  DateException ('parameter $time must be set, null is not allowed');
        }
        ...
        if ($time instanceof  Date ) {
            ...
            if (is_array($time)) {
                if ((isset($time['hour']) === true) or (isset($time['minute']) === true) or
                    ...
                    require_once 'Zend/Date/Exception.php';
                    throw new  DateException ("no hour, minute or second given in array");
                }
            } else {
                if (self::$_Options['format_type'] == 'php') {
                    $format =  Format ::convertPhpToIsoFormat($format);
     ...
     *
     * @param  string|integer|array| Date   $time    Time to set
     ...
     *
     * @param  string|integer|array| Date   $time    Time to add
     * @param  string                          $format  OPTIONAL Timeformat \for parsing input
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return  Date   new time
     * @throws  DateException 
     ...
     * For example: HH:mm:ss -> 10 -> -10 hours
     *
     * @param  string|integer|array| Date   $time    Time to sub
     * @param  string                          $format  OPTIONAL Timeformat \for parsing input
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return  Date   new time
     * @throws  DateException 
     ...
     * For example: HH:mm:ss -> 10 -> 10 hours
     *
     * @param  string|integer|array| Date   $time    Time to compare
     * @param  string                          $format  OPTIONAL Timeformat \for parsing input
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return integer  0 = equal, 1 = later, -1 = earlier
     * @throws  DateException 
     ...
     *
     * @param  string                          $calc    Calculation to make
     * @param  string|integer|array| Date   $date    Date to calculate with, if null the actual date is taken
     * @param  string                          $format  Date format \for parsing
     * @param  string| Locale               $locale  Locale \for parsing input
     * @return integer| Date   new date
     * @throws  DateException 
     */
        ...
        if ($date instanceof  Date ) {
            ...
            if (is_array($date)) {
                if ((isset($date['year']) === true) or (isset($date['month']) === true) or
                    ...
                    require_once 'Zend/Date/Exception.php';
                    throw new  DateException ("no day,month or year given in array");
                }
            } else {
                if (self::$_Options['format_type'] == 'php') {
                    $format =  Format ::convertPhpToIsoFormat($format);
     ...
     *
     * @param  string|integer|array| Date   $date    Date to set
     ...
     *
     * @param  string|integer|array| Date   $date    Date to add
     ...
     *
     * @param  string|integer|array| Date   $date    Date to sub
     * @param  string                          $format  OPTIONAL Date format \for parsing input
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return  Date   new date
     * @throws  DateException 
     ...
     *
     * @param  string|integer|array| Date   $date    Date to compare
     * @param  string                          $format  OPTIONAL Date format \for parsing input
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return  Date   new date
     * @throws  DateException 
     ...
     *
     * @param  string|integer| Date   $date    ISO Date to set
     * @param  string| Locale         $locale  OPTIONAL Locale \for parsing input
     * @return integer| Date   new date
     * @throws  DateException 
     */
     ...
     * Returned is the new date object
     *
     * @param  string|integer| Date   $date    ISO Date to add
     * @param  string| Locale         $locale  OPTIONAL Locale \for parsing input
     * @return integer| Date   new date
     * @throws  DateException 
     */
     ...
     * Returned is the new date object
     *
     * @param  string|integer| Date   $date    ISO Date to sub
     * @param  string| Locale         $locale  OPTIONAL Locale \for parsing input
     * @return integer| Date   new date
     * @throws  DateException 
     */
     ...
     * Returned is the new date object
     *
     * @param  string|integer| Date   $date    RFC 822 to set
     * @param  string| Locale         $locale  OPTIONAL Locale \for parsing input
     * @return integer| Date   new date
     * @throws  DateException 
     */
     ...
     * Returned is the new date object
     *
     * @param  string|integer| Date   $date    RFC 822 Date to add
     * @param  string| Locale         $locale  OPTIONAL Locale \for parsing input
     * @return integer| Date   new date
     * @throws  DateException 
     */
     ...
     * Returned is the new date object
     *
     * @param  string|integer| Date   $date    RFC 822 Date to sub
     * @param  string| Locale         $locale  OPTIONAL Locale \for parsing input
     * @return integer| Date   new date
     * @throws  DateException 
     */
    ...
    private function _checkLocation($location)
    {
        if (!isset($location['longitude']) or !isset($location['latitude'])) {
            require_once 'Zend/Date/Exception.php';
            throw new  DateException ('Location must include \'longitude\' and \'latitude\'', $location);
        }
        if (($location['longitude'] > 180) or ($location['longitude'] < -180)) {
            require_once 'Zend/Date/Exception.php';
            throw new  DateException ('Longitude must be between -180 and 180', $location);
        }
        if (($location['latitude'] > 90) or ($location['latitude'] < -90)) {
            require_once 'Zend/Date/Exception.php';
            throw new  DateException ('Latitude must be between -90 and 90', $location);
        }
     ...
     * For a list \of cities and correct locations use the class  Date _Cities
     *
     ...
     *
     * @param  integer|array| Date   $year  Year to check
     * @return boolean
     */
    public static function checkLeapYear($year)
    {
        ...
        if (is_array($year)) {
            if (isset($year['year']) === true) {
                ...
                require_once 'Zend/Date/Exception.php';
                throw new  DateException ("no year given in array");
            }
        }
        if (!is_numeric($year)) {
            require_once 'Zend/Date/Exception.php';
            throw new  DateException ("year ($year) has to be integer \for checkLeapYear()", $year);
        }
     ...
     *
     * @param  string                          $calc    Calculation to make
     * @param  string|integer|array| Date   $date    Date or Part to calculate
     ...
     *
     * @param  string                    $calc    Calculation to make
     * @param  string|integer| Date   $value   Datevalue to calculate with, if null the actual value is taken
     * @param  string| Locale         $locale  Locale \for parsing input
     * @return integer| Date   new date
     * @throws  DateException 
     */
    private function _calcvalue($calc, $value, $type, $parameter, $locale)
    {
        if (is_null($value)) {
            require_once 'Zend/Date/Exception.php';
            throw new  DateException ("parameter $type must be set, null is not allowed");
        }
        ...
        if ($value instanceof  Date ) {
            ...
            require_once 'Zend/Date/Exception.php';
            throw new  DateException ("invalid $type ($value) operand", $value);
        }
     ...
     * Returned is the new date object
     *
     * @param  string|integer|array| Date   $date    Year to set
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return  Date   new date
     * @throws  DateException 
     */
     ...
     * Returned is the new date object
     *
     * @param  string|integer|array| Date   $date    Year to add
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return  Date   new date
     * @throws  DateException 
     */
     ...
     * Returned is the new date object
     *
     * @param  string|integer|array| Date   $date    Year to sub
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return  Date   new date
     * @throws  DateException 
     */
     ...
     * Returns if equal, earlier or later
     *
     * @param  string|integer|array| Date   $year    Year to compare
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return integer  0 = equal, 1 = later, -1 = earlier
     * @throws  DateException 
     */
     ...
     *
     * @param  string                          $calc    Calculation to make
     * @param  string|integer|array| Date   $month   Month to calculate with, if null the actual month is taken
     * @param  string| Locale               $locale  Locale \for parsing input
     * @return integer| Date   new time
     * @throws  DateException 
     */
    private function _month($calc, $month, $locale)
    {
        if (is_null($month)) {
            require_once 'Zend/Date/Exception.php';
            throw new  DateException ('parameter $month must be set, null is not allowed');
        }
        ...
        if ($month instanceof  Date ) {
            ...
            if (is_numeric($month)) {
                ...
                if (isset($month['month']) === true) {
                    ...
                    require_once 'Zend/Date/Exception.php';
                    throw new  DateException ("no month given in array");
                }
            } else {
                $monthlist  =  LocaleData ::getList($locale, 'month');
                $monthlist2 =  LocaleData ::getList($locale, 'month', array('gregorian', 'format', 'abbreviated'));
                ...
                if ($found == 0) {
                    require_once 'Zend/Date/Exception.php';
                    throw new  DateException ("unknown month name ($month)", $month);
                }
            }
        }
     ...
     * Returned is the new date object
     *
     * @param  string|integer|array| Date   $month   Month to set
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return  Date   new date
     * @throws  DateException 
     */
     ...
     * Returned is the new date object
     *
     * @param  string|integer|array| Date   $month   Month to add
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return  Date   new date
     * @throws  DateException 
     */
     ...
     * Returned is the new date object
     *
     * @param  string|integer|array| Date   $month   Month to sub
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return  Date   new date
     * @throws  DateException 
     */
     ...
     * Returns if equal, earlier or later
     *
     * @param  string|integer|array| Date   $month   Month to compare
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return integer  0 = equal, 1 = later, -1 = earlier
     * @throws  DateException 
     */
     ...
     *
     * @param $locale  string| Locale   OPTIONAL Locale \for parsing input
     * @return  Date 
     */
     ...
     * @param $day     string|integer| Date   Day to calculate, when null the actual day is calculated
     * @param $locale  string| Locale         Locale \for parsing input
     * @return  Date |integer
     */
    private function _day($calc, $day, $locale)
    {
        if (is_null($day)) {
            require_once 'Zend/Date/Exception.php';
            throw new  DateException ('parameter $day must be set, null is not allowed');
        }
        ...
        if (is_numeric($day)) {
            ...
            if (isset($day['day']) === true) {
                ...
                require_once 'Zend/Date/Exception.php';
                throw new  DateException ("no day given in array");
            }
        } else {
            switch (strlen($day)) {
                case 1 :
     ...
     *
     * @param  string|integer|array| Date   $month   Day to set
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return  Date   new date
     * @throws  DateException 
     */
     ...
     *
     * @param  string|integer|array| Date   $month   Day to add
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return  Date   new date
     * @throws  DateException 
     */
     ...
     *
     * @param  string|integer|array| Date   $month   Day to sub
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return  Date   new date
     * @throws  DateException 
     */
     ...
     * Returns if equal, earlier or later
     *
     * @param  string|integer|array| Date   $day     Day to compare
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return integer  0 = equal, 1 = later, -1 = earlier
     * @throws  DateException 
     */
     ...
     *
     * @param  $calc     string                          Type \of calculation to make
     * @param  $weekday  string|integer|array| Date   Weekday to calculate, when null the actual weekday is calculated
     * @param  $locale   string| Locale               Locale \for parsing input
     * @return  Date |integer
     * @throws  DateException 
     */
    private function _weekday($calc, $weekday, $locale)
    {
        if (is_null($weekday)) {
            require_once 'Zend/Date/Exception.php';
            throw new  DateException ('parameter $weekday must be set, null is not allowed');
        }
        ...
        if (is_numeric($weekday)) {
            ...
            if (isset($weekday['weekday']) === true) {
                ...
                require_once 'Zend/Date/Exception.php';
                throw new  DateException ("no weekday given in array");
            }
        } else {
     ...
     *
     * @param  string|integer|array| Date   $month   Weekday to set
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return  Date   new date
     * @throws  DateException 
     */
     ...
     * wednesday.
     *
     * @param  string|integer|array| Date   $month   Weekday to add
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return  Date   new date
     * @throws  DateException 
     */
     ...
     * wednesday.
     *
     * @param  string|integer|array| Date   $month   Weekday to sub
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return  Date   new date
     * @throws  DateException 
     */
     ...
     * Returns if equal, earlier or later
     *
     * @param  string|integer|array| Date   $weekday  Weekday to compare
     * @param  string| Locale               $locale   OPTIONAL Locale \for parsing input
     * @return integer  0 = equal, 1 = later, -1 = earlier
     * @throws  DateException 
     */
     ...
     *
     * @param  string|integer|array| Date   $day     Day \of Year to set
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return  Date   new date
     * @throws  DateException 
     */
     ...
     *
     * @param  string|integer|array| Date   $day     Day \of Year to add
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return  Date   new date
     * @throws  DateException 
     */
     ...
     *
     * @param  string|integer|array| Date   $day     Day \of Year to sub
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return  Date   new date
     * @throws  DateException 
     */
     ...
     * Returns if equal, earlier or later
     *
     * @param  string|integer|array| Date   $day     Day \of Year to compare
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return integer  0 = equal, 1 = later, -1 = earlier
     * @throws  DateException 
     */
     ...
     *
     * @param  string|integer|array| Date   $hour    Hour to set
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return  Date   new date
     * @throws  DateException 
     */
     ...
     *
     * @param  string|integer|array| Date   $hour    Hour to add
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return  Date   new date
     * @throws  DateException 
     */
     ...
     *
     * @param  string|integer|array| Date   $hour    Hour to sub
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return  Date   new date
     * @throws  DateException 
     */
     ...
     * Returns if equal, earlier or later
     *
     * @param  string|integer|array| Date   $hour    Hour to compare
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return integer  0 = equal, 1 = later, -1 = earlier
     * @throws  DateException 
     */
     ...
     *
     * @param  string|integer|array| Date   $minute  Minute to set
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return  Date   new date
     * @throws  DateException 
     */
     ...
     *
     * @param  string|integer|array| Date   $minute  Minute to add
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return  Date   new date
     * @throws  DateException 
     */
     ...
     *
     * @param  string|integer|array| Date   $minute  Minute to sub
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return  Date   new date
     * @throws  DateException 
     */
     ...
     * Returns if equal, earlier or later
     *
     * @param  string|integer|array| Date   $minute  Hour to compare
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return integer  0 = equal, 1 = later, -1 = earlier
     * @throws  DateException 
     */
     ...
     *
     * @param  string|integer|array| Date  $second Second to set
     * @param  string| Locale              $locale (Optional) Locale \for parsing input
     * @return  Date   new date
     * @throws  DateException 
     */
     ...
     *
     * @param  string|integer|array| Date  $second Second to add
     * @param  string| Locale              $locale (Optional) Locale \for parsing input
     * @return  Date   new date
     * @throws  DateException 
     */
     ...
     *
     * @param  string|integer|array| Date  $second Second to sub
     * @param  string| Locale              $locale (Optional) Locale \for parsing input
     * @return  Date   new date
     * @throws  DateException 
     */
     ...
     * Returns if equal, earlier or later
     *
     * @param  string|integer|array| Date  $second Second to compare
     * @param  string| Locale              $locale (Optional) Locale \for parsing input
     * @return integer  0 = equal, 1 = later, -1 = earlier
     * @throws  DateException 
     */
    ...
    public function setFractionalPrecision($precision)
    {
        if (!intval($precision) or ($precision < 0) or ($precision > 9)) {
            require_once 'Zend/Date/Exception.php';
            throw new  DateException ("precision ($precision) must be a positive integer less than 10", $precision);
        }
     ...
     *
     * @param  integer| Date  $milli     (Optional) Millisecond to set, when null the actual millisecond is set
    ...
    public function setMilliSecond($milli = null, $precision = null)
    {
        if ($milli === null) {
            ...
            require_once 'Zend/Date/Exception.php';
            throw new  DateException ("invalid milli second ($milli) operand", $milli);
        }
     ...
     *
     * @param  integer| Date  $milli     (Optional) Millisecond to add, when null the actual millisecond is added
     ...
     *
     * @param  integer| Date  $milli     (Optional) Millisecond to sub, when null the actual millisecond is subtracted
     * @param  integer           $precision (Optional) Fractional precision \for the given milliseconds
     * @return integer
     */
     ...
     *
     * @param  integer| Date   $milli  OPTIONAL Millisecond to compare, when null the actual millisecond is compared
     * @param  integer            $precision  OPTIONAL Fractional precision \for the given milliseconds
     * @return integer
     */
    public function compareMilliSecond($milli = null, $precision = null)
    {
        ...
        if ($precision === 0) {
            require_once 'Zend/Date/Exception.php';
            throw new  DateException ('precision is 0');
        }
     ...
     *
     * @param  string|integer|array| Date   $week    Week to set
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return  Date 
     * @throws  DateException 
     */
     ...
     *
     * @param  string|integer|array| Date   $week    Week to add
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return  Date 
     * @throws  DateException 
     */
     ...
     *
     * @param  string|integer|array| Date   $week    Week to sub
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return  Date 
     * @throws  DateException 
     */
     ...
     *
     * @param  string|integer|array| Date   $week    Week to compare
     * @param  string| Locale               $locale  OPTIONAL Locale \for parsing input
     * @return integer 0 = equal, 1 = later, -1 = earlier
     */
     ...
     *
     * @param  string| Locale  $locale (Optional) Locale \for parsing input
     * @throws  DateException  When the given locale does not exist
     * @return string
     */
    public function setLocale($locale = null)
    {
        if ($locale instanceof  Locale ) {
            $this->_Locale = $locale;
        } else if (!$locale =  Locale ::isLocale($locale, true)) {
            require_once 'Zend/Date/Exception.php';
            throw new  DateException ("Given locale ($locale) does not exist", $locale);
        } else {
     ...
     * @param  string| Locale  $locale (Optional) Locale \for parsing date parts
