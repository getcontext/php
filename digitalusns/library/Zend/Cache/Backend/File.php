...
class  File  extends  Backend  implements  CacheBackendInterface 
{
    /**
     * Available options
     *
    ...
    public function __construct(array $options = array())
    {
        ...
        if (isset($this->_options['file_name_prefix'])) { // particular case \for this option
            if (!preg_match('~^[\w]+$~', $this->_options['file_name_prefix'])) {
                 Cache ::throwException('Invalid file_name_prefix : must use only [a-zA-A0-9_]');
            }
        }
    ...
    {
        if (!is_dir($value)) {
             Cache ::throwException('cache_dir must be a directory');
        }
        if (!is_writable($value)) {
             Cache ::throwException('cache_dir is not writable');
        }
    ...
    public function load($id, $doNotTestCacheValidity = false)
    {
        ...
        if ($this->_options['read_control']) {
            ...
            if ($hashData != $hashControl) {
                ...
                $this->_log('\Zend\Cache\Backend\File::load() / read_control : stored hash and computed hash do not match');
                $this->remove($id);
                return false;
            }
        }
    ...
    private function _remove($file)
    {
        ...
        if (!@unlink($file)) {
            ...
            $this->_log("\Zend\Cache\Backend\File::_remove() : we can't remove $file");
            return false;
        }
        return true;
    }
     ...
     * @return boolean True if no problem
     */
    private function _clean($dir, $mode =  Cache ::CLEANING_MODE_ALL, $tags = array())
    {
        if (!is_dir($dir)) {
            return false;
        }
