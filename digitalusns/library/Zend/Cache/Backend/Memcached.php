...
class  Memcached  extends  Backend  implements  CacheBackendInterface 
{
    /**
    ...
    protected $_options = array(
        'servers' => array(array(
            ...
            'persistent' =>  Memcached ::DEFAULT_PERSISTENT
        )),
        'compression' => false
    );
    ...
    public function __construct(array $options = array())
    {
        if (!extension_loaded('memcache')) {
             Cache ::throwException('The memcache extension must be loaded \for using this backend !');
        }
        ...
        foreach ($this->_options['servers'] as $server) {
            if (!array_key_exists('persistent', $server)) {
                $server['persistent'] =  Memcached ::DEFAULT_PERSISTENT;
            }
            if (!array_key_exists('port', $server)) {
                $server['port'] =  Memcached ::DEFAULT_PORT;
            }
    ...
    public function load($id, $doNotTestCacheValidity = false)
    {
        ...
        if ($doNotTestCacheValidity) {
            $this->_log("\Zend\Cache\Backend\Memcached::load() : \$doNotTestCacheValidity=true is unsupported by the Memcached backend");
        }
    ...
    public function save($data, $id, $tags = array(), $specificLifetime = false)
    {
        ...
        if (count($tags) > 0) {
            $this->_log("\Zend\Cache\Backend\Memcached::save() : tags are unsupported by the Memcached backend");
        }
        return $result;
    }
    ...
    public function clean($mode =  Cache ::CLEANING_MODE_ALL, $tags = array())
    {
        ...
        if ($mode== Cache ::CLEANING_MODE_OLD) {
            $this->_log("\Zend\Cache\Backend\Memcached::clean() : CLEANING_MODE_OLD is unsupported by the Memcached backend");
        }
        if ($mode== Cache ::CLEANING_MODE_MATCHING_TAG) {
            $this->_log("\Zend\Cache\Backend\Memcached::clean() : tags are unsupported by the Memcached backend");
        }
