...
class  Sqlite  extends  Backend  implements  CacheBackendInterface 
{
    /**
     * Available options
     *
    ...
    public function __construct(array $options = array())
    {
        ...
        if (is_null($this->_options['cache_db_complete_path'])) {
             Cache ::throwException('cache_db_complete_path option has to set');
        }
        if (!extension_loaded('sqlite')) {
             Cache ::throwException("Cannot use SQLite storage because the 'sqlite' extension is not loaded in the current PHP environment");
        }
        $this->_getConnection();
    }
    ...
    public function save($data, $id, $tags = array(), $specificLifetime = false)
    {
        ...
        $res = $this->_query($sql);
        if (!$res) {
            $this->_log("\Zend\Cache\Backend\Sqlite::save() : impossible to store the cache id=$id");
            return false;
        }
        $res = true;
        foreach ($tags as $tag) {
    ...
    private function _getConnection()
    {
        if (is_resource($this->_db)) {
            ...
            if (!(is_resource($this->_db))) {
                 Cache ::throwException("Impossible to open " . $this->_options['cache_db_complete_path'] . " cache DB file");
            }
            return $this->_db;
        }
    }
    ...
    private function _registerTag($id, $tag) {
        ...
        if (!$res) {
            $this->_log("\Zend\Cache\Backend\Sqlite::_registerTag() : impossible to register tag=$tag on id=$id");
            return false;
        }
        return true;
    }
    ...
    private function _checkStructureVersion()
    {
        ...
        if (((int) $row['num']) != 1) {
            // old cache structure
            $this->_log('\Zend\Cache\Backend\Sqlite::_checkStructureVersion() : old cache structure version detected => the cache is going to be dropped');
            return false;
        }
        return true;
    }
     ...
     * @return boolean True if no problem
     */
    private function _clean($mode =  Cache ::CLEANING_MODE_ALL, $tags = array())
    {
    ...
    private function _checkAndBuildStructure()
    {
        if (!($this->_structureChecked)) {
            if (!$this->_checkStructureVersion()) {
                ...
                if (!$this->_checkStructureVersion()) {
                     Cache ::throwException("Impossible to build cache structure in " . $this->_options['cache_db_complete_path']);
                }
            }
