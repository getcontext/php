...
class  Core 
{
    ...
    public function save($data, $id = null, $tags = array(), $specificLifetime = false)
    {
        ...
        if ($this->_options['automatic_cleaning_factor'] > 0) {
            ...
            if ($rand==1) {
                if ($this->_backend->isAutomaticCleaningAvailable()) {
                    ...
                    $this->_log('\Zend\Cache\Core::save() / automatic cleaning is not available with this backend');
                }
            }
        }
        ...
        if (!$result) {
            ...
            if ($this->_options['logging']) {
                $this->_log("\Zend\Cache\Core::save() : impossible to save cache (id=$id)");
            }
            $this->remove($id);
            return false;
        }
        if ($this->_options['write_control']) {
            ...
            if ($data!=$data2) {
                $this->_log('\Zend\Cache\Core::save() / write_control : written and read data do not match');
                $this->remove($id);
                return false;
            }
        }
    ...
    public function clean($mode = 'all', $tags = array())
    {
        ...
        if (!in_array($mode, array( Cache ::CLEANING_MODE_ALL,  Cache ::CLEANING_MODE_OLD,  Cache ::CLEANING_MODE_MATCHING_TAG,  Cache ::CLEANING_MODE_NOT_MATCHING_TAG))) {
    ...
    /**
     * Make sure if we enable logging that the  Log  class
     * is available.
    ...
    protected function _loggerSanity()
    {
        ...
        try {
            ...
            require_once 'Zend/Loader.php';
             Loader ::loadClass('\Zend\Log');
        } catch (\Zend_Exception $e) {
             Cache ::throwException('Logging feature is enabled but the  Log  class is not available');
        }
        if (isset($this->_options['logger']) && $this->_options['logger'] instanceof  Log ) {
            return;
        }
        // Create a default logger to the standard output stream
         Loader ::loadClass('\Zend\Log\Writer\Stream');
        $logger = new  Log (new  Stream ('php://output'));
