...
use Zend\File\Transfer\Exception as FileTransferException;
...
abstract class  AdapterAbstract  implements  ValidateInterface 
{
    /**@+
     ...
     * @throws  FileTransferException  on invalid type
     */
    public function setPluginLoader( LoaderPluginLoaderInterface  $loader, $type)
    {
        ...
        switch ($type) {
            ...
            default:
                ...
                throw new  FileTransferException (sprintf('Invalid type "%s" provided to setPluginLoader()', $type));
        }
    }
    ...
    public function getPluginLoader($type)
    {
        ...
        switch ($type) {
            ...
            default:
                require_once 'Zend/File/Transfer/Exception.php';
                throw new  FileTransferException (sprintf('Invalid type "%s" provided to getPluginLoader()', $type));
        }
    }
     ...
     * @param  string $path 
     * @return  AdapterAbstract 
     * @throws  FileTransferException  \for invalid type
     */
    ...
    public function addValidator($validator, $options = null, $files = null)
    {
        if ($validator instanceof  ValidateInterface ) {
            ...
            require_once 'Zend/File/Transfer/Exception.php';
            throw new  FileTransferException ('Invalid validator specified');
        }
    ...
    public function addValidators(array $validators, $files = null)
    {
        foreach ($validators as $key => $spec) {
            if ($spec instanceof  ValidateInterface ) {
     ...
     * @throws  FileTransferException  When file not found
     */
    public function getValidators($files = null)
    {
        ...
        foreach ($files as $file) {
            if (!isset($this->_files[$file])) {
                require_once 'Zend/File/Transfer/Exception.php';
                throw new  FileTransferException ('Unknown file');
            }
        ...
        foreach ($check as $file => $content) {
            ...
            if (array_key_exists('validators', $content)) {
                foreach ($content['validators'] as $class) {
                    ...
                    if (($class === '\Zend\Validate\File\Upload') and (count($this->_messages) > 0)) {
                        $uploaderror = true;
                    }
                }
            }
    ...
    public function addFilter($filter, $options = null, $files = null)
    {
        if ($filter instanceof  FilterInterface ) {
            ...
            require_once 'Zend/File/Transfer/Exception.php';
            throw new  FileTransferException ('Invalid filter specified');
        }
    ...
    public function addFilters(array $filters, $files = null)
    {
        foreach ($filters as $key => $spec) {
            if ($spec instanceof  FilterInterface ) {
     ...
     * @throws  FileTransferException  Not implemented
     */
    public function getFile()
    {
        require_once 'Zend/File/Transfer/Exception.php';
        throw new  FileTransferException ('Method not implemented');
    }
     ...
     * @return \Zend\File\Transfer_Abstract
     */
    ...
    public function getDestination($files = null)
    {
        ...
        if (!is_string($files)) {
            require_once 'Zend/File/Transfer/Exception.php';
            throw new  FileTransferException ('Invalid file value passed to getDestination()');
        }

        if (!array_key_exists($files, $this->_files)) {
            require_once 'Zend/File/Transfer/Exception.php';
            throw new  FileTransferException (sprintf('Unknown file "%s" passed to getDestination()', $files));
        }
     ...
     * 
     * @return string
     * @throws  FileTransferException  if unable to determine directory
     */
        ...
        if (null === $this->_tmpDir) {
            if (function_exists('sys_get_temp_dir')) {
                ...
                if ($tempFile) {
                    ...
                    require_once 'Zend/File/Transfer/Exception.php';
                    throw new  FileTransferException ('Could not determine temp directory');
                }
            }
     ...
     * @throws  FileTransferException  On false filename
     */
    protected function _getFiles($files)
    {
        ...
        if (is_array($files)) {
            foreach ($files as $find) {
                ...
                if ($found === null) {
                    require_once 'Zend/File/Transfer/Exception.php';
                    throw new  FileTransferException (sprintf('"%s" not found by file transfer adapter', $file));
                }
