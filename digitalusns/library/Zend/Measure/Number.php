...
use Zend\Measure\Exception as MeasureException;
use Zend\Measure\MeasureAbstract as MeasureAbstract;
...
class  Number  extends  MeasureAbstract 
{
     ...
     * @param  string             $type   (Optional) A  Number  Type
     * @param  string| Locale  $locale (Optional) A  Locale 
     * @throws  MeasureException  When language is unknown
     * @throws  MeasureException  When type is unknown
     */
    public function __construct($value, $type, $locale = null)
    {
        if ( Locale ::isLocale($type) !== false) {
            $locale = $type;
            $type = null;
        }
        ...
        if ( Locale ::isLocale($locale, true) === false) {
            ...
            throw new  MeasureException ("Language ($locale) is unknown");
        }
        ...
        if (isset($this->_units[$type]) === false) {
            require_once 'Zend/Measure/Exception.php';
            throw new  MeasureException ("Type ($type) is unknown");
        }
     ...
     * @param  integer            $value  Value
     * @param  string             $type   (Optional) A  Number  Type
     * @param  string| Locale  $locale (Optional) A  Locale  Type
    ...
    public function setValue($value, $type = null, $locale = null)
    {
        ...
        if (empty($this->_units[$type])) {
            require_once 'Zend/Measure/Exception.php';
            throw new  MeasureException ('unknown type \of number:' . $type);
        }
        ...
        switch($type) {
            ...
            default:
                try {
                    $value =  Format ::getInteger($value, array('locale' => $locale));
                } catch (\Exception $e) {
                    require_once 'Zend/Measure/Exception.php';
     ...
     * @return string
     * @throws  MeasureException  When more than 200 digits are calculated
     */
    private function _fromDecimal($value, $type)
    {
        ...
        if ($this->_units[$type][0] <= 16) {
            ...
            while (call_user_func( Math ::$comp, $value, 0, 25) <> 0) {
                ...
                if ($count === 0) {
                    require_once 'Zend/Measure/Exception.php';
                    throw new  MeasureException ("Your value '$tempvalue' cannot be processed because it extends 200 digits");
                }
            }
     ...
     * @throws  MeasureException  When a unknown type is given
     * @return void
     */
